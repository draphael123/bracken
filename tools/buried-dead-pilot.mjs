...
try {
  ...
  const camp = await pg.evalp(`(async()=>{const{LEVELS}=await import('/src/level.js');const{T}=await import('/src/level.js');BK.manualSimulation=true;BK.SET.speed=1;const out=[];
    ...
    for(const [px,py] of picks){BK.setHero('knight');BK.reset({fresh:true});BK.load(LEVELS.findIndex(l=>l.id==='burial'));BK.state='play';BK.god=false;
      ...
      const b=BK.boss,P=BK.P;BK.tp(px,py-1);BK.sim(2);for(const k in BK.keys)BK.keys[k]=false;P.hp=P.maxHp;const hitBy={};let lost=0,died=false;
      for(let f=0;f<3600;f++){const h0=P.hp,m=b.mode;BK.sim(1);if(P.hp<h0){hitBy[m]=(hitBy[m]||0)+(h0-P.hp);lost+=h0-P.hp;}if(P.dead){died=true;break;}}
      out.push({perch:[px,py],upPx:A.floor-py*16,lost,died,hitBy});}
    return out;})()`, 600000);
  for (const c of camp) console.log('CAMPER on ' + c.perch + ' (' + c.upPx + ' px up), 60 s standing still: lost ' + c.lost + (c.died ? ' and DIED' : '') + ' ' + JSON.stringify(c.hitBy));
