(async()=>{
 ...
 for(const [id,views] of [['harbor',[[118,29,'customs-hold'],[195,23,'salvage-cranes']]],['burial',[[162,33,'ossuary-shelves'],[235,41,'buried-bridge']]]]){
  ...
  }if(id==='burial'){   /* batch 4b: the Grave Warden's room above, then the Buried Dead */const A=BK.L.arena;BK.look(A.trigger/16+2,A.floor/16-1);BK.sim(300);const b=BK.boss;if(!BK.bossActive||b?.t!=='burieddead')throw Error('Buried Dead did not wake');b.mode='rest';BKT.hurtEnemy(b,99999,b.x+80,false);BK.sim(240);if(b.alive||BK.bossActive||BK.enemies().some(e=>e.graveAdd&&e.alive))throw Error('Buried Dead did not clear');gateOpen=BK.L.grid[31*BK.L.W+A.wallR]!==T.PORT;if(!gateOpen)throw Error('Buried Dead exit locked');}
