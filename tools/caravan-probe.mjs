...
try {
  const r = await pg.evalp(`(async () => {
    ...
    let peak = 0, stages = new Set(); for (let k = 0; k < 60 * 20; k++) { BK.tp(20, foot(20)); BK.step(1); const cv = BK.caravan && BK.caravan(); if (cv) { stages.add(cv.stage || 0); peak = Math.max(peak, cv.stage || 0); } if (peak >= 3) break; }   /* held on the spot (a fight nearby shoves you about) until the build reaches its third stage */
    shots.push(['sun', snap()]);
