...
const shut={...keep,grid:keep.grid.slice()};for(let y=52;y<=58;y++)shut.grid[y*keep.W+109]=T.SOLID;const closed=floodReach(shut,T,{rides:true});assert(closed.jumpNear(101,58),'keeper reachable with door shut');assert(!closed.jumpNear(178,51),'keeper door blocks throne room');
