/* tools/burning-walk.mjs [heroes] - F9 for THE BURNING VILLAGE (docs/briefs/burning-village-rework.md): the in-page play bot
...
try {
  const r = await pg.evalp(`(async()=>{const rep=await BK.playtest({levels:['burning'],heroes:${JSON.stringify(heroes)},mode:'both',quiet:true,log:false});return rep.text;})()`, 1800000);
  console.log(r);
