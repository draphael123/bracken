...
for(const id of ['harbor','burial']){
 ...
 if(id==='burial'){assert.equal(L.mini.boss,'gravewarden','THE GRAVE WARDEN holds the ossuary (batch 4b)');assert.equal(L.arena.boss,'burieddead');assert(L.W+3*Math.max(0,L.H-30)<=720,'THE CAVERNS ARE CUT (claude/burial2): '+L.W+'x'+L.H+' is '+(L.W+3*Math.max(0,L.H-30))+' cols, wanted about 700');assert(reach.jumpNear(exit.x,exit.y),'burial exit reachable after boss');}   /* and on through the guardian checks: his door must hold */
