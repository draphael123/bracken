...
const noop=()=>{},P={x:Math.round((R.x0+R.x1)/2),y:R.y+8,vx:0,vy:20,dead:false,ground:false};const c=vm.createContext({L,P,time:0,jumpPress:false,SFX:new Proxy({},{get:()=>noop})});vm.runInContext(s.slice(s.indexOf('function updateMoorWind'),s.indexOf('function drawMoorWeather')),c);c.updateMoorWind(1/60);assert.equal(P.vx,R.speed);c.jumpPress=true;c.updateMoorWind(1/60);assert.equal(P.vy,-260);assert.equal(P.railRelease,.6);c.jumpPress=false;
