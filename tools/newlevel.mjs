...
for (const lv of LEVELS) {
  ...
  for (const p of (L.pools || [])) {
    if (p.streetTide || p.causeTide || p.arenaTide || p.tide || p.draining || p.sea) continue;   /* causeTide: THE DROWNED CAUSEWAY's tide, built at low water */
  ...
  { const solidish = t => t === T.SOLID || t === T.ONEWAY || t === T.CRATE || t === T.PALISADE || t === T.PLANK || t === T.NET
    ...
    const swims = new Set(['clinger', 'eel', 'siren', 'urchin', 'angler', 'petrel', 'wasp', 'drone', 'spider', 'weaver', 'bat', 'crow', 'harpy', 'kite',
      ...
      'ram', 'sailer', 'turtle', 'crab', 'heronfoe', 'scout', 'siren', 'rook', 'farmhand', 'marshlight', 'haunt', 'boo', 'broom', 'imp',
      'puffer', 'jelly', 'lamprey', 'manta']);   /* THE MAGE'S FOLLY's brooms and imps fly; the fields' boo drifts through walls; the sea's new wildlife swims (puffer, jelly, lamprey) or flies over open water (manta) */
