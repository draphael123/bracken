...
const one = async (p, mode, secs) => {
  await pg.reload();
  const r = await pg.evalp(`(async()=>{BK.SET.speed=1;const seen={};
    const o=await BK.bossLab({bosses:['causeway'],heroes:${JSON.stringify(HEROES)},healthMode:'${mode}',maxSecs:${secs},modes:true,salt:'kraken-pass-${p}',
      onFrame:({boss,h,f})=>{if(f%30)return;seen[h]={stage:boss.stage,turnAt:(boss.turnAt||[]).map(t=>Math.round(t)),ledger:Object.fromEntries(Object.entries(boss.ledger||{}).map(([k,v])=>[k,Math.round(v)])),tide:boss.tideN||0,rung:boss.tideRung||0};}});
