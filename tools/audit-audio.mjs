...
try {
  ...
  if (want('heroes')) { const list = only.heroes || ['knight', 'pyro', 'paladin', 'pirate', 'reaper']; say('heroes'); out.passes.heroes = await ex('__aud.heroes(' + JSON.stringify(list) + ', {})'); say('heroes done: ' + out.passes.heroes.log.length + ' sounds'); }
  ...
  if (want('voices')) {
    ...
    for (const n of info.sfx) { if (n === 'hurtOf' || n === 'dieOf') continue;
      if (heroP.includes(n)) { for (const h of ['knight', 'pyro', 'paladin', 'pirate', 'reaper']) for (const a of (ARGS[n] || [[]])) items.push({ n, args: a, hero: h }); continue; }
