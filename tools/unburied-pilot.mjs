/* tools/grave-warden-pilot.mjs [passes=4] — THE UNBURIED FIELD's STANDARD-BEARER (mini) or FIRST DEATH KNIGHT (boss, arg 2 = boss) at NORMAL health: all six heroes, `passes` seeded passes each (four is
...
try {
  for (let p = 0; p < passes; p++) { await pg.reload();
    const r = await pg.evalp(`(async()=>{BK.manualSimulation=true;let seed=${4051 + p * 173};Math.random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296};
      const o=await BK.bossLab({bosses:['unburied'],mini:${JSON.stringify(process.argv[3] !== 'boss')},healthMode:'normal',maxSecs:240,modes:true});
