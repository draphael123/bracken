...
// This drives each hero in the page (god mode, THE WORM'S HOLLOW's forty flat tiles before he is woken - the level's longest flat):
...
try {
  const r = await pg.evalp(`(async () => {
    ...
    for (const hero of ${JSON.stringify(heroes)}) {
      ...
      const L = BK.L, x0 = L.arena.x0 / 16 - 12, row = L.arena.floor / 16 - 1; BK.tp(x0, row); BK.sim(30);
