...
for(const lv of LEVELS){assert(GROUND_KITS[lv.id],lv.id+' needs its own ground kit');assert(ALLOWED_DECORATIONS[lv.id],lv.id+' needs an allowlist');const L=lv.build();for(const kind of [...GROUND_KITS[lv.id].kinds,...(DRESS[lv.id]||[]).map(x=>x[0]),...L.ents.filter(e=>e.t==='deco').map(e=>e.kind),...(L.groundZones||[]).flatMap(z=>z.kit||[])]){count++;if(!permitted(lv.id,kind))bad.push(lv.id+': '+kind);}}
