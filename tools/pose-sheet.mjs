...
try {
  ...
  const before = MODE === 'after' ? 'data:image/png;base64,' + readFileSync(join(ROOT, 'docs/pose-before.png')).toString('base64') : null;
  const url = await pg.evalp(`(async()=>{const rows=${JSON.stringify(rows)},shots=__shots,W=${CW * SC},H=${CH * SC},LW=150,cols=4,gap=6,half=cols*(W+gap);
    ...
    const x0=LW+(pair?half+20:0);g.fillText(pair?'BEFORE (master 313e0da)':'BEFORE: master 313e0da',LW,24);if(pair)g.fillText('AFTER (claude/poses)',x0,24);
  ...
  const out = join(ROOT, 'docs/pose-' + MODE + '.png');
