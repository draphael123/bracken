  ...
  for (const id of ids) {
    ...
    for (const f of res) {
      ...
      if (f.png) {
        ...
        const tags = [f.dark.bad && 'dark', f.foot.bad && 'footing', f.creatures.some(c => c.bad) && 'creature', f.hero && f.hero.bad && 'hero', f.boss && 'boss-' + f.boss.t, f.rep && 'rep'].filter(Boolean).join('+') || 'clean';
