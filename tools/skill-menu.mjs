...
}return{rows,shots};})()`);assert.equal(r.rows.length,6);assert.deepEqual(pg.errors,[]);if(process.env.SKILL_SCREEN)for(const s of r.shots)writeFileSync(process.env.SKILL_SCREEN+'-'+s.name+'.png',Buffer.from(s.png.split(',')[1],'base64'));console.log(JSON.stringify({rows:r.rows}));
}finally{pg.close();}
