...
const SWORDS = listOf('SWORDS'), SKINS = listOf('SKINS'), UPGRADES = listOf('UPGRADES'), CHARMS = listOf('CHARMS'), HEROES = listOf('HEROES'), TRAINING = listOf('TRAINING'), ABILITIES = listOf('ABILITIES');
...
const HERO = {
  knight: { hp: 100, hit: 10, top: 10 + 6 + 5 },
  pyro: { hp: 80, hit: Math.round(10 * 0.7), top: Math.round(21 * 0.7) },
  paladin: { hp: 120, hit: 16, top: 16 + 6 + 5 },
};
...
for (const lv of LEVELS) {
  ...
  const htk = (hp, h) => `${Math.ceil(hp / HERO.knight[h])}/${Math.ceil(hp / HERO.pyro[h])}/${Math.ceil(hp / HERO.paladin[h])}`;
  ...
  const htd = `${Math.ceil(HERO.knight.hp / blow)}/${Math.ceil(HERO.pyro.hp / blow)}/${Math.ceil(HERO.paladin.hp / blow)}`;
...
const useful = UPGRADES.reduce((a, s) => a + s.price, 0) + knightKit + TRAINING.filter(t => t.id === 'vigour' || t.id === 'temper').reduce((a, t) => a + t.prices.reduce((x, y) => x + y, 0), 0);
console.log(`  the power buys (gear + knight skills + vigour + temper): ${useful}`);
