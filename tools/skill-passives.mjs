...
for(const h of ['knight','pyro','paladin','pirate','reaper','warden']){
 prep(h,[]);for(const n of SKILLS.filter(n=>n.hero===h&&!n.active)){/* PASSIVES COME WITH LEVELS (hero-kits 1b): on at level 16 with nothing owned or slotted, never in a slot, and off when its level is out of reach */if(BKT.tal(n.id)!==1)throw Error('passive not on at its level '+n.id);BKT.PROG.loadouts[h]=[n.id];if(BKT.skillAt(0)!==null)throw Error('passive in a slot '+n.id);BKT.PROG.loadouts[h]=[];const lv=n.level;n.level=99;const off=BKT.tal(n.id);n.level=lv;if(off!==0)throw Error('passive on above its level '+n.id);catalog.push(h+'/'+n.id);}
}
for(const [h,id]of[['knight','flurry'],['pyro','heatShield'],['paladin','sanctuary'],['pirate','shareOut'],['reaper','gleaner'],['warden','keenPoint']]){const values=[];const node=SKILLS.find(n=>n.hero===h&&n.id===id),lv0=node.level;for(const enabled of [false,true]){
 /* OFF is the passive's level moved out of reach (it is on at 16 by its level alone now); ON is its own level */node.level=enabled?lv0:99;prep(h,[]);let value;
