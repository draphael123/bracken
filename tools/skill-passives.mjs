...
const prep=(h,ids)=>{for(const k in BK.keys)BK.keys[k]=false;BK.setHero(h);BK.reset({fresh:true});BKT.PROG.xp[h]=xpFloor(16);BKT.PROG.skillOwned[h]=Object.fromEntries(ids.map(id=>[id,true]));BKT.PROG.loadouts[h]=ids;BK.applyUpgrades();BK.load(0);BK.state='play';BK.enemies().forEach(e=>e.alive=false);BK.ambushes().forEach(a=>a.st='done');BK.tp(3,21);BK.sim(3);BK.P.hp=BK.P.maxHp/2;BK.P.inv=0;BK.P.st=BK.P.maxSt;BK.P.hurt=0;};
for(const h of ['knight','pyro','paladin','pirate','reaper','warden']){
 prep(h,[]);for(const n of SKILLS.filter(n=>n.hero===h&&!n.active)){BKT.PROG.skillOwned[h][n.id]=true;BKT.PROG.loadouts[h]=[n.id];if(BKT.tal(n.id)!==1||BKT.skillAt(0)!==null)throw Error('passive binding '+n.id);BKT.PROG.loadouts[h]=[];if(BKT.tal(n.id)!==0)throw Error('unequipped passive '+n.id);catalog.push(h+'/'+n.id);}
}
for(const [h,id]of[['knight','flurry'],['pyro','heatShield'],['paladin','sanctuary'],['pirate','shareOut'],['reaper','gleaner'],['warden','keenPoint']]){const values=[];for(const enabled of [false,true]){
 prep(h,enabled?[id]:[]);let value;
 ...
 if(h==='reaper'){BKT.PROG.skillOwned[h].summonSkeleton=true;BKT.PROG.loadouts[h]=[enabled?id:null,'summonSkeleton'];BK.press('skill2');BK.sim(1);value=BK.P.cds.summonSkeleton;}
...
assert.equal(r.catalog.length,134);for(const r0 of r.rows)assert.notEqual(r0.baseline,r0.equipped,r0.id);assert.deepEqual(pg.errors,[]);console.log(JSON.stringify(r));
}finally{pg.close();}
