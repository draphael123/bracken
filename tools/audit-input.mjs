...
//   coyote     walk off a ledge, press jump k frames after the feet leave it: the last k that still jumps
//   buffer     fall onto flat ground, press jump k frames before landing: the last k that still jumps on landing
...
async function INPUT(o) {
  ...
  { fresh(ledge.x - 6, ledge.y); const x0 = P.x; BK.keys.right = true; let fl = null; for (let f = 0; f < 120; f++) { BK.sim(1); if (!P.ground) { fl = f; break; } }
    const res = []; for (let k = 0; k <= 14; k++) { fresh(ledge.x - 6, ledge.y); BK.keys.right = true; let off = null, ok = false;
      ...
      res.push(ok ? 1 : 0); }
    out.coyote = { byK: res.join(''), frames: res.lastIndexOf(1), leftAtF: fl }; }
  /* JUMP BUFFER */
  { const drop = () => { fresh(flat.x, flat.y); P.y -= 64; P.vy = 0; P.ground = false; };
    ...
    const res = []; for (let k = 0; k <= 14; k++) { drop(); let ok = false;
      ...
      res.push(ok ? 1 : 0); }
    out.buffer = { byK: res.join(''), frames: res.lastIndexOf(1), landAt: land }; }
