/* tools/gargoyle-pilot.mjs [passes=4] — THE GATE GARGOYLE (the Witchlight Stair's boss) at NORMAL health: all six heroes, `passes` seeded passes each (four is
   twenty-four fights, over the brief's twenty-one). One life per fight, no refills: what the bot loses it keeps. Prints a row
   per fight (outcome, seconds, health left, how often he HUNG from a broken edge, which of his attacks did the damage) and the summary the
...
try {
  for (let p = 0; p < passes; p++) { await pg.reload();
    const r = await pg.evalp(`(async()=>{BK.manualSimulation=true;let seed=${7331 + p * 211};Math.random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296};
      const o=await BK.bossLab({bosses:['witchlight'],healthMode:'normal',maxSecs:300,modes:true});
