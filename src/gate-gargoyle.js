...
// (its GARGOYLE section, unchanged from witchlight-stair.md), REWORKED 2026-09-25: docs/briefs/gargoyle-rework.md. A huge stone
// gargoyle bolted over the tower's outer gate, woken by the loose magic. He is fought on the stair's top: FLOATING SLABS that drift
// over the garden terrace, some solid, some CRACKED. His art: src/redraw/queue_bosses.js (bakeGateGargoyle, drawn at GARG.K = 1.5):
// 0 perched | 1,2 fly | 3 dive tell | 4 dive | 5 gust tell | 6 gust | 7 spit tell | 8 spit | 9 shriek | 10 STUNNED | 11 CRASH.
...
// GARG.stun seconds, every blow counting twice. Still on it when he lands and it is you he lands on. GARG.smashAny says which slabs
// give: every one (Daniel's ask), or false for the CRACKED ones only. A broken slab grows back (GARG.regrow, slower in phase two)
// and never fewer than GARG.minLive stand, so the room does not run out of footing; the rune columns lift you from the floor.
// HE WILL NOT COME DOWN TO THE GARDEN OTHERWISE: fall to the terrace and he keeps above the slabs and throws rubble. PHASE TWO (half
// his health): the slabs drift faster, dives come in pairs, every flare comes with a gust, and a slab he breaks is gone nearly twice
// as long. Touching him never hurts (the touch rule).
export const GARG = {
  ...
  dmg: { dive: 18, gust: 6, spit: 7, crash: 12 },
...
/* HIS HEALTH: a blow while he lies stunned on the floor counts twice; nothing reaches him asleep on the gate */
export function gargTake(e, dmg) { if (!(dmg > 0) || e.mode === 'sleep') return 0; return gargOpen(e) ? Math.round(dmg * GARG.stunMul) : dmg; }
...
/* THE CAMERA FRAMES HIM AND YOU TOGETHER (Daniel: "much further zoomed out"): the point between you, weighted to you, never letting
   you out of the frame; and low enough that the garden floor - where he lies when he is open - is in it while you are over it.
...
export function gargCam(P, e, A, VW, VH) {
  ...
  let ty = my - VH * 0.5; ty = Math.max(ty, A.floor + 20 - VH);                     /* never below the garden floor's own edge... */
...
export function updateGargoyle(e, dt, c) {
  ...
  switch (e.mode) {
    ...
    case 'hover': {
      ...
      const tx = Math.max(A.x0 + 16 * K, Math.min(A.x1 - 16 * K, P.x + e.side * e.hx)), ty = terrace ? top - GARG.high : Math.min(P.y, top + 32) - GARG.hoverUp;
    ...
    case 'dive': { const m = e.tgt && !e.tgt.broken ? e.tgt : null, gy = m ? m.y : A.floor; e.x = m ? footX(e, m) : e.tx; e.y += GARG.diveV * dt; e.vx = 0; e.vy = GARG.diveV;
      if (e.y < gy) return;
    ...
    /* THE CRASH: through it and down to the garden floor. Anyone under it is hit */
    case 'crash': { e.vx = 0; e.vy += GARG.crashG * dt; e.y += e.vy * dt; if (e.y < A.floor && e.modeT > 0) return;
      ...
      if (!P.dead && Math.abs(P.x - e.x) < 20 * K && Math.abs(P.y - A.floor) < 30) c.hit(e.x, GARG.dmg.crash, true, 'THE CRASH');
      e.mode = 'stunned'; e.modeT = GARG.stun; e.open = GARG.stun; c.say('STUNNED: STRIKE HIM NOW', false, true); c.sound('crack'); return; }
    case 'stunned': if (e.modeT <= 0) { e.mode = 'rise'; e.modeT = GARG.rise * 1.6; e.cd = 0.9; c.say('HE SHAKES IT OFF', false); } return;
    ...
    case 'spitTell': { const sd = e.sd = e.sd || side(); toward(e, P.x + sd * 84 * K, (terrace ? top - 30 : P.y - 40), dt, 3); e.face = -sd;
      if (e.modeT <= 0) { e.mode = 'spit'; e.modeT = 0.35; c.sound('spit'); const mx = e.x + e.face * 14 * K, my = e.y - 18 * K, a0 = Math.atan2(P.y - 10 - my, P.x - mx);
...
/* HIS MARKS ON THE WORLD: the dive's shadow (and its red cross) on the slab he is coming down on - and on the garden under it once the
   slab is empty, since that is where he is going; the crack running across a slab as it gives; the glyph lit under a slab; the rubble;
   the gust's wind; and the stunned mark, circling stars in a green ring, for as long as he lies open */
export function drawGargoyleWorld(g, e, cx, cy, time, P) {
  ...
  if (e.mode === 'stunned') { const k = 0.5 + 0.5 * Math.sin(time * 10), hx = Math.round(e.x + (e.face || 1) * 12 * K - cx), hy = Math.round(e.y - 18 * K - cy), left = Math.max(0, e.modeT) / GARG.stun;
    ...
    g.fillStyle = '#8fd160'; g.globalAlpha = 0.8; g.fillRect(Math.round(e.x - cx) - 14, Math.round(e.y - 30 * K - cy), Math.round(28 * left), 2); g.globalAlpha = 1; }   /* and how long he has left down there */
}
