...
const HEROES = [
  ...
  { id: 'pyro', name: 'THE PYROMANCER', price: 15, silver: true, desc: 'staff and ember, no shield. tap C for an ember, hold C for a jet of flame. 70 health, quicker on foot, a longer reach, a lighter blow. plunge and flame kick in the air' },
];
const TRAINING = [
...
const BEASTS = [
  ...
  { t: 'golem', name: 'THE FACET', sub: 'the heart of the glassworks', desc: 'Each facet wants its own colour: blue from the left crystal, violet from the right, green from the roof, and the last one two colours at once. Lit for two seconds it drinks the beam and throws it back in a sweep, the one light in the works that burns you. A golem grown from the cavern crystal. Dark, it turns every blade. Put a beam on it and it bleeds: turn the two mirrors so the light crosses the floor, and cut it while it stands lit, facet by facet. Every facet that shatters staggers it and makes it faster. It stamps, and a stamp turns any mirror it stands by. It throws shards. Lit too long, it raises ice against the beam: the beam melts that too.' },
...
const JET_LEN = 84; // five tiles of flame
...
function updatePlayer(dt) {
  ...
  P.jet = false;
  if (isPyro()) { // C tapped = an ember; C held = the jet, a held tongue of flame that costs heat. Overheated, you have neither.
    ...
    P.jet = keys.block && P.cHeld >= 0.14 && !(P.overheat > 0) && !stunned && !dodging && !attacking && !P.plunge && !thrown;
    if (P.jet) { P.heat += 18 * dt; if (P.heat >= 100) { P.heat = 100; P.overheat = 1.6; P.jet = false; number(P.x, P.y - 24, 'OVERHEATED', '#ff6b6b'); SFX.gasp(); } }
  ...
  if (isPyro() && P.jet) { // the jet: a held tongue of flame five tiles long. It burns what stands in it and what flies through it. Heat is the cost.
    ...
    P.jetTick = (P.jetTick || 0) - dt; if (P.jetTick <= 0) { P.jetTick = 0.2; for (const e of enemies) if (e.alive && !e.harmless && e.x + e.w / 2 > j.l && e.x - e.w / 2 < j.r && e.y > j.t && e.y - e.h < j.b) { hurtEnemy(e, e.maxHp ? 2 : 4, P.x - P.face * 10, false); e.burn = Math.max(e.burn || 0, 0.8); } if (Math.random() < 0.4) SFX.puff(); }
  ...
  const cap = ((P.block || P.jet) ? 32 : wading ? 46 : spored ? 40 : RUN * (PROG.charm === 'swift' ? 1.12 : 1) * (isPyro() ? 1.15 : 1)) + (P.gustT > 0 && !P.ground ? 150 : 0); // a gust can carry you faster than your legs
  if (move && !groundAtk) {
...
function updateEmbers(dt) {
  for (const b of embers) { b.life -= dt; b.vy += 110 * dt; b.x += b.vx * dt; b.y += b.vy * dt;
    ...
    if (b.life > 0 && isSolid(tx, ty)) { b.life = 0; const fy = isSolid(tx, ty) && !isSolid(tx, ty - 1) ? ty * TS : Math.floor((b.y - b.vy * dt) / TS) * TS + TS; if (b.vy > 0) fires.push({ x: b.x, y: fy, life: 1.2, delay: 0, own: true }); burst(b.x, b.y, 6, ['#ff9a5c', '#ffd36b'], 50, 0.35, 100, 1); SFX.crack(); }
  }
...
function drawWorld(cx, cy, showPlayer) {
  ...
  if (showPlayer && !P.dead) {
    ...
    if (vis) {
      ...
      else if (P.block || P.jet) { key = 'block'; frame = Math.floor(P.anim * 2) % 2; }
