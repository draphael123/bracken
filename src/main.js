...
import { bakeKeeper, bakeBard, bakeOldKnight, bakeMiner, bakeBat, bakeForeman, bakeLamplighter, bakeForgemaster, bakePyro, bakeCragRam, bakeTroll, bakeGreatHound, bakeSpider, bakeSquirrel, bakeOwl, bakeWoodsman, bakeFerryman, bakeSquire, bakeElder, bakeHarpy, bakeGoatRider, bakeRamLord, bakeShepherd, bakeSheep, bakeKnight, bakeSprig, bakeShield, bakeSpitter, bakeSpitterParts, bakeWasp, bakeSeed, bakeThornback, bakeQueen, bakeArcher, bakeBird, bakeFrog, bakeHopper, HOPPER_COLORS, bakeSapper, bakeBomb, bakeBrute, bakeHound, bakeFox, bakeChief, bakeSporeling, bakeLurker, bakeDrone, bakeShaman, bakeThief, bakePike, bakeFolk, bakeMaster, bakeKing } from './chars.js';
...
function spawnEnt(e) {
  ...
  const n0 = enemies.length;
  {
    ...
    switch (e.t) {
      ...
      case 'minerlamp': props.push({ t: 'minerlamp', x: px, y: py, lit: e.lit !== false, hitCd: 0 }); lights.push({ x: px, y: py - 8, r: 60, glow: true, warm: true, lantern: props[props.length - 1] }); break;
...
function updateForgemaster(e, dt) {
  ...
  if (e.mode !== 'wake') {
    ...
    e.cartT -= dt; if (e.cartT <= 0) { e.cartT = p2 ? 4.5 : 6; const c = movers.find(mv => mv.kind === 'cart' && mv.gone && mv.auto); if (c) { c.gone = false; c.x = c.x0; c.y = c.y0; c.vx = 0; c.vy = 0; c.rolling = true; c.ridden = false; c.kicked = false; c.hit.clear(); number(c.x0 + 14, c.y0 - 12, 'A CART COMES', '#8a919c'); SFX.stone(); } }
    ...
    if (p2 && !e.doused) { e.doused = true; L.dark = 0.88; for (const pr of props) if (pr.t === 'minerlamp') pr.lit = false; number(e.x, e.y - e.h - 12, 'THE LAMPS GO OUT. STRIKE THEM BACK', '#c080ff'); SFX.roar(); for (const dx of [-90, 90]) enemies.push({ t: 'bat', x: e.x + dx, y: floor - 90, hx: e.x + dx, hy: floor - 90, w: 10, h: 6, hp: EHP.bat, hp0: EHP.bat, mode: 'hang', modeT: 0, cd: 0.5, face: -1, alive: true, dying: 0, anim: Math.random(), flash: 0, stagger: 0, vx: 0, vy: 0 }); }
  }
  let want = 0;
  switch (e.mode) {
    ...
    case 'kickTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'kick'; e.modeT = 0.4; const c = movers.find(mv => mv.kind === 'cart' && mv.gone); if (c) { c.gone = false; c.x = e.x + e.face * 20; c.y = floor - c.h; c.vx = e.face * 220; c.vy = 0; c.rolling = true; c.dir = e.face; c.kicked = true; c.ridden = false; c.hit.clear(); number(e.x, e.y - e.h - 12, 'A CART, KICKED', '#ff6b6b'); SFX.heavy(); } } break;
...
function updateProps(dt) {
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'minerlamp') { pr.hitCd = Math.max(0, pr.hitCd - dt); if (!pr.lit && !P.dead) { // a cold lamp: strike it with the sword, an ember, or the pyromancer's flare
      ...
      const flare = isPyro() && P.block && Math.abs(P.x - pr.x) < 26 && Math.abs(P.y - pr.y) < 26;
