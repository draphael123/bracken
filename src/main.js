...
const TIER = { wood: 0, marsh: 0.15, stockade: 0.3, spore: 0.45, kings: 0.6, scree: 0.75, hanging: 0.9, spire: 1, moor: 1.1, storm: 1.2 }; // how far up the slope a level sits
...
function bakeAll(pal = {}) {
  ...
  PROP.relic.fleece = ART.bakeFleeceIcon(); PROP.relic.spurs = ART.bakeSpursIcon(); PROP.relic.shoes = ART.bakeShoesIcon(); PROP.relic.sunshard = ART.bakeSunshardIcon(); PROP.relic.lamp = PROP.relic.lamp || PROP.lampIcon; // the miner's lamp relic had no icon: the HUD threw every frame once you held it PROP.relic.windcloak = (() => { const [c, g] = canvas(10, 12); g.fillStyle = '#bfe6f5'; g.beginPath(); g.moveTo(5, 0); g.lineTo(9, 3); g.lineTo(9, 11); g.lineTo(5, 9); g.lineTo(1, 11); g.lineTo(1, 3); g.closePath(); g.fill(); g.fillStyle = '#7aa8c8'; g.fillRect(4, 1, 2, 8); g.fillStyle = '#ffd36b'; g.fillRect(4, 0, 2, 1); return c; })();
...
const MEDALS = { storm: [600, 820, 1150], moor: [480, 660, 960], scree: [450, 630, 920], spire: [520, 700, 980], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
function updateCrystal(dt) {
  ...
  for (const sh of shards) { sh.vy = Math.min(340, sh.vy + 780 * dt); sh.y += sh.vy * dt; sh.life -= dt;
    ...
    if (isSolid(tx, ty)) { sh.life = 0; burst(sh.x, sh.y, 6, ['#dff2ff', '#8fc8e8'], 50, 0.4); SFX.clank(); } }
...
const BEASTS = [
  ...
  { t: 'suncatcher', name: 'THE SUNCATCHER', sub: 'it has been drinking this mountain', desc: 'It does not chase. It stands on the peak and works on the ground under you: it raises spires out of the rock, it throws the light back in handfuls, and when it drinks it crazes every crystal you are standing on and burns you off the glass. While it is bright nothing touches it. Break one of its own spires down onto its head and it goes dim, and a dim Suncatcher takes everything, and a half more.' },
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster', 'golem', 'windcaller', 'lance', 'suncatcher'];
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.hp <= 0) {
    ...
    if (e.t === 'chief') chiefDies(); else if (e.t === 'queen' || e.t === 'frog' || e.t === 'king' || e.t === 'ram' || e.t === 'owl' || e.t === 'forgemaster' || e.t === 'golem' || e.t === 'windcaller' || e.t === 'lance' || e.t === 'suncatcher') queenDies(e);
...
function updatePlayer(dt) {
  ...
  for (const e of enemies) {
    ...
    if (e.t === 'brute' || e.t === 'chief' || e.t === 'mother' || e.t === 'gill' || e.t === 'heart' || e.t === 'folk' || e.t === 'king' || e.t === 'bearer' || e.t === 'pike' || e.t === 'master' || e.t === 'thief' || e.t === 'ram' || e.t === 'goat' || e.t === 'harpy' || e.t === 'troll' || e.t === 'greathound' || e.t === 'spider' || e.t === 'owl' || e.t === 'miner' || e.t === 'bat' || e.t === 'forgemaster' || e.t === 'lance' || e.t === 'suncatcher' || (e.big && e.t !== 'sailer')) continue; // their damage comes from their attacks, not from touching them
...
function desiredView() { if (state === 'editor') return 'zoom';
  const inLevel = state === 'play' || state === 'talk' || state === 'win' || state === 'gameover' || (state === 'menu' && menuFrom === 'play'); if (!inLevel) return 'normal'; return (SET.zoom === 'wide' || (bossActive && boss && (boss.t === 'mother' || boss.t === 'owl' || boss.t === 'forgemaster' || boss.t === 'golem' || boss.t === 'windcaller' || boss.t === 'king' || boss.t === 'lance' || boss.t === 'suncatcher'))) ? 'zoom' : 'normal'; }
