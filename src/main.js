...
function bakeAll(pal = {}) {
  ...
  const sky = (pal.sky === 'night' || pal.sky === 'teal' || pal.sky === 'autumn' || pal.sky === 'crag' || pal.sky === 'sea') ? null : (pal.sky || [[104, 170, 220], [205, 232, 210]]);
  SHORE = SHORE || LWT.bakeShoreTiles(); PROP.fallArt = PROP.fallArt || { make: h => LWT.bakeWaterfall(h) };
  BG = { sky: pal.sky === 'sea' ? LWT.bakeSkySea(VH) : sky ? ART.bakeSky(VH, sky[0], sky[1]) : pal.sky === 'teal' ? ART.bakeSkyTeal(VH) : pal.sky === 'autumn' ? ART.bakeSkyAutumn(VH) : pal.sky === 'crag' ? ART.bakeSkyCrag(VH) : ART.bakeSkyNight(VH), skyDusk: ART.bakeSkyDusk(VH), sun: ART.bakeSun(), far: pal.far === 'sea' ? LWT.bakeFarSea(320, 90, 1) : pal.far === 'crag' ? ART.bakeFarCrags(320, 90, 1) : ART.bakeFar(320, 90, 1), mid: pal.mid === 'coast' ? LWT.bakeMidCoast(480, 140, 2) : pal.mid === 'crag' ? ART.bakeMidCrags(480, 140, 2) : ART.bakeMid(480, 140, 2), near: pal.near === 'shore' ? LWT.bakeNearShore(640, 300, 3) : pal.near === 'crag' ? ART.bakeNearCrag(640, 300, 3) : pal.near === 'mushroom' ? ART.bakeNearMushrooms(640, 300, 3) : pal.near === 'autumn' ? ART.bakeNearAutumn(640, 300, 3) : ART.bakeNear(640, 300, 3, pal.canopy), nearTrees: pal.near === 'mushroom' ? ART.bakeNear(640, 300, 5, pal.canopy) : null, fg: pal.fg === 'shore' ? LWT.bakeFGShore(640, VH, 4) : ART.bakeFG(640, VH, 4) };
}
bakeAll();
applySkin();
...
function resolveTiles() {
  ...
  for (let y = 0; y < LH; y++) for (let x = 0; x < LW; x++) {
    ...
    const shore = L.palette && L.palette.set === 'shore' && SHORE, wetT = shore && L.wetZone && x >= L.wetZone[0] && x <= L.wetZone[1];
    if (underPool) { tileSpr[y * LW + x] = shore ? SHORE.silt[(rnd() * 3) | 0] : TILE.silt[(rnd() * 3) | 0]; continue; }
    if (t === T.SOLID) {
      ...
      if (up !== T.SOLID && up !== T.CRATE) {
        s = shore ? (wetT && eL + '' + eR === '00' ? SHORE.wet[(rnd() * 3) | 0] : SHORE.top[eL + '' + eR][(rnd() * 4) | 0]) : L.palette && L.palette.myc ? TILE.mycTop[eL + '' + eR][(rnd() * 3) | 0] : TILE.top[eL + '' + eR][(rnd() * 4) | 0];
      ...
      } else if (eL || eR) s = shore ? SHORE.edge[eL + '' + eR][(rnd() * 2) | 0] : TILE.edge[eL + '' + eR][(rnd() * 2) | 0];
      else if (shore) s = SHORE.fill[(rnd() * 4) | 0];
      ...
      const crag = L.palette && L.palette.dress === 'crag', shoreOW = L.palette && L.palette.set === 'shore' && SHORE;
      s = inHive ? (!l ? TILE.combL : !r ? TILE.combR : TILE.comb[(rnd() * 3) | 0]) : shoreOW ? (!l ? SHORE.ledgeL : !r ? SHORE.ledgeR : SHORE.ledge[(rnd() * 3) | 0]) : crag ? (!l ? TILE.ledgeL : !r ? TILE.ledgeR : TILE.ledge[(rnd() * 3) | 0]) : !l ? TILE.logL : !r ? TILE.logR : TILE.log[(rnd() * 3) | 0];
...
const MEDALS = { longwater: [540, 760, 1080], crown: [660, 900, 1260], storm: [600, 820, 1150], moor: [480, 660, 960], scree: [450, 630, 920], spire: [520, 700, 980], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
const COAST_PATH = [[48, 172], [74, 160], [108, 150], [134, 132], [152, 104]]; // down off Highcrown's back face to the river and the town
...
const NODE_AT = [0, 3, 6, 8, 10, 12, 20, 23, 25, 28, 31, 34, 39, 44]; // PATH index of each node: wood 0-5, the crags, then the coast
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster', 'golem', 'windcaller', 'lance', 'roc', 'gqueen', 'herald'];
...
function hurtEnemy0(e, dmg, fromX, plunge) {
  ...
  if (e.hp <= 0) {
    ...
    if (e.t === 'chief') chiefDies(); else if (!e.mini && (e.t === 'gqueen' || e.t === 'queen' || e.t === 'frog' || e.t === 'king' || e.t === 'ram' || e.t === 'owl' || e.t === 'forgemaster' || e.t === 'golem' || e.t === 'windcaller' || e.t === 'lance' || e.t === 'suncatcher' || e.t === 'roc' || e.t === 'herald')) queenDies(e); // (a boss missing from this list leaves the walls shut and the fight running)
...
function updatePlayer(dt) {
  ...
  P.swim = !!swimP; const breathMax = P.relic === 'tidecharm' ? 12 : 6;
  if (P.swim) { const under = P.y - swimP.y;
    P.vy += 150 * dt; if (under > 15) P.vy -= 560 * dt; if (keys.down) P.vy += 380 * dt; if (keys.up || (keys.jump && under > 18)) P.vy -= 320 * dt;
    ...
    if (under > 24) { P.breath = (P.breath ?? breathMax) - dt; if (Math.random() < dt * 4) parts.push({ x: P.x + (Math.random() - 0.5) * 6, y: P.y - 16, vx: 0, vy: -30, life: 0.9, max: 0.9, col: '#e8f4f0', size: 1, grav: -20 });
...
const TAM_MAP = {
  ...
  storm: ['HER KNIGHT HOLDS THE BRIDGE.', 'HER CASTLE IS ABOVE US.'], crown: ['THE QUEEN. THE LAST OF THEM.', 'IT IS DONE. WE CAN GO HOME.'], longwater: ['THE RIVER RUNS SALT.', 'THE GLAIVE POINTS OUT TO SEA.'],
};
const NPC_LINES = pr => {
...
const GROUNDED_DECO = new Set(['spire', 'cairn', 'stone', 'bones', 'tent', 'lanternPost', 'barrels', 'cart', 'well', 'spearRack', 'skullPile', 'deadTree', 'anvil', 'forge', 'frozen', 'coralTuft', 'barnacleRock', 'drownedHut', 'fishCottage', 'bellTower', 'seaLantern', 'tributeChest', 'rowboat', 'netPoles', 'tidePool']);
...
function updateProps(dt) {
  ...
  for (const z of (L.gusts || [])) { if (z.arena && (!bossActive || callerCalm())) continue; const ph = (time + (z.phase || 0)) % z.period, on = ph < z.on, soon = ph > z.period - 0.5; const zd = z.alt ? (Math.floor((time + (z.phase || 0)) / z.period) % 2 ? -z.dir : z.dir) : z.dir; if (P.x > z.x0 && P.x < z.x1 && P.y > z.y0 && P.y <= z.y1 + 4) { windFx.dir = zd; windFx.on = on; windFx.soon = !on && soon; windFx.k = z.k || 1; windFx.t = 0.2; if (on && !P.dead) { P.vx += zd * (P.ground ? 200 : 260) * (z.k || 1) * (z.brace && (P.block || P.aegis || P.jet) ? 0.3 : 1) * dt; if (z.moor) P.gustT = 0.25; if (Math.random() < dt * 40) parts.push({ x: camX + Math.random() * VW, y: z.y0 + Math.random() * (z.y1 - z.y0), vx: zd * 220, vy: 0, life: 0.35, max: 0.35, col: '#dfe8c0', size: 1, grav: 0 }); } else if (soon && Math.random() < dt * 12) parts.push({ x: camX + Math.random() * VW, y: z.y0 + Math.random() * (z.y1 - z.y0), vx: zd * 90, vy: 0, life: 0.4, max: 0.4, col: '#c9d1a0', size: 1, grav: 0 }); } }
  for (const p of (L.pools || [])) if (p.streetTide) { // SALTREACH'S TIDE: in and out on a slow beat, the bell as it turns
    ...
    if (p.lastK !== undefined && (p.lastK < 0.97) !== (k < 0.97) && Math.abs(P.x - p.x0) < 900) SFX.seaBell(); p.lastK = k; }
  updateBore(dt);
...
function drawWater(cx, cy, surfaceOnly = false) {
  for (const p of (L.pools || [])) {
    ...
    if (p.dry) continue;
    const x0 = Math.max(p.x0, cx) - cx, x1 = Math.min(p.x1, cx + VW) - cx, y = p.y - cy, h = p.shallow ? (p.depth || 22) : p.bottom !== undefined ? Math.max(4, p.bottom - p.y) : VH - y, sea = L.palette && L.palette.set === 'shore';
    if (!surfaceOnly) {
      if (p.shallow) {
        const d = p.depth || 22;
    ...
    if (p.swim && !p.shallow) { g.globalAlpha = 0.5; g.fillStyle = sea ? '#2e7a88' : '#3b7fae'; g.fillRect(x0, y + 3, x1 - x0, h - 3); g.globalAlpha = 1; } // over whoever is swimming in it: they are IN the water
