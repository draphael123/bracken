...
const TIER = { wood: 0, marsh: 0.15, stockade: 0.3, spore: 0.45, kings: 0.6, scree: 0.75, hanging: 0.9, mineworks: 1, moor: 1.1, storm: 1.2 }; // how far up the slope a level sits
...
const HEROES = [
  ...
  { id: 'pyro', name: 'THE PYROMANCER', price: 15, silver: true, desc: 'staff and ember, no shield. tap C for an ember, hold C for a jet of flame. the hotter she runs the harder everything she throws lands, up to half again at a full bar. 70 health, quicker on foot, a lighter blow. one flame kick in the air, and it is paid back if it connects' },
];
const TRAINING = [
...
function bakeAll(pal = {}) {
  ...
  PROP.relic.fleece = ART.bakeFleeceIcon(); PROP.relic.spurs = ART.bakeSpursIcon(); PROP.relic.shoes = ART.bakeShoesIcon(); PROP.relic.lamp = PROP.relic.lamp || PROP.lampIcon; // the miner's lamp relic had no icon: the HUD threw every frame once you held it PROP.relic.windcloak = (() => { const [c, g] = canvas(10, 12); g.fillStyle = '#bfe6f5'; g.beginPath(); g.moveTo(5, 0); g.lineTo(9, 3); g.lineTo(9, 11); g.lineTo(5, 9); g.lineTo(1, 11); g.lineTo(1, 3); g.closePath(); g.fill(); g.fillStyle = '#7aa8c8'; g.fillRect(4, 1, 2, 8); g.fillStyle = '#ffd36b'; g.fillRect(4, 0, 2, 1); return c; })();
...
const MEDALS = { storm: [600, 820, 1150], moor: [480, 660, 960], scree: [450, 630, 920], mineworks: [540, 720, 1000], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
function openGateCol(col) {
  for (let ty = 0; ty < LH; ty++) { const i = ty * LW + col; if (L.grid[i] === T.PORT) { L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); } }
  resolveTiles();
}
...
const CRAG_PATH = [[36, 128], [62, 120], [92, 104], [120, 92], [150, 84], [184, 66], [214, 58], [250, 50], [280, 36], [270, 72], [252, 100], [232, 118], [210, 134], [184, 146], [160, 150], [134, 148], [108, 144], [82, 150]];
...
const NODE_AT = [0, 3, 6, 8, 10, 12, 20, 23, 25, 28, 31, 34]; // PATH index of each node: wood 0-5, then scree, the village, the high store
...
const BEASTS = [
  ...
  { t: 'cutter', name: 'ROPE CUTTER', sub: 'he wants the rope', desc: 'He is not interested in you at all. He is interested in the span you are standing on, and three swings of that axe is all it takes. Kill him, throw the shield, or be quick. Once the rope is gone he remembers you.' },
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster', 'golem', 'windcaller', 'lance'];
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.hp <= 0) {
    ...
    if (e.t === 'chief') chiefDies(); else if (e.t === 'queen' || e.t === 'frog' || e.t === 'king' || e.t === 'ram' || e.t === 'owl' || e.t === 'forgemaster' || e.t === 'golem' || e.t === 'windcaller' || e.t === 'lance') queenDies(e);
...
// which is what makes holding the jet worth the overheat it is walking her towards.
const heatMul = () => 1 + (P.heat || 0) / 100 * 0.7;
...
function updatePlayer(dt) {
  ...
  if (isPyro() && P.jbuf > 0 && !P.ground && P.coyote <= 0 && P.kickT <= 0 && !P.kicked && !P.cling && !stunned && !dodging && !(P.overheat > 0) && P.heat < 100) {
    P.jbuf = 0; P.kickT = 0.28; let hit = false;
    for (const e of enemies) if (e.alive && !e.harmless && Math.abs(e.x - P.x) < 15 && e.y - e.h / 2 > P.y - 6 && e.y - e.h / 2 < P.y + 30) { hit = true; if (e.t === 'shield' || e.t === 'pike' || (e.t === 'ram' && !ramOpen(e)) || e.t === 'mother' || e.t === 'gill') { SFX.clank(); } else { hurtEnemy(e, heatDmg(12), P.x, true); e.burn = Math.max(e.burn || 0, 1.2); } }
    for (const pr of props) if (pr.t === 'puffball' && !pr.popped && Math.abs(pr.x - P.x) < 14 && pr.y > P.y && pr.y < P.y + 30) { hit = true; }
    // a kick that lands is paid back; a kick into thin air is the only one you get before you touch down
    if (hit) { P.vy = POGO; pogoCount++; P.kicked = false; SFX.pogo(); number(P.x, P.y - 24, 'KICK', '#ff9a5c'); } else { P.vy = -250; P.kicked = true; P.heat = Math.min(100, P.heat + 20); SFX.puff(); }
  ...
  if (P.abuf > 0 && !stunned && !P.plunge && !dodging) {
    if (!P.ground && (keys.down || P.abufDown)) { P.abuf = 0; P.abufDown = false; if (spend(plungeCost())) { P.plunge = true; P.vy = Math.max(P.vy, 60); P.atk = -1; P.hitSet.clear(); SFX.slash();
      if (isPyro()) { // the fireball goes down ahead of her and lands first
        ...
        P.heat = Math.min(100, (P.heat || 0) + 10); SFX.puff();
  ...
  for (const e of enemies) {
    ...
    if (e.t === 'brute' || e.t === 'chief' || e.t === 'mother' || e.t === 'gill' || e.t === 'heart' || e.t === 'folk' || e.t === 'king' || e.t === 'bearer' || e.t === 'pike' || e.t === 'master' || e.t === 'thief' || e.t === 'ram' || e.t === 'goat' || e.t === 'harpy' || e.t === 'troll' || e.t === 'greathound' || e.t === 'spider' || e.t === 'owl' || e.t === 'miner' || e.t === 'bat' || e.t === 'forgemaster' || e.t === 'lance' || (e.big && e.t !== 'sailer')) continue; // their damage comes from their attacks, not from touching them
...
const MINI_DONE = { greathound: 'THE KENNELS OPEN', troll: 'THE GULLY IS CLEAR', spider: 'THE WEB COMES DOWN', sailer: 'THE ROAD IS OPEN' };
...
function miniEnd(e) {
  if (!L.mini) return;
  for (let ty = 0; ty < LH; ty++) { const i = ty * LW + L.mini.gate; if (L.grid[i] === T.PORT) { L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); } }
...
function desiredView() { if (state === 'editor') return 'zoom';
  const inLevel = state === 'play' || state === 'talk' || state === 'win' || state === 'gameover' || (state === 'menu' && menuFrom === 'play'); if (!inLevel) return 'normal'; return (SET.zoom === 'wide' || (bossActive && boss && (boss.t === 'mother' || boss.t === 'owl' || boss.t === 'forgemaster' || boss.t === 'golem' || boss.t === 'windcaller' || boss.t === 'king' || boss.t === 'lance'))) ? 'zoom' : 'normal'; }
