...
function spawnEntities() {
  enemies = []; seeds = []; movers = []; corpses = []; waves = []; bombs = []; fires = []; props = []; lights = []; bridges = []; foxes = []; clouds2 = []; roots = []; shelfT = {}; mother = null; boss = null; throneBlock = null; talkTo = null; talk = null; slide = null; burnT = {}; bossActive = false; bossWon = 0; camLock = null; impacts = []; rings = []; escape = null; thrown = null; deco = []; pwaves = []; rain = []; bolts = []; vines = []; rocks = []; miniActive = false; miniDone = false;
...
function updatePlayer(dt) {
  ...
  if (isPyro() && P.jbuf > 0 && !P.ground && P.coyote <= 0 && P.kickT <= 0 && !P.cling && !stunned && !dodging && !(P.overheat > 0) && P.heat < 100) {
    ...
    for (const e of enemies) if (e.alive && !e.harmless && Math.abs(e.x - P.x) < 15 && e.y - e.h / 2 > P.y - 6 && e.y - e.h / 2 < P.y + 30) { hit = true; if (e.t === 'shield' || e.t === 'pike' || (e.t === 'ram' && !ramOpen(e)) || e.t === 'mother' || e.t === 'gill') { SFX.clank(); } else { hurtEnemy(e, 12, P.x, true); e.burn = Math.max(e.burn || 0, 1.2); } }
...
function updateKing(e, dt) {
  ...
  if (e.phase === 1) { // carried: the litter paces the hall under four straining bearers; goblets and guards
    e.face = Math.sign(d) || e.face; e.x += e.dir * 36 * dt; if (e.x < A.x0 + 60) e.dir = 1; if (e.x > A.x1 - 60) e.dir = -1; e.y = floor - 9;
  ...
  if (e.phase === 3 && e.mode === 'walk' && Math.abs(e.vx) > 20 && e.grounded) { e.stepT -= dt; if (e.stepT <= 0) { e.stepT = 0.55; shakeCam(3); SFX.thud(); dust(e.x - e.face * 12, e.y, 6); const rx = Math.max(A.x0 + 16, Math.min(A.x1 - 16, P.x + (Math.random() - 0.5) * 60 + P.vx * 0.3)); rocks.push({ x: rx, y: floor - 170, vx: 0, vy: 0, t: 0, dead: false, thrown: true, lamp: true }); burst(rx, floor - 172, 4, ['#5a5a66', '#8a8478'], 30, 0.4); } }
...
function updateEmbers(dt) {
  for (const b of embers) { b.life -= dt; b.vy += 110 * dt; b.x += b.vx * dt; b.y += b.vy * dt;
    ...
    for (const e of enemies) { if (!e.alive || e.harmless || b.hit.has(e)) continue; if (Math.abs(e.x - b.x) < e.w / 2 + 5 && b.y > e.y - e.h - 5 && b.y < e.y + 5) { b.hit.add(e); const big = !!e.maxHp; if (e.t === 'ram' && !ramOpen(e)) { SFX.clank(); number(e.x, e.y - e.h - 8, 'HIS HIDE TURNS IT', '#9aa39a'); } else if (e.t === 'mother' || e.t === 'gill' || e.t === 'heart' || e.t === 'drone') { SFX.clank(); } else { hurtEnemy(e, big ? 4 : 12, b.x, false); if (!big) e.burn = Math.max(e.burn || 0, 1.2); flinch(e); } b.life = 0; burst(b.x, b.y, 8, ['#ff9a5c', '#ffd36b', '#ff6b2c'], 60, 0.4, 100, 2); break; } }
...
function updateProps(dt) {
 ...
 if (Math.random() < dt * 12) parts.push({ x: c.x + (Math.random() - 0.5) * c.r * 1.6, y: c.y + (Math.random() - 0.5) * c.r, vx: (Math.random() - 0.5) * 10, vy: -8, life: 0.8, max: 0.8, col: c.sleep ? '#c9a0ff' : '#d8d0c8', size: 1, grav: 0 }); }
  ...
  for (const f of fires) { if (f.delay > 0) { f.delay -= dt; continue; } f.life -= dt;
    { const ftx = Math.floor(f.x / TS), fty = Math.floor((f.y - 1) / TS); for (const dx of [-1, 0, 1]) for (let dy = 0; dy <= 2; dy++) { const tx = ftx + dx, ty = fty - dy; if (tileAt(tx, ty) !== T.PALISADE) continue; const i = ty * LW + tx; burnT[i] = (burnT[i] || 0) + dt; if (Math.random() < dt * 6) parts.push({ x: tx * TS + Math.random() * TS, y: ty * TS + Math.random() * TS, vx: 0, vy: -30, life: 0.4, max: 0.4, col: Math.random() < 0.5 ? '#ff9a5c' : '#5a5a66', size: 1, grav: 0 }); if (burnT[i] > 0.9) { L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); burst(tx * TS + 8, ty * TS + 8, 8, ['#8a5a32', '#ff9a5c', '#3a2416'], 60, 0.5); SFX.crack(); if (!fires.some(q => Math.abs(q.x - (tx * TS + 8)) < 6 && Math.abs(q.y - (ty + 1) * TS) < 6)) fires.push({ x: tx * TS + 8, y: (ty + 1) * TS, life: 2.5, delay: 0.1 }); } } } /* fire climbs and eats a stake wall */ if (!P.dead && Math.abs(P.x - f.x) < 9 && P.y > f.y - 14 && P.y <= f.y + 2) { if (isPyro() && PROG.items.kindle) { P.kindleT = (P.kindleT || 0) + dt; if (P.kindleT >= 0.5) { P.kindleT = 0; if (P.hp < P.maxHp) { P.hp = Math.min(P.maxHp, P.hp + 1); number(P.x, P.y - 24, '+1', '#8fd160'); } if (Math.random() < 0.7) parts.push({ x: P.x + (Math.random() - 0.5) * 8, y: P.y - 10, vx: 0, vy: -30, life: 0.5, max: 0.5, col: '#8fd160', size: 1, grav: 0 }); } } else damagePlayer(f.x, DMG.fire, { up: true, unblockable: true }); } for (const e of enemies) if (e.alive && e.t !== 'chief' && e.t !== 'wasp' && e.t !== 'king' && e.t !== 'master' && Math.abs(e.x - f.x) < 9 && Math.abs(e.y - f.y) < 6 && !(e.fireT > 0)) { e.fireT = 0.6; hurtEnemy(e, 10, f.x, false); } }
...
function update(dt) {
  ...
  updateMovers(wdt); updatePlayer(wdt); updateEnemies(wdt); updateWisp(wdt); updateSlide(wdt); updateProps(wdt); updateCorpses(wdt); updateParticles(wdt); updateWeather(dt); updateCamera(dt);
  updatePolish(dt);
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const e of enemies) {
    ...
    const bigF = e.t === 'frog' ? 1.35 : e.t === 'ram' ? 1.5 : 1; const sq = e.sq > 0 ? e.sq / 0.16 : 0;
