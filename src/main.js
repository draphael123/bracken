...
const HEROES = [
  ...
  { id: 'paladin', name: 'THE PALADIN', price: 30, silver: true, desc: 'maul and holy light. slower and heavier, 120 health. every blow and every hit turned aside fills the LIGHT. tap C: MEND. hold C: AEGIS, a ward that stops everything while you stand in it. a full bar and C again: JUDGEMENT, light out of the sky on everything near. the plunge is HAMMERFALL. the dead take double' },
];
const TRAINING = [
...
function applyUpgrades() { P.maxHp = (isPyro() ? 70 : isPaladin() ? 120 : 100) + (PROG.items.heart ? 25 : 0) + 10 * rankOf('vigour'); P.maxSt = 100 + (PROG.items.wind ? 30 : 0) + 10 * rankOf('breath'); }
...
const skillNow = () => { const k = PROG.skill; if (k === 'none' || k === 'kindle') return null; if (k && PROG.items[k] && (abilityHero(k) === hero() || (isPaladin() && abilityHero(k) === 'knight' && k !== 'shieldThrow'))) return k; if (isPaladin()) { if (PROG.items.groundSlam) return 'groundSlam'; if (PROG.items.risingCut) return 'risingCut'; return null; } if (isPyro()) { if (PROG.items.fireWall) return 'fireWall'; if (PROG.items.cinderStep) return 'cinderStep'; return null; } if (PROG.items.shieldThrow) return 'shieldThrow'; if (PROG.items.groundSlam) return 'groundSlam'; return null; };
...
function respawn() {
  ...
  mendAll(); resetCastle(); spawnEntities(); seeds = []; embers = []; pyres = []; P.full = false; P.fullT = 0; nums = []; ghosts = []; wisp = null; rain = []; P.heat = 0; P.overheat = 0; P.light = 0; P.cHeld = 0; music.play(L.music || 'theme'); setReverb(L.dark ? 0.34 : (L.interiors && L.interiors.length) ? 0.16 : (L.palette && L.palette.hall) ? 0.12 : 0.04);
  ...
  { const k = items[storeI]; const mx = pvX + pvW / 2;
    if (k) {
      ...
      if (tab.key === 'skin' || tab.key === 'sword' || tab.key === 'hero') {
        const set = tab.key === 'hero' ? (k.id === 'paladin' ? preview('hero:paladin', () => bakePaladin({})) : k.id === 'pyro' ? preview('hero:pyro', () => bakePyro(PYRO_SETS[PROG.skin] || {})) : preview('hero:knight', () => bakeKnight(Object.assign({}, skinById(PROG.skin).pal, swordById(PROG.sword).pal))))
...
function damagePlayer(fromX, dmg, { up = false, unblockable = false } = {}) {
  ...
  if (P.aegis) { gainLight(12); P.st = Math.max(0, P.st - 8); P.stDelay = ST.delay; hitstop(0.05); shakeCam(1.5); SFX.block(); ringAt(P.x, P.y - 10, 16, '#ffd36b', 0.25); const sd = Math.sign(fromX - P.x) || P.face; sparks(P.x + sd * 11, P.y - 10, sd, 6); return 'blocked'; } // the ward stops everything, even what a shield cannot
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.t === 'gqueen') { if (e.mode === 'ceil' || e.mode === 'roofWait' || e.mode === 'sleep') { SFX.clank(); return; } if (gqOpen(e)) dmg = Math.round(dmg * 1.5); }
...
function updatePlayer(dt) {
  ...
  const cap = ((P.block || P.jet) ? 32 : wading ? 46 : spored ? 40 : RUN * (PROG.charm === 'swift' ? 1.12 : 1) * (isPyro() ? 1.15 : isPaladin() ? 0.9 : 1)) + (P.gustT > 0 && !P.ground ? 150 : 0); // a gust can carry you faster than your legs
  if (move && !groundAtk) {
  ...
  if (P.ground && !wasGround) {
    ...
    if (P.plunge) {
      ...
      if (broke) { P.vy = POGO; P.ground = false; P.plunge = false; P.canCut = false; SFX.pPogo(); P.hitSet.clear(); squash(0.8, 1.25, 0.1); }
        ...
        if (isPaladin()) { for (const d of [-1, 1]) pwaves.push({ x: P.x + d * 8, y: P.y, dir: d, life: 1.0, sp: 200, hit: new Set() }); shakeCam(6); zoomKick(1.06, 0.2); ringAt(P.x, P.y - 2, 30, '#ffd36b', 0.3); SFX.heavy(); } } // HAMMERFALL: the maul comes down and the ground carries it both ways
...
function setWall(col, solid) {
  ...
  for (let ty = top; ty <= bot; ty++) { const i = ty * LW + col; L.grid[i] = solid ? T.SOLID : T.AIR; tileSpr[i] = solid ? (L.arena.boss === 'chief' ? TILE.palisade[(ty + col) % 3] : (L.arena.boss === 'ram' || L.arena.boss === 'lance' || L.arena.boss === 'suncatcher' || L.arena.boss === 'roc' || L.arena.boss === 'gqueen') ? TILE.drystone[(ty + col) % 3] : TILE.vine[(ty + col) % 4]) : null; }
...
const gqOpen = e => ['slamRec', 'rec', 'dazed', 'struck', 'topple'].includes(e.mode);
const GQ_FRAME = { sleep: 0, wake: 0, court: 0, point: 1, throw: 2, topple: 10, rise: 3, stand: e => Math.abs(e.vx) > 6 ? 4 + Math.floor(e.anim * 5) % 2 : 3, slamTell: 6, slam: 7, slamRec: 7, sweepTell: 3, sweep: 8, rec: 8,
  chargeTell: 6, charge: 9, dazed: 10, ceilTell: 6, ceil: 11, roofWait: 3, roof: e => Math.abs(e.vx) > 6 ? 4 + Math.floor(e.anim * 5) % 2 : 3, slateTell: 12, slate: 12, leapTell: 6, leap: 11, land: 7, struck: 13 };
let HALLWIN = null;
...
function updateGQueen(e, dt) {
  ...
  if (e.phase === 3 && e.mode !== 'ceil' && e.mode !== 'ceilTell' && e.mode !== 'roofWait') {
    if (!(e.gather > 0)) { e.boltT -= dt; if (e.boltT <= 0) { e.boltT = 5; e.gather = 0.9; e.markX = P.x; const rods = props.filter(p => p.t === 'rod'); const near = rods.sort((a, b) => Math.abs(a.x - e.x) - Math.abs(b.x - e.x))[0]; e.rodTarget = near && Math.abs(near.x - e.x) < 30 && e.y <= near.y + 6 ? near : null; SFX.thunder(); flash = Math.max(flash, 0.1); } }
      ...
      else { bolts.push({ x: e.markX, y: roof, life: 0.3, storm: true }); SFX.thunder(); shakeCam(6); flash = Math.max(flash, 0.3); if (!P.dead && Math.abs(P.x - e.markX) < 20 && P.y > roof - 48) damagePlayer(e.markX, DMG.gqBolt, { unblockable: true, up: true }); }
      e.rodTarget = null; } }
  }
  switch (e.mode) {
    ...
    case 'court': { e.x = e.throneX; e.y = e.throneY; e.vx = 0; e.vy = 0; e.face = -1; e.pointT -= dt; e.bombT -= dt; e.guardT -= dt;
      ...
      if (guards < 2 && e.guardT <= 0) { e.guardT = 8; const n0 = enemies.length; spawnEnt({ t: 'shield', x: Math.floor(e.throneX / TS) - 5 - guards * 3, y: Math.floor(hall / TS) - 1, face: -1 }); for (let i = n0; i < enemies.length; i++) enemies[i].royal = true; SFX.clank(); }
      const archers = enemies.some(en => en.alive && en.gallery);
      if (e.modeT <= 0) {
        if (e.pointT <= 0 && archers && !L.galleryDown) { e.pointT = 3.8; e.mode = 'point'; e.modeT = 0.9; e.markX = Math.max(A.x0 + 20, Math.min(A.x1 - 60, P.x)); SFX.bellow(); }
    ...
    case 'stand': { e.face = Math.sign(d) || e.face; want = ad > 44 ? e.face * 40 : 0; e.slamT -= dt; e.sweepT -= dt; e.chargeT -= dt;
      if (e.modeT <= 0 && !P.dead) {
        if (e.chargeT <= 0 && ad > 100) { e.chargeT = 7; e.mode = 'chargeTell'; e.modeT = 0.8; SFX.bellow(); }
        else if (e.slamT <= 0 && ad < 96) { e.slamT = 3.4; e.mode = 'slamTell'; e.modeT = 0.7; SFX.charge(); }
        else if (e.sweepT <= 0 && ad < 54) { e.sweepT = 2.6; e.mode = 'sweepTell'; e.modeT = 0.45; SFX.snort(); }
      } break; }
    ...
    case 'ceil': { const hx = (A.hole.x0 + A.hole.x1 + 1) * TS / 2; e.x += (hx - e.x) * Math.min(1, dt * 5); e.y -= 360 * dt;
      if (e.y <= roof - 40) { e.x = hx + 48; e.y = roof; e.vy = 0; e.vx = 0; e.mode = 'roofWait'; e.modeT = 0.5; shakeCam(6); SFX.heavy(); } break; }
    ...
    case 'roof': { e.face = Math.sign(d) || e.face; want = ad > 60 ? e.face * 34 : 0; e.slateT -= dt; e.leapT -= dt;
      if (e.modeT <= 0 && !P.dead) {
        if (e.slateT <= 0 && ad > 50) { e.slateT = 2.8; e.mode = 'slateTell'; e.modeT = 0.45; }
...
function raiseAlarm(id) {
  ...
  sec.on = true; SFX.thunder(); SFX.clank(); SFX.bellow(); shakeCam(5); flash = Math.max(flash, 0.18);
  for (const [col, y0, y1] of sec.gates) for (let ty = y0; ty <= y1; ty++) { const i = ty * LW + col; if (L.grid[i] === T.AIR) { L.grid[i] = T.PORT; tileSpr[i] = TILE.port[(ty + col) % 2]; } }
  resolveTiles();
...
function updateAlarms() {
  for (const sec of (L.alarms || [])) if (sec.on && !sec.done && !enemies.some(e => e.alive && e.garrison === sec.id)) {
    sec.done = true; SFX.medal(); SFX.clank();
    for (const [col, y0, y1] of sec.gates) for (let ty = y0; ty <= y1; ty++) { const i = ty * LW + col; if (L.grid[i] === T.PORT) { L.grid[i] = T.AIR; tileSpr[i] = null; } }
    resolveTiles();
  }
}
...
function updateCastleProps(dt, hb) {
  for (const pr of props) {
    if (pr.t === 'winch') {
      ...
      if (struck) { P.hitSet.add(pr); if (pr.open <= 0) { pr.open = pr.hold; SFX.clank(); SFX.stone(); for (let ty = pr.gy0; ty <= pr.gy1; ty++) { const i = ty * LW + pr.gate; if (L.grid[i] === T.PORT) { L.grid[i] = T.AIR; tileSpr[i] = null; } } resolveTiles(); } else { pr.open = pr.hold; SFX.ui(); } }
      if (pr.open > 0) { pr.open -= dt; pr.spin += dt * 6; if (Math.floor(pr.open * 2) !== Math.floor((pr.open + dt) * 2)) SFX.text();
        if (pr.open <= 0) { pr.open = 0; SFX.heavy(); shakeCam(3); const gx = pr.gate * TS + 8;
          for (let ty = pr.gy0; ty <= pr.gy1; ty++) { const i = ty * LW + pr.gate; if (L.grid[i] === T.AIR) { L.grid[i] = T.PORT; tileSpr[i] = TILE.port[(ty + pr.gate) % 2]; } } resolveTiles();
...
function gainLight(n) { const was = (P.light || 0) >= 100; P.light = Math.min(100, (P.light || 0) + n); if (!was && P.light >= 100) { SFX.heatFull(); ringAt(P.x, P.y - 10, 22, '#fff6c8', 0.4); } }
function castMend() { if ((P.light || 0) < 34) { SFX.buzz(); P.stFlash = 0.3; return; } P.light -= 34; P.hp = Math.min(P.maxHp, P.hp + 20); P.castT = 0.3; SFX.spark(); SFX.sting(); number(P.x, P.y - 24, '+20', '#fff6c8');
...
function castJudgement() { P.light = 0; P.blastT = 0.5; P.inv = Math.max(P.inv, 0.6); P.vx = 0; SFX.thunder(); SFX.medal(); SFX.heavy(); shakeCam(9); zoomKick(1.12, 0.35); killFlash = 0.035; hitstop(0.08); // a white flash: the red one is for being hurt
  for (let k = -3; k <= 3; k++) bolts.push({ x: P.x + k * 48, y: P.y, life: 0.45, storm: true, holy: true });
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const e of enemies) {
    ...
    if (e.t === 'gqueen' && e.alive && e.gather > 0 && e.markX !== undefined && !e.rodTarget) { const k = 1 - e.gather / 0.9, fy = Math.round(L.arena.roof - cy); g.globalAlpha = 0.3 + 0.5 * k; g.strokeStyle = '#dfe8ff'; g.lineWidth = 1; g.beginPath(); g.ellipse(Math.round(e.markX - cx), fy - 1, 22 - k * 10, 5 - k * 2, 0, 0, Math.PI * 2); g.stroke(); g.globalAlpha = 1; }
