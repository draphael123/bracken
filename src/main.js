...
const swordDmg = () => Math.round((sword().dmg + (PROG.items.edge ? 3 : 0) + (PROG.items.edge2 ? 3 : 0) + rankOf('temper')) * (isPyro() ? 0.7 : 1));
...
const CD_MAX = { shieldThrow: 2.5, groundSlam: 3, fireWall: 4, cinderStep: 3, risingCut: 2, vent: 3, wisp: 8 };
...
function winLevel() {
  ...
  if (id === 'hanging' && isPyro() && !PROG.items.kindle) PROG.storeHint = 'kindle';
  if (id === 'mineworks' && isPyro() && !PROG.items.wisp) PROG.storeHint = 'wisp';
...
const BEASTS = [
  ...
  { t: 'forgemaster', name: 'THE FORGEMASTER', guardNote: true, sub: 'engineer of the deep', desc: 'Strapped into a steam rig at the forge. Three things never stop: the hammer stamps the floor on a rhythm, carts roll through on the rail, slag drips from marked spots. He lunges on a piston, sprays scalding steam, and kicks carts at you. Ride a cart into his boiler, or burn it with embers, and it bursts: he is scalded and open. Past half his blood the lamps go out and the bats come.' },
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.t === 'forgemaster' && e.guard !== false && e.mode !== 'scald') { SFX.clank(); sparks(e.x + (Math.sign(fromX - e.x) || 1) * 10, e.y - 12, Math.sign(e.x - fromX) || 1, 4); return; } // the apron: a cart breaks it, the scald opens it
  if (e.t === 'forgemaster' && (e.mode === 'scald' || e.mode === 'reel')) dmg *= 2; // the scald and the reel are the windows
...
function bossStart() {
  ...
  SFX.roar(); shakeCam(6); music.stop(); bossMusicT = 1.1; number(boss.x, boss.y - 30, boss.t === 'frog' ? 'THE BULLFROG KING' : boss.t === 'chief' ? 'THE GOBLIN CHIEFTAIN' : boss.t === 'mother' ? 'THE MOTHER CAP' : boss.t === 'king' ? 'KING GORM UNDERLEAF' : boss.t === 'ram' ? 'THE RAM LORD' : boss.t === 'owl' ? 'THE OWL REEVE' : boss.t === 'forgemaster' ? (boss.guard === false ? 'THE FORGEMASTER  OPEN' : 'THE FORGEMASTER') : 'THE HORNET QUEEN', '#ffd36b'); zoomKick(1.1, 0.4);
...
function playerLight() { let r = P.relic === 'lamp' ? 90 : 26; if (wisp) r = Math.max(r, 64); if (P.torch > 0) r = Math.max(r, 84); if (isPyro() && P.block) r = Math.max(r, 70); if (isPyro() && embers.length) r = Math.max(r, 50); return r; }
...
function updateCarts(dt) {
  for (const m of movers) {
    ...
    if (boss && boss.t === 'forgemaster' && boss.alive && Math.abs(m.vx) > 60 && (m.ridden || m.kicked) && !m.hit.has(boss) && boss.x + 12 > m.x - 4 && boss.x - 12 < m.x + m.w + 4 && Math.abs(boss.y - (m.y + m.h)) < 20) { m.hit.add(boss); boss.guard = false; boss.guardT = 4.5; boss.mode = 'reel'; boss.modeT = 1.4; boss.stagger = 1.4; boss.vx = Math.sign(m.vx) * 90; hurtEnemy(boss, 24, m.x, false); number(boss.x, boss.y - boss.h - 12, 'GUARD BROKEN', '#8fd160'); SFX.heavy(); SFX.clank(); shakeCam(7); hitstop(0.1); zoomKick(1.1, 0.3); burst(m.x + m.w / 2, m.y + 4, 14, ['#5a6270', '#8a919c', '#ffd36b'], 90, 0.6); m.gone = true; m.respawnT = 4; if (P.onMover === m) { P.onMover = null; P.vx = -Math.sign(m.vx) * 160; P.vy = -140; P.ground = false; } continue; }
...
function updateForgemaster(e, dt) {
  ...
  if (e.guard === false) { e.guardT = (e.guardT || 0) - dt; if (e.guardT <= 0) { e.guard = true; number(e.x, e.y - e.h - 12, 'THE APRON', '#9aa39a'); SFX.clank(); } }
  if (e.mode !== 'wake') {
...
function updateWisp(dt) {
  if (!wisp) return; const w = wisp; w.t += dt; w.life -= dt; w.cd = Math.max(0, w.cd - dt);
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const e of enemies) {
    ...
    else if (e.t === 'owl') frame = e.mode === 'crash' || e.mode === 'grounded' ? 4 : e.mode === 'screech' || e.mode === 'screechTell' ? 3 : e.mode === 'sit' || e.mode === 'sleep' || e.mode === 'wake' || e.mode === 'fanTell' || e.mode === 'fan' ? 0 : e.mode === 'swoop' ? 2 : e.mode === 'takeoff' || e.mode === 'land' ? 1 : 1 + Math.floor(e.anim * 8) % 2;
  ...
  if (L.dark) { // the mine: black, with holes for every lamp, fire and the light you carry
    ...
    for (const lt of lights) if (lt.r > 0 && lt.x > cx - 120 && lt.x < cx + VW + 120 && !(lt.ref && lt.ref.dark > 0) && !(lt.lantern && !lt.lantern.lit) && !(lt.bracket && lt.bracket.taken)) hole(lt.x - cx, lt.y - cy, lt.r * 1.3);
...
function render() {
  ...
  if (state === 'play' || state === 'talk' || state === 'win' || state === 'gameover' || (state === 'menu' && menuFrom === 'play')) {
    ...
    { const sk = skillNow(); if (sk) { const cd = skillCd(sk), max = CD_MAX[sk] || 3; g.globalAlpha = (sk === 'shieldThrow' && thrown) || cd > 0 ? 0.45 : 1; g.drawImage(sk === 'groundSlam' ? SLAM_ICON : sk === 'shieldThrow' ? SHIELD_ICON : sk === 'risingCut' ? RISE_ICON : FLAME_ICON, 76, 13); g.globalAlpha = 1; if (cd > 0) { g.fillStyle = '#c9d1dc'; g.fillRect(76, 26, Math.round(10 * (1 - cd / max)), 1); } } }
