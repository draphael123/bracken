...
function spawnEnt(e) {
  ...
  const n0 = enemies.length;
  {
    ...
    switch (e.t) {
      ...
      case 'vent': props.push({ t: 'vent', x: px, y: py, period: e.period || 4, on: e.on || 1.6, phase: e.phase || 0, h: e.h || 112, wind: !!e.wind, w: e.w || 13 }); break;
...
function updateWindcaller(e, dt) {
  ...
  switch (e.mode) {
    ...
    case 'cast': { e.castT -= dt; e.howlT -= dt;
      if (e.castT <= 0 && !P.dead) { e.castT = p2 ? 0.75 : 1.15; e.castN = (e.castN || 0) + 1; const fan = p2 && e.castN % 3 === 0, n = fan ? 3 : 1; const a0 = Math.atan2((P.y - 8) - (e.y - 14), P.x - e.x);
        for (let k = 0; k < n; k++) { const a = a0 + (k - (n - 1) / 2) * 0.28; seeds.push({ x: e.x + Math.cos(a) * 12, y: e.y - 14 + Math.sin(a) * 12, vx: Math.cos(a) * 210, vy: Math.sin(a) * 210, dead: false, life: 2.6, bolt: true }); }
    ...
    case 'howl': { const dir = e.howlDir; if (!P.dead && P.x > A.x0 && P.x < A.x1) { P.vx += dir * (P.ground ? 420 : 520) * dt; P.gustT = 0.25; if (Math.random() < dt * 70) parts.push({ x: camX + Math.random() * VW, y: camY + Math.random() * VH, vx: dir * 320, vy: 0, life: 0.3, max: 0.3, col: '#e8f0f8', size: 1, grav: 0 }); } if ((e.hits || 0) >= (p2 ? 1 : 2)) goBlink(); else if (e.modeT <= 0) goBlink(); break; }
    case 'blink': if (e.modeT <= 0) { const far = roosts.filter(r => Math.abs(r[0] * TS + 16 - e.x) > 20 && Math.abs(r[0] * TS + 16 - P.x) > 50); const pool = far.length ? far : roosts; const pick = pool[Math.floor(Math.random() * pool.length)]; if (pick) { e.x = pick[0] * TS + 16; e.y = (pick[1] + 1) * TS; } e.mode = 'appear'; e.modeT = 0.45; burst(e.x, e.y - 12, 14, ['#c9a0ff', '#e8dcc0'], 60, 0.5, -40, 1); SFX.puff(); } break;
    case 'appear': if (e.modeT <= 0) { e.mode = 'cast'; e.modeT = castLen; e.castT = 0.6; } break;
  }
}
const callerOpen = e => e.mode === 'cast' || e.mode === 'howlTell' || e.mode === 'howl';
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'vent') { // an updraft of spores on a timer: ride it up
      ...
      if (pr.active) {
        ...
        if (!P.dead && Math.abs(P.x - pr.x) < (pr.w || 13) && P.y <= pr.y + 2 && P.y > pr.y - pr.h) { P.vy = Math.min(P.vy, -190); P.ground = false; P.canCut = false; P.plunge = false; if (!pr.said) { pr.said = true; number(pr.x, pr.y - 20, 'UPDRAFT', '#c8bcb0'); } }
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const e of enemies) {
    ...
    else if (e.t === 'windcaller') frame = e.mode === 'blink' || e.mode === 'appear' ? 2 : (e.castFlash > 0 || e.mode === 'howlTell' || e.mode === 'howl') ? 1 : 0;
