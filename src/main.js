...
import { bakeKeeper, bakeBard, bakeOldKnight, bakeMiner, bakeBat, bakeForgemaster, bakePyro, bakeCragRam, bakeTroll, bakeGreatHound, bakeSpider, bakeSquirrel, bakeOwl, bakeWoodsman, bakeFerryman, bakeSquire, bakeElder, bakeHarpy, bakeGoatRider, bakeRamLord, bakeShepherd, bakeSheep, bakeKnight, bakeSprig, bakeShield, bakeSpitter, bakeSpitterParts, bakeWasp, bakeSeed, bakeThornback, bakeQueen, bakeArcher, bakeBird, bakeFrog, bakeHopper, HOPPER_COLORS, bakeSapper, bakeBomb, bakeBrute, bakeHound, bakeFox, bakeChief, bakeSporeling, bakeLurker, bakeDrone, bakeShaman, bakeThief, bakePike, bakeFolk, bakeMaster, bakeKing } from './chars.js';
...
function progDefaults() { if (!PROG.heroes) PROG.heroes = { knight: true }; if (!PROG.hero) PROG.hero = 'knight'; if (!PROG.music) PROG.music = { select: true }; if (!PROG.music.select) PROG.music.select = true; PROG.coins = PROG.coins || 0; PROG.skins = PROG.skins || { bracken: true }; PROG.skin = PROG.skin || 'bracken'; PROG.swords = PROG.swords || { steel: true }; PROG.sword = PROG.sword || 'steel'; PROG.items = PROG.items || {}; PROG.charms = PROG.charms || {}; if (!PROG.skill && PROG.items.shieldThrow) PROG.skill = 'shieldThrow'; }
...
const DMG = { miner: 22, bat: 10, cartHit: 12, gas: 20, piston: 25, steam: 12, hammer: 30, greathound: 20, pounce: 25, snap: 15, spider: 15, owlSwoop: 25, screech: 12, feather: 10, troll: 25, sprig: 20, shield: 25, spit: 15, wasp: 15, thorn: 30, spike: 20, seed: 15, spined: 20, queen: 30, wave: 20, venom: 18, archer: 15, arrow: 18, frog: 25, tongue: 25, hopper: 15, crown: 15, sapper: 15, bomb: 25, brute: 20, bruteOver: 30, bruteSweep: 20, hound: 18, chief: 25, chiefOver: 35, chiefSweep: 20, chiefGrab: 20, fire: 15, sporeling: 15, lurker: 22, drone: 15, shaman: 15, root: 15, roller: 12, sporeRain: 10, pike: 20, master: 20, whip: 15, goblet: 15, sceptre: 25, shout: 10, kingSlam: 35, ram: 20, cage: 15, bolt: 20, crownToss: 18, lash: 15, vine: 15, harpy: 18, goat: 20, ramLord: 30, ramStamp: 20, rock: 20 };
const EHP = { miner: 30, bat: 8, forgemaster: 260, greathound: 220, spider: 15, owl: 320, troll: 60, sprig: 10, shield: 20, spit: 10, wasp: 10, thorn: 30, queen: 220, archer: 10, frog: 340, hopper: 10, sapper: 10, brute: 40, hound: 15, chief: 400, sporeling: 10, lurker: 20, drone: 10, shaman: 20, gill: 20, heart: 6, mother: 9999, thief: 10, pike: 20, folk: 1, master: 160, bearer: 20, king: 420, harpy: 18, goat: 20, ram: 300 };
...
const HEROES = [
  { id: 'knight', name: 'THE KNIGHT', price: 0, desc: 'sword, shield and the plunge. 100 health' },
  { id: 'pyro', name: 'THE PYROMANCER', price: 15, silver: true, desc: 'staff and ember. hold C for the flare instead of a shield. 70 health. flame kick in the air' },
];
const STORE_TABS = [{ name: 'HEROES', items: HEROES, key: 'hero', owned: 'heroes' }, { name: 'SKINS', items: SKINS, key: 'skin', owned: 'skins' }, { name: 'SWORDS', items: SWORDS, key: 'sword', owned: 'swords' }, { name: 'GEAR', items: UPGRADES, key: null, owned: 'items' }, { name: 'SKILLS', items: ABILITIES, key: 'skill', owned: 'items' }, { name: 'CHARMS', items: CHARMS, key: 'charm', owned: 'charms' }, { name: 'MUSIC', items: MENU_MUSIC, key: 'menu', owned: 'music' }];
...
const swordDmg = () => Math.round((sword().dmg + (PROG.items.edge ? 3 : 0) + (PROG.items.edge2 ? 3 : 0)) * (isPyro() ? 0.7 : 1));
...
function applyUpgrades() { P.maxHp = (isPyro() ? 70 : 100) + (PROG.items.heart ? 25 : 0); P.maxSt = 100 + (PROG.items.wind ? 30 : 0); }
...
const skillNow = () => { const k = PROG.skill; if (k === 'none') return null; if (k && PROG.items[k] && abilityHero(k) === hero()) return k; if (isPyro()) { if (PROG.items.fireWall) return 'fireWall'; if (PROG.items.cinderStep) return 'cinderStep'; return null; } if (PROG.items.shieldThrow) return 'shieldThrow'; if (PROG.items.groundSlam) return 'groundSlam'; return null; };
...
const MASTER = bakeMaster(), KING = bakeKing(); SPR.harpy = bakeHarpy(); SPR.goat = bakeCragRam(); SPR.troll = bakeTroll(); SPR.greathound = bakeGreatHound(); SPR.spider = bakeSpider(); SPR.squirrel = bakeSquirrel(); SPR.owl = bakeOwl(); SPR.lamplighter = SPR.woodsman; SPR.keeper = bakeKeeper(); SPR.bard = bakeBard(); SPR.oldknight = bakeOldKnight(); SPR.miner = bakeMiner(); SPR.bat = bakeBat(); SPR.forgemaster = bakeForgemaster(); SPR.foreman = SPR.woodsman; SPR.ram = bakeRamLord(); SPR.shepherd = bakeShepherd(); SPR.sheep = bakeSheep(); SPR.keeper = bakeKeeper(); SPR.woodsman = bakeWoodsman(); SPR.ferryman = bakeFerryman(); SPR.squire = bakeSquire(); SPR.elder = bakeElder(); SPR.master = MASTER.mounted; SPR.masterFoot = MASTER.foot; SPR.king = KING.seated; SPR.kingUp = KING.standing; SPR.bearer = SPR.sprig;
...
function bakeAll(pal = {}) {
  ...
  TILE = {
    ...
    log: [0, 1, 2].map(i => ART.bakeLog(50 + i)), logL: ART.bakeLogEnd(60, false), logR: ART.bakeLogEnd(61, true), comb: [0, 1, 2].map(i => ART.bakeCombPlat(560 + i)), combL: ART.bakeCombPlat(563, 'L'), combR: ART.bakeCombPlat(564, 'R'), ropeNet: [0, 1].map(i => ART.bakeRopeNet(i)), rail: [0, 1].map(i => ART.bakeRail(i)), soft: [0, 1, 2].map(i => ART.bakeSoftRock(930 + i)), ladder: { S: [0, 1].map(i => ART.bakeRopeLadder(i, null)), L: [0, 1].map(i => ART.bakeRopeLadder(i, 'L')), R: [0, 1].map(i => ART.bakeRopeLadder(i, 'R')) }, ledge: [0, 1, 2].map(i => ART.bakeLedge(570 + i, null)), ledgeL: ART.bakeLedge(573, 'L'), ledgeR: ART.bakeLedge(574, 'R'), climb: [ART.bakeClimbFace(0), ART.bakeClimbFace(1)],
  ...
  PROP.relic.fleece = ART.bakeFleeceIcon();
...
function resolveTiles() {
  ...
  for (let y = 0; y < LH; y++) for (let x = 0; x < LW; x++) {
    ...
    if (t === T.SOLID) {
      ...
      for (const z of (L.stone || [])) if (x >= z[0] && x <= z[1] && y >= z[2] && y <= z[3]) s = up === T.SOLID ? TILE.drystone[(x + y) % 3] : TILE.drystoneTop[(x + y) % 2];
    ...
    else if (t === T.NET) { const n = (dx, dy) => tileAt(x + dx, y + dy) === T.NET; let vr = 1, hr = 1; for (let k = 1; n(0, -k); k++) vr++; for (let k = 1; n(0, k); k++) vr++; for (let k = 1; n(-k, 0); k++) hr++; for (let k = 1; n(k, 0); k++) hr++; s = hr > vr ? TILE.ropeNet[(x + y) % 2] : TILE.ladder[n(1, 0) && !n(-1, 0) ? 'L' : n(-1, 0) && !n(1, 0) ? 'R' : 'S'][y % 2]; }
...
const MEDALS = { mineworks: [540, 720, 1000], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
const RELICS = { lamp: { name: "MINER'S LAMP", desc: 'light around you in the dark', col: '#ffd36b' }, fleece: { name: 'GOLDEN FLEECE', desc: 'stamina returns twice as fast', col: '#ffe6a0' }, crown: { name: 'HORNET CROWN', desc: 'stomps strike like plunges', col: '#e0b040' }, charm: { name: "HUNTER'S CHARM", desc: 'gold comes to you', col: '#ffd34a' }, gauntlet: { name: 'IRON GAUNTLET', desc: 'swings cost no stamina', col: '#c9d1dc' }, lantern: { name: 'GLOW LANTERN', desc: 'spores cannot put you to sleep', col: '#4aa0b0' }, cloak: { name: 'THIEF CLOAK', desc: 'thieves cannot take your gold', col: '#6a3aa0' } };
...
function applyWorld() { let ch = false; for (const e of L.ents) if (e.t === 'felltree' && marks.has('tree:' + e.x)) for (let x = e.x + 1; x <= e.x + (e.len || 12); x++) { const i = e.y * LW + x; if (L.grid[i] !== T.PLANK) { L.grid[i] = T.PLANK; tileSpr[i] = null; ch = true; } } return ch; }
...
function spawnEntities() {
  enemies = []; seeds = []; movers = []; corpses = []; waves = []; bombs = []; fires = []; props = []; lights = []; bridges = []; foxes = []; clouds2 = []; roots = []; shelfT = {}; mother = null; boss = null; bossActive = false; bossWon = 0; camLock = null; impacts = []; rings = []; escape = null; thrown = null; deco = []; pwaves = []; rain = []; bolts = []; vines = []; rocks = []; miniActive = false; miniDone = false;
...
function spawnEnt(e) {
  ...
  const n0 = enemies.length;
  {
    ...
    switch (e.t) {
      ...
      case 'bell': props.push({ t: 'bell', x: px, y: py, hp: 3, rung: false, broken: false, gate: e.gate, ringT: 0 }); break;
      ...
      case 'forgemaster': boss = { ...base, t: 'forgemaster', w: 26, h: 22, hp: EHP.forgemaster, maxHp: EHP.forgemaster, mode: 'sleep', modeT: 0, face: -1, phase: 1, hitT: 0, lungeT: 3, sprayT: 6, kickT: 9, hammerT: 2.2, cartT: 4, slagT: 5 }; enemies.push(boss); break;
      ...
      case 'minerlamp': props.push({ t: 'minerlamp', x: px, y: py - (e.hang ? 0 : 0), lit: true }); lights.push({ x: px, y: py - 8, r: 60, glow: true, warm: true }); break;
      ...
      case 'hammer': props.push({ t: 'hammer', x: px, y: py, drop: 0, tell: 0 }); break;
      ...
      case 'spider': enemies.push({ ...base, t: 'spider', w: 10, h: 8, hp: EHP.spider, restY: py, drop: e.drop || 100, mode: 'hang', modeT: 0, cd: 0, face: 1 }); break;
      ...
      case 'lantern': props.push({ t: 'lantern', x: px, y: py, lit: true, hits: 0 }); lights.push({ x: px, y: py - 30, r: 50, glow: true, lantern: props[props.length - 1] }); break;
      case 'troll': enemies.push({ ...base, t: 'troll', w: 16, h: 18, hp: EHP.troll, speed: 22, mode: 'walk', modeT: 0, throwT: 2, hitT: 0 }); break;
      ...
      case 'ramlord': boss = { ...base, t: 'ram', w: 26, h: 14, hp: EHP.ram, maxHp: EHP.ram, mode: 'sleep', modeT: 0, face: -1, phase: 1, hitT: 0, n: 0 }; enemies.push(boss); break;
      ...
      case 'stray': props.push({ t: 'stray', kind: e.kind || 'sheep', x: px, y: py, got: straysGot.has(px), anim: Math.random() * 6 }); break;
...
function respawn() {
  ...
  spawnEntities(); seeds = []; embers = []; nums = []; ghosts = []; rain = []; P.heat = 0; P.overheat = 0; P.cHeld = 0; music.play(L.music || 'theme');
...
const CRAG_NODES = [
  ...
  { id: 'mineworks', kind: 'level', level: 7, x: 232, y: 118, name: 'THE MINEWORKS' },
];
const CRAG_PATH = [[36, 128], [62, 120], [92, 104], [120, 92], [150, 84], [184, 66], [214, 58], [250, 50], [280, 36], [270, 72], [252, 100], [232, 118]];
...
const NODE_AT = [0, 3, 6, 8, 10, 12, 20, 23, 25, 28]; // PATH index of each node: wood 0-5, then scree, the village, the high store
...
const nodeLocked = nd => nd.kind === 'level' ? levelLocked(LEVELS[nd.level]) : nd.kind === 'store' ? !!(nd.needs && !(PROG[nd.needs] && PROG[nd.needs].cleared) && !q.get('unlock')) : false;
...
function updateStore(dt) {
  ...
  if (confirmPress && items.length) {
    ...
    if (owned && tab.key) { PROG[tab.key] = k.id; if (tab.key === 'menu') music.play(menuTrack()); if (tab.key === 'hero') { applySkin(); applyUpgrades(); P.hp = Math.min(P.hp, P.maxHp); } applySkin(); saveProgress(); SFX.uiSel(); storeMsg = k.name + ' equipped' + (tab.key === 'skill' ? ' on F' : tab.key === 'charm' ? ' (worn)' : ''); storeMsgT = 2; }
...
const BEASTS = [
  ...
  { t: 'forgemaster', name: 'THE FORGEMASTER', sub: 'engineer of the deep', desc: 'Strapped into a steam rig at the forge. Three things never stop: the hammer stamps the floor on a rhythm, carts roll through on the rail, slag drips from marked spots. He lunges on a piston, sprays scalding steam, and kicks carts at you. Ride a cart into his boiler, or burn it with embers, and it bursts: he is scalded and open. Past half his blood the lamps go out and the bats come.' },
  ...
  { t: 'king', name: 'KING GORM UNDERLEAF', sub: 'lord of the court', desc: 'Sits on a palanquin and does not get up. Cut the four bearers and the throne falls. Then: the sceptre sweeps, a bolt marks the floor and strikes, the crown comes back like a boomerang, cages drop from the rafters, and the court throws when he shouts. At the end he stands.' },
  ...
  { t: 'ram', name: 'THE RAM LORD', sub: 'lord of the fold', desc: 'His horns turn your blade from the front. Dodge or jump his charge and he hits the wall, stunned and soft: cut him from behind or plunge between the horns. He stamps the ground and hops at you. Half dead, he charges twice and rocks fall.' },
];
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster'];
...
function pollGamepad() {
  ...
  const now = { jump: b(0), atk: b(2), dodge: b(1), throw: b(3), block: b(4) || b(5), pause: b(9), left: b(14) || ax < -0.5, right: b(15) || ax > 0.5, up: b(12) || ay < -0.5, down: b(13) || ay > 0.5 };
  const rose = k => now[k] && !pad.prev[k];
  if (Object.values(now).some(Boolean)) { initAudio(); anyPress = anyPress || Object.keys(now).some(rose); }
  if (rose('jump')) { jumpPress = true; confirmPress = true; } if (rose('atk')) atkPress = true; if (rose('dodge')) dodgePress = true; if (rose('throw')) throwPress = true; if (rose('pause')) pausePress = true;
...
// ---------- collision ----------
const isSolid = (tx, ty) => { const t = tileAt(tx, ty); return t === T.SOLID || t === T.CRATE || t === T.PALISADE || t === T.PORT || t === T.CLIMB || t === T.SOFT; };
const isOneWay = t => t === T.ONEWAY || t === T.REED || t === T.PLANK || t === T.NET || t === T.BOUNCER || t === T.SHELF || t === T.RAIL;
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.hp <= 0) {
    e.alive = false; kills++; killFlash = 0.05; rumble(70, 0.35); ringAt(e.x, e.y - e.h / 2, e.t === 'queen' || e.t === 'frog' || e.t === 'chief' ? 40 : 16, COLS[e.t] ? COLS[e.t][0] : '#fff6e0'); if (['sprig', 'archer', 'sapper', 'shield', 'thorn', 'brute', 'chief', 'queen', 'frog', 'hound', 'harpy', 'goat', 'ram'].includes(e.t)) SFX.kill(); if (e.t === 'shield' || e.t === 'queen' || e.t === 'frog' || e.t === 'chief') SFX.heavy();
    ...
    if (e.t === 'bearer' && boss && boss.t === 'king' && boss.alive) { boss.y += 2; shakeCam(3); SFX.thud(); number(boss.x, boss.y - 30, 'THE THRONE LURCHES', '#ffd36b'); if (!enemies.some(b => b.alive && b.t === 'bearer')) { boss.phase = 2; boss.y = L.arena.floor; boss.mode = 'grounded'; boss.modeT = 1.2; number(boss.x, boss.y - 36, 'THE THRONE FALLS', '#ff6b6b'); SFX.heavy(); shakeCam(8); zoomKick(1.12, 0.4); } }
    if (e.t === 'greathound') { for (const h of enemies) if (h.alive && h.t === 'hound' && h.pack) { h.alive = false; burst(h.x, h.y - 3, 6, COLS.hound, 50, 0.4); } if (L.mini) { for (let ty = 0; ty < LH; ty++) { const i = ty * LW + L.mini.gate; if (L.grid[i] === T.PORT) { L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); } } setWallAt(L.mini.wallL, false, L.mini.floor); miniActive = false; miniDone = true; camLock = null; number(e.x, e.y - 30, 'THE KENNELS OPEN', '#8fd160'); SFX.heavy(); } music.play(L.music || 'theme'); }
  } else {
    ...
    const voice = { miner: 'gobHurt', bat: 'chitter', forgemaster: 'snort', greathound: 'yelp', spider: 'hiss', owl: 'screech', troll: 'snort', harpy: 'screech', goat: 'goatCry', ram: 'bellow', queen: 'bossHurt', frog: 'croak', sprig: 'gobHurt', shield: 'gobHurt', archer: 'gobHurt', sapper: 'gobHurt', thorn: 'clatter', brute: 'snort', chief: 'snort', hound: 'yelp', wasp: 'chitter', spit: 'hiss', hopper: 'ribbit', sporeling: 'squelch', shaman: 'squelch', lurker: 'squelch', gill: 'squelch', heart: 'heart', mother: 'thump' }[e.t]; if (voice && SFX[voice]) SFX[voice](); else SFX.hit(); if (e.t === 'thorn' || e.t === 'sprig' || e.t === 'archer' || e.t === 'sapper' || e.t === 'shield' || e.t === 'brute' || e.t === 'chief') SFX.hit();
    ...
    if (e.t === 'king' && e.phase === 2 && e.hp <= e.maxHp * 0.4) { e.phase = 3; e.mode = 'rise'; e.modeT = 1.5; number(e.x, e.y - 40, 'THE KING RISES', '#ff6b6b'); SFX.roar(); shakeCam(6); for (const f of enemies) if (f.alive && f.t === 'folk' && f.court) f.cower = true; }
...
function updatePlayer(dt) {
  ...
  if (throwPress && isPyro() && skillNow() === 'fireWall' && P.ground && !(P.slamCd > 0) && !P.dead && !(P.hurt > 0) && !(P.asleep > 0) && !(P.dodge > 0)) { if (spend(25)) { P.slamCd = 4; P.atk = -1; for (let i = 1; i <= 5; i++) fires.push({ x: P.x + P.face * i * 14, y: P.y, life: 3, delay: i * 0.08 }); SFX.heavy(); SFX.puff(); number(P.x, P.y - 24, 'FIRE WALL', '#ff9a5c'); shakeCam(2); } else number(P.x, P.y - 22, 'TIRED', '#9aa39a'); }
  if (throwPress && isPyro() && skillNow() === 'cinderStep' && !(P.throwCd > 0) && !P.dead && !(P.hurt > 0) && !(P.asleep > 0) && !(P.dodge > 0)) { if (spend(20)) { P.throwCd = 3; P.dodge = 0.3; P.vx = P.face * 320; P.inv = Math.max(P.inv, 0.4); for (let i = 0; i < 3; i++) fires.push({ x: P.x - P.face * i * 14, y: P.y, life: 1.6, delay: 0 }); SFX.throwWhoosh(); number(P.x, P.y - 24, 'CINDER STEP', '#ff9a5c'); } else number(P.x, P.y - 22, 'TIRED', '#9aa39a'); }
  ...
  if (P.stDelay <= 0 && P.st < P.maxSt) P.st = Math.min(P.maxSt, P.st + ST.regen * (P.relic === 'fleece' ? 2 : 1) * dt);
  ...
  if (isPyro()) { // C tapped = an ember; C held = the flare, a shield of fire that costs heat. Overheated, you have neither.
    P.heat = Math.max(0, Math.min(100, (P.heat || 0))); P.overheat = Math.max(0, (P.overheat || 0) - dt);
    if (keys.block) P.cHeld = (P.cHeld || 0) + dt; else { if (P.cHeld > 0 && P.cHeld < 0.14) castEmber(0); P.cHeld = 0; }
    if (keys.block && P.cHeld >= 0.14 && !(P.overheat > 0)) { P.heat += 22 * dt; if (P.heat >= 100) { P.heat = 100; P.overheat = 1.6; number(P.x, P.y - 24, 'OVERHEATED', '#ff6b6b'); SFX.gasp(); } }
    else P.heat -= (P.asleep > 0 || P.sleepM > 0.2 ? 8 : 16) * dt;
    if (P.overheat > 0 && Math.random() < dt * 20) parts.push({ x: P.x + (Math.random() - 0.5) * 10, y: P.y - 12, vx: 0, vy: -30, life: 0.4, max: 0.4, col: '#8a8478', size: 1, grav: 0 });
  }
  P.block = !!keys.block && P.ground && !attacking && !P.plunge && !dodging && !stunned && P.guardTired <= 0 && P.st > 0 && !thrown && (!isPyro() || (P.cHeld >= 0.14 && !(P.overheat > 0)));
  if (isPyro() && P.block) { // the flare: burns what is next to you and what flies at you
    if (Math.random() < dt * 30) parts.push({ x: P.x + (Math.random() - 0.5) * 22, y: P.y - 8 + (Math.random() - 0.5) * 20, vx: (Math.random() - 0.5) * 20, vy: -40, life: 0.3, max: 0.3, col: Math.random() < 0.5 ? '#ff9a5c' : '#ffd36b', size: 1, grav: 0 });
    P.flareTick = (P.flareTick || 0) - dt; if (P.flareTick <= 0) { P.flareTick = 0.3; for (const e of enemies) if (e.alive && !e.harmless && !e.maxHp && Math.abs(e.x - P.x) < 22 && Math.abs(e.y - P.y) < 22) { hurtEnemy(e, 2, P.x, false); e.burn = Math.max(e.burn || 0, 0.8); } }
    for (const s of seeds) if (!s.dead && Math.abs(s.x - P.x) < 24 && Math.abs(s.y - (P.y - 8)) < 20) { s.dead = true; parries++; burst(s.x, s.y, 4, ['#ff9a5c', '#ffd36b'], 40, 0.3, 0, 1); }
    for (const c of clouds2) if (Math.abs(c.x - P.x) < 30 && Math.abs(c.y - (P.y - 8)) < 24) c.life = Math.min(c.life, 0.2);
  }
  ...
  const cap = P.block ? 32 : wading ? 46 : spored ? 40 : RUN * (PROG.charm === 'swift' ? 1.12 : 1);
  if (move && !groundAtk) {
  ...
  if (isPyro() && P.jbuf > 0 && !P.ground && P.coyote <= 0 && P.kickT <= 0 && !P.cling && !stunned && !dodging && !(P.overheat > 0) && P.heat < 100) {
    P.jbuf = 0; P.kickT = 0.28; let hit = false;
    for (const e of enemies) if (e.alive && !e.harmless && Math.abs(e.x - P.x) < 15 && e.y - e.h / 2 > P.y - 6 && e.y - e.h / 2 < P.y + 30) { hit = true; if (e.t === 'shield' || e.t === 'pike' || (e.t === 'ram' && e.mode !== 'crash') || e.t === 'mother' || e.t === 'gill') { SFX.clank(); } else { hurtEnemy(e, 12, P.x, true); e.burn = Math.max(e.burn || 0, 1.2); } }
    for (const pr of props) if (pr.t === 'puffball' && !pr.popped && Math.abs(pr.x - P.x) < 14 && pr.y > P.y && pr.y < P.y + 30) { hit = true; }
    if (hit) { P.vy = POGO; pogoCount++; SFX.pogo(); number(P.x, P.y - 24, 'KICK', '#ff9a5c'); } else { P.vy = -250; P.heat = Math.min(100, P.heat + 20); SFX.puff(); }
    P.canCut = false; P.plunge = false; P.ground = false; burst(P.x, P.y + 2, 10, ['#ff9a5c', '#ffd36b', '#ff6b2c'], 70, 0.4, 200, 2); dust(P.x, P.y + 4, 4); // a kick is a burst: a tap gives the full launch
  }
  ...
  if (P.abuf > 0 && !stunned && !P.plunge && !dodging) {
    ...
    else if (!P.ground && (keys.down || P.abufDown)) { P.abuf = 0; P.abufDown = false; if (spend(ST.plunge)) { P.plunge = true; P.vy = Math.max(P.vy, 60); P.atk = -1; P.hitSet.clear(); SFX.slash(); } }
  ...
  if (hb) {
    for (const e of enemies) {
      ...
      if (P.plunge) {
        ...
        if (e.t === 'ram') { if (e.mode === 'crash') { hurtEnemy(e, PLUNGE_DMG * 2, P.x, true); number(e.x, e.y - 30, 'BETWEEN THE HORNS', '#ffd36b'); } else { SFX.clank(); number(e.x, e.y - 24, 'HE SHRUGS IT OFF', '#9aa39a'); } P.vy = POGO; P.ground = false; P.plunge = false; P.canCut = false; P.hitSet.clear(); continue; }
        if (e.t === 'king' && e.phase === 1) { SFX.clank(); number(e.x, e.y - 24, 'THE BEARERS FIRST', '#9aa39a'); P.vy = POGO; P.ground = false; P.plunge = false; P.canCut = false; continue; }
      ...
      if (e.t === 'ram' && e.mode !== 'crash') { SFX.clank(); hitstop(0.05); sparks(e.x + e.face * 14, e.y - 8, P.face, 5); P.vx = e.face * 120; number(e.x, e.y - e.h - 6, 'HORNS', '#c9a83a'); continue; }
      ...
      if (e.t === 'king' && e.phase === 1) { SFX.clank(); number(e.x, e.y - 24, 'THE BEARERS FIRST', '#9aa39a'); continue; }
      ...
      } else { hurtEnemy(e, swordDmg(), P.x, false); swordEffect(e); }
    }
  ...
  for (const e of enemies) {
    ...
    if (!P.plunge && P.vy > 40 && pb.b <= e.y - e.h + 7 && P.dodge <= 0) {
      ...
      if (e.t === 'shield' || e.t === 'queen' || e.t === 'brute' || e.t === 'chief' || e.t === 'king' || (e.t === 'master' && e.mounted) || (e.t === 'ram' && e.mode !== 'crash')) { P.vy = keys.jump ? -260 : -190; SFX.clank(); sparks(e.x, e.y - e.h, P.face, 6); e.stagger = Math.max(e.stagger, 0.3); number(e.x, e.y - e.h - 6, 'HELM', '#c9d1dc'); squash(0.85, 1.2, 0.1); continue; }
      ...
      hurtEnemy(e, P.relic === 'crown' ? PLUNGE_DMG : 10, P.x, true); P.vy = keys.jump ? -290 : -220; SFX.pogo(); squash(0.8, 1.25, 0.1); continue;
    }
    if (e.t === 'brute' || e.t === 'chief' || e.t === 'mother' || e.t === 'gill' || e.t === 'heart' || e.t === 'folk' || e.t === 'king' || e.t === 'bearer' || e.t === 'pike' || e.t === 'master' || e.t === 'thief' || e.t === 'ram' || e.t === 'goat' || e.t === 'harpy' || e.t === 'troll' || e.t === 'greathound' || e.t === 'spider' || e.t === 'owl' || e.t === 'miner' || e.t === 'bat' || e.t === 'forgemaster') continue; // their damage comes from their attacks, not from touching them
    const res = e.turncoat ? 'none' : damagePlayer(e.x, e.t === 'hopper' ? HOP[e.color || 'green'].dmg : e.pack ? 12 : DMG[e.t]);
    if (res === 'blocked') {
  ...
  for (const s of seeds) if (!s.dead && !s.reflected && overlap(cb, { l: s.x - 2, r: s.x + 2, t: s.y - 2, b: s.y + 2 })) { s.dead = true; damagePlayer(s.x - s.vx * 0.1, s.venom ? DMG.venom : s.arrow ? DMG.arrow : s.spore ? DMG.sporeRain : s.goblet ? DMG.goblet : s.feather ? DMG.feather : s.steam ? DMG.steam : s.slag ? DMG.fire : DMG.seed); }
  ...
  if (L.mini && !miniActive && !miniDone && P.x > L.mini.trigger && P.x < L.mini.x1 - 24 && P.ground) { miniActive = true; miniIntroT = 1.6; const mb = enemies.find(e => e.alive && e.t === 'greathound'); if (mb) { mb.mode = 'idle'; mb.modeT = 1.2; music.play('boss'); setWallAt(L.mini.wallL, true, L.mini.floor); camLock = { x0: L.mini.x0, x1: L.mini.x1 }; number(mb.x, mb.y - 30, 'THE GREAT HOUND', '#ffd36b'); SFX.roar(); SFX.bark(); shakeCam(4); zoomKick(1.08, 0.3); } }
...
function bossStart() {
  bossActive = true; boss.mode = 'wake'; boss.modeT = 1.6; if (boss.t === 'mother') { boss.rootT = 3; boss.belchT = 8; setView('zoom'); } if (boss.t === 'owl' || boss.t === 'forgemaster') setView('zoom'); camLock = { x0: L.arena.x0, x1: L.arena.x1 }; if (boss.t === 'frog') SFX.croak(); if (boss.t === 'chief') SFX.gobDie(); if (boss.t === 'ram') SFX.bellow();
  setWall(L.arena.wallL, true); setWall(L.arena.wallR, true);
  SFX.roar(); shakeCam(6); music.stop(); bossMusicT = 1.1; number(boss.x, boss.y - 30, boss.t === 'frog' ? 'THE BULLFROG KING' : boss.t === 'chief' ? 'THE GOBLIN CHIEFTAIN' : boss.t === 'mother' ? 'THE MOTHER CAP' : boss.t === 'king' ? 'KING GORM UNDERLEAF' : boss.t === 'ram' ? 'THE RAM LORD' : boss.t === 'owl' ? 'THE OWL REEVE' : boss.t === 'forgemaster' ? 'THE FORGEMASTER' : 'THE HORNET QUEEN', '#ffd36b'); zoomKick(1.1, 0.4);
...
function updateRain(dt) {
  for (const r of rain) { r.t -= dt; if (r.t <= 0 && !r.fired) { r.fired = true; if (r.bolt) strikeBolt(r); else seeds.push({ x: r.x, y: r.y - 170, vx: 0, vy: 240, g: 260, dead: false, life: 1.6, arrow: true, owner: r.owner }); } }
...
function updateChief(e, dt) {
  ...
  const pickAttack = () => {
    ...
    if (e.stance === 'club') { const pool = ['over', 'sweep', 'grab', 'whirl', 'sweep']; let pick = pool[(Math.random() * pool.length) | 0]; if (pick === e.last) pick = pool[(Math.random() * pool.length) | 0]; e.last = pick;
      ...
      else if (pick === 'whirl') { e.mode = 'whirlWind'; e.modeT = 0.5; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.charge(); }
    ...
    else if (Math.random() < (p2 ? 0.5 : 0.4) && e.last !== 'rain') { e.last = 'rain'; e.mode = 'rainAim'; e.modeT = 0.6; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.bow(); }
    else { e.last = 'aim'; e.mode = 'aim'; e.modeT = 0.55; e.shots = p2 ? 3 : 2; number(e.x, e.y - e.h - 12, '!', '#ffd36b'); SFX.bow(); }
  };
  switch (e.mode) {
    case 'sleep': e.y = floor; e.vy = 0; return;
...
function playerLight() { let r = P.relic === 'lamp' ? 90 : 26; if (isPyro() && P.block) r = Math.max(r, 70); if (isPyro() && embers.length) r = Math.max(r, 50); return r; }
...
function updateBat(e, dt) {
  ...
  const lightHere = (x, y) => { let best = null, bd = 150; const try1 = (lx, ly, lr, who) => { const dd = Math.hypot(lx - x, ly - y); if (dd < bd && lr > 20) { bd = dd; best = { x: lx, y: ly, who }; } }; for (const lt of lights) if (!(lt.ref && lt.ref.dark > 0) && !(lt.lantern && !lt.lantern.lit)) try1(lt.x, lt.y, lt.r, null); for (const f of fires) if (f.delay <= 0) try1(f.x, f.y - 6, 40, null); if (!P.dead && playerLight() > 40) try1(P.x, P.y - 8, playerLight(), 'P'); return best; };
...
function updateCarts(dt) {
  for (const m of movers) {
    if (m.kind === 'orelift') { const loaded = movers.some(c => c.kind === 'cart' && !c.gone && c.x + c.w > m.x && c.x < m.x + m.w && Math.abs(c.y + c.h - m.y) < 8); const oy = m.y; m.y = loaded ? Math.min(m.y1, m.y + 40 * dt) : Math.max(m.y0, m.y - 30 * dt); m.dy = m.y - oy; if (loaded && m.dy !== 0 && Math.random() < dt * 8) parts.push({ x: m.x + Math.random() * m.w, y: m.y, vx: 0, vy: 20, life: 0.3, max: 0.3, col: '#8a919c', size: 1, grav: 0 }); continue; }
    if (m.kind !== 'cart') continue;
    if (m.gone) { m.respawnT -= dt; if (m.respawnT <= 0 && Math.abs(P.x - m.x0) > 60) { m.gone = false; m.x = m.x0; m.y = m.y0; m.vx = 0; m.vy = 0; m.rolling = m.auto; m.hit.clear(); number(m.x + 14, m.y - 10, 'ANOTHER CART', '#8a919c'); } continue; }
    const ox = m.x, oy = m.y;
    if (P.onMover === m) m.rolling = true;
    const pan = movers.find(l => l.kind === 'orelift' && m.x + m.w > l.x && m.x < l.x + l.w && Math.abs(m.y + m.h - l.y) < 12);
    if (m.rolling) { m.vx += (m.dir * m.speed - m.vx) * Math.min(1, dt * 3); m.x += m.vx * dt; }
    ...
    if (ahead === T.PORT || ahead === T.CRATE || ahead === T.PALISADE || ahead === T.SOFT) { const i = Math.floor((m.y + m.h - 4) / TS) * LW + fx; L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); burst(fx * TS + 8, m.y + 4, 10, ['#8a5a32', '#c9b27c'], 80, 0.5); shakeCam(3); SFX.crack(); number(fx * TS + 8, m.y - 12, 'SMASHED', '#8fd160'); }
    ...
    else { m.vy += 900 * dt; m.y += m.vy * dt; if (m.y > LH * TS) { m.gone = true; m.respawnT = 5; if (P.onMover === m) P.onMover = null; continue; } const ly = Math.floor((m.y + m.h) / TS), lt = tileAt(Math.floor((m.x + m.w / 2) / TS), ly); if (m.vy > 0 && (lt === T.RAIL || isSolid(Math.floor((m.x + m.w / 2) / TS), ly)) && (m.y + m.h) - ly * TS < 10) { m.y = ly * TS - m.h; m.vy = 0; dust(m.x + m.w / 2, m.y + m.h, 4); } }
...
function updateOwl(e, dt) {
  const A = L.arena, floor = A.floor, p2 = e.phase === 2; e.modeT -= dt; e.hitT = Math.max(0, e.hitT - dt);
  const d = P.x - e.x, ad = Math.abs(d);
  if (e.mode === 'sleep') return;
  const lanterns = props.filter(pr => pr.t === 'lantern');
  if (e.mode !== 'wake') { // two things always going on under its moves: spiders on threads from the bough, and feathers on their own clock
    e.spiderT = (e.spiderT === undefined ? 5 : e.spiderT - dt); const nSp = enemies.filter(q => q.alive && q.t === 'spider').length;
    if (e.spiderT <= 0 && nSp < (p2 ? 3 : 2)) { e.spiderT = p2 ? 7 : 9; const sx = Math.max(A.x0 + 40, Math.min(A.x1 - 40, P.x + (Math.random() < 0.5 ? -50 : 50))); enemies.push({ t: 'spider', x: sx, y: e.py + 30, restY: e.py + 30, drop: 150, mode: 'hang', modeT: 0, cd: 1.2, face: 1, w: 10, h: 8, hp: EHP.spider, hp0: EHP.spider, alive: true, dying: 0, anim: Math.random(), flash: 0, stagger: 0, vx: 0, vy: 0 }); number(sx, e.py + 20, 'A SPIDER DROPS', '#c080ff'); SFX.hiss(); }
    e.rainT = (e.rainT === undefined ? 6 : e.rainT - dt);
    if (e.rainT <= 0 && e.mode !== 'crash') { e.rainT = p2 ? 6 : 8.5; number(e.x, e.y - 24, 'FEATHERS', '#e8dcc0'); for (let i = 0; i < (p2 ? 7 : 5); i++) { const x = A.x0 + 20 + Math.random() * (A.x1 - A.x0 - 40); seeds.push({ x, y: floor - 150, vx: (Math.random() - 0.5) * 20, vy: 30 + Math.random() * 40, dead: false, life: 3.5, g: 260, feather: true }); } }
  }
  ...
  const perchX = e.px, perchY = e.py;
  switch (e.mode) {
    case 'wake': if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 1.2; } break;
    case 'perch': { e.face = Math.sign(d) || e.face; toward(perchX, perchY, 200); for (const k of ['swoopT', 'screechT', 'gustT', 'featherT']) e[k] -= dt;
      if (e.modeT <= 0 && !P.dead) {
        if (p2 && !e.doused) { e.doused = true; e.mode = 'douse'; e.modeT = 1.0; number(e.x, e.y - 24, 'IT BEATS OUT THE LIGHT', '#c080ff'); SFX.screech(); }
        else if (e.screechT <= 0) { e.screechT = p2 ? 9 : 12; e.mode = 'landing'; e.modeT = 1.2; e.tx = Math.max(A.x0 + 30, Math.min(A.x1 - 30, P.x + (Math.sign(e.x - P.x) || 1) * 44)); number(e.x, e.y - 24, 'IT COMES DOWN', '#ffd36b'); }
        else if (e.gustT <= 0 && ad < 200) { e.gustT = p2 ? 10 : 14; e.mode = 'gust'; e.modeT = 1.4; e.gustDir = P.x - e.x > 0 ? 1 : -1; number(e.x, e.y - 24, 'WING BEAT', '#c9d1dc'); SFX.buzz(); }
        else if (e.swoopT <= 0) { e.swoopT = p2 ? 3.2 : 4.2; e.grab = Math.random() < (p2 ? 0.4 : 0.25); e.mode = 'swoopTell'; e.modeT = 0.6; number(e.x, e.y - 24, e.grab ? 'TALONS' : '!!', e.grab ? '#ff9a5c' : '#ff6b6b'); SFX.screech(); }
      } break; }
    case 'douse': if (e.modeT <= 0) { let n = 0; for (const pr of lanterns) if (pr.lit && n < 2) { pr.lit = false; n++; burst(pr.x, pr.y - 30, 8, ['#ffd36b', '#3a3444'], 40, 0.5); } number(e.x, e.y - 24, 'TWO LANTERNS DARK. STRIKE THEM TWICE.', '#ffd36b'); for (const dx of [-30, 30]) enemies.push({ t: 'spider', x: e.px + dx, y: e.py + 30, restY: e.py + 30, drop: 140, mode: 'hang', modeT: 0, cd: 1, face: 1, w: 10, h: 8, hp: EHP.spider, hp0: EHP.spider, alive: true, dying: 0, anim: Math.random(), flash: 0, stagger: 0, vx: 0, vy: 0 }); e.mode = 'perch'; e.modeT = 1; } break;
    case 'swoopTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'swoop'; e.modeT = 1.6; const tx = P.x + (Math.sign(P.x - e.x) || 1) * 28, ty = P.y - 8; const dd = Math.hypot(tx - e.x, ty - e.y) || 1; e.vx = (tx - e.x) / dd * 300; e.vy = (ty - e.y) / dd * 300; e.face = Math.sign(e.vx) || e.face; SFX.leap(); } break;
    case 'swoop': { e.x += e.vx * dt; e.y += e.vy * dt;
      ...
      for (const pr of lanterns) if (pr.lit && Math.abs(pr.x - e.x) < 16 && Math.abs((pr.y - 30) - e.y) < 18) { e.mode = 'crash'; e.modeT = p2 ? 2.0 : 2.5; e.stagger = e.modeT; e.vx = 0; e.vy = 0; number(e.x, e.y - 24, 'INTO THE LANTERN: HIT IT', '#8fd160'); SFX.heavy(); SFX.screech(); shakeCam(6); zoomKick(1.1, 0.3); burst(e.x, e.y, 16, COLS.owl, 80, 0.7); break; }
      ...
      if (e.modeT <= 0 && e.mode === 'swoop') { e.mode = 'perch'; e.modeT = 1.4; } break; }
    case 'carry': { e.x += e.carryDir * 170 * dt; e.y = Math.max(floor - 70, e.y + e.vy * dt); P.x = e.x; P.y = e.y + 12; P.vx = 0; P.vy = 0; P.ground = false; P.caged = 0.2; if (Math.random() < dt * 20) parts.push({ x: P.x, y: P.y - 8, vx: 0, vy: 30, life: 0.3, max: 0.3, col: '#e8dcc0', size: 1, grav: 0 }); if (e.modeT <= 0 || e.x < A.x0 + 30 || e.x > A.x1 - 30) { damagePlayer(e.x, DMG.screech, { unblockable: true, up: true }); P.vx = e.carryDir * 200; P.vy = 120; number(P.x, P.y - 24, 'DROPPED', '#ff6b6b'); e.mode = 'perch'; e.modeT = 1.2; } break; }
    case 'crash': { if (e.y < floor - 8) e.y = Math.min(floor - 8, e.y + 300 * dt); if (Math.floor(e.modeT / 0.7) !== Math.floor((e.modeT + dt) / 0.7)) number(e.x, e.y - 24, 'DAZED', '#8fd160'); if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 1.0; e.stagger = 0; } break; }
    case 'parried': if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 0.8; e.stagger = 0; } break;
    case 'landing': { const left = toward(e.tx, floor - 8, 260); e.face = Math.sign(P.x - e.x) || e.face; if (left <= 1 || e.modeT <= 0) { e.mode = 'screechTell'; e.modeT = 0.5; number(e.x, e.y - 24, 'IT DRAWS BREATH', '#ffd36b'); } break; }
    case 'screechTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'screech'; e.modeT = 1.1; SFX.screech(); SFX.screech(); shakeCam(3); if (!P.dead && ad < 100 && Math.abs(P.y - e.y) < 40) { const res = damagePlayer(e.x, DMG.screech); if (res === 'hit') { P.hurt = Math.max(P.hurt || 0, 0.6); number(P.x, P.y - 24, 'DEAFENED', '#ff6b6b'); } } number(e.x, e.y - 24, 'ON THE GROUND: HIT IT', '#8fd160'); } break;
    case 'screech': if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 1.2; } break;
    case 'gust': { if (!P.dead && Math.abs(P.y - floor) < 50) { P.vx += e.gustDir * 230 * dt; if (Math.random() < dt * 50) parts.push({ x: camX + Math.random() * VW, y: floor - 10 - Math.random() * 90, vx: e.gustDir * 240, vy: 0, life: 0.3, max: 0.3, col: '#e8dcc0', size: 1, grav: 0 }); } if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 1.0; } break; }
    case 'feathers': if (e.modeT <= 0) { e.mode = 'perch'; e.modeT = 1.2; } break;
  }
  if (e.mode !== 'swoop' && e.mode !== 'crash') e.vy = 0;
...
function updateForgemaster(e, dt) {
  const A = L.arena, floor = A.floor, p2 = e.phase === 2; e.modeT -= dt; e.hitT = Math.max(0, e.hitT - dt); e.vy += 1000 * dt; if (e.vy > 300) e.vy = 300;
  const d = P.x - e.x, ad = Math.abs(d);
  if (e.mode === 'sleep') return;
  if (e.mode !== 'wake') {
    e.hammerT -= dt; if (e.hammerT <= 0) { e.hammerT = p2 ? 2.4 : 3.2; const hm = props.find(pr => pr.t === 'hammer'); if (hm) { hm.tell = 0.6; number(hm.x, hm.y - 40, 'THE HAMMER', '#ff9a5c'); SFX.charge(); } }
    e.cartT -= dt; if (e.cartT <= 0) { e.cartT = p2 ? 4.5 : 6; const c = movers.find(mv => mv.kind === 'cart' && mv.gone && mv.auto); if (c) { c.gone = false; c.x = c.x0; c.y = c.y0; c.vx = 0; c.vy = 0; c.rolling = true; c.hit.clear(); number(c.x0 + 14, c.y0 - 12, 'A CART COMES', '#8a919c'); SFX.stone(); } }
    e.slagT -= dt; if (e.slagT <= 0) { e.slagT = p2 ? 4 : 5.5; for (const sx of (A.slag || [])) if (Math.random() < 0.7) { seeds.push({ x: sx, y: floor - 140, vx: 0, vy: 40, dead: false, life: 3, g: 300, slag: true }); parts.push({ x: sx, y: floor - 140, vx: 0, vy: 20, life: 0.5, max: 0.5, col: '#ff9a5c', size: 2, grav: 0 }); } number(e.x, e.y - e.h - 12, 'SLAG', '#ff9a5c'); }
    if (p2 && !e.doused) { e.doused = true; L.dark = 0.88; for (const pr of props) if (pr.t === 'minerlamp') pr.lit = false; for (const lt of lights) if (lt.warm && !lt.forge) lt.r = 0; number(e.x, e.y - e.h - 12, 'THE LAMPS GO OUT', '#c080ff'); SFX.roar(); for (const dx of [-90, 90]) enemies.push({ t: 'bat', x: e.x + dx, y: floor - 90, hx: e.x + dx, hy: floor - 90, w: 10, h: 6, hp: EHP.bat, hp0: EHP.bat, mode: 'hang', modeT: 0, cd: 0.5, face: -1, alive: true, dying: 0, anim: Math.random(), flash: 0, stagger: 0, vx: 0, vy: 0 }); }
  }
  let want = 0;
  switch (e.mode) {
    case 'wake': if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 1; } break;
    case 'scald': if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 0.6; e.stagger = 0; } break;
    case 'pace': { e.face = Math.sign(d) || e.face; want = ad > 90 ? e.face * 40 : ad < 40 ? -e.face * 40 : 0; for (const k of ['lungeT', 'sprayT', 'kickT']) e[k] -= dt;
      if (e.modeT <= 0 && !P.dead) { if (e.kickT <= 0) { e.kickT = p2 ? 8 : 11; e.mode = 'kickTell'; e.modeT = 0.6; number(e.x, e.y - e.h - 12, 'HE WINDS UP', '#ffd36b'); } else if (e.sprayT <= 0 && ad < 110) { e.sprayT = p2 ? 5 : 7; e.mode = 'sprayTell'; e.modeT = 0.5; number(e.x, e.y - e.h - 12, 'STEAM', '#e8e0d0'); SFX.hiss(); } else if (e.lungeT <= 0 && ad > 30) { e.lungeT = p2 ? 3 : 4; e.mode = 'lungeTell'; e.modeT = 0.55; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.charge(); } }
      break; }
    case 'lungeTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'lunge'; e.modeT = 0.45; e.vx = e.face * 280; SFX.heavy(); } break;
    case 'lunge': want = e.face * 280; if (!P.dead && ad < 22 && Math.abs(P.y - e.y) < 22 && e.hitT <= 0) { e.hitT = 0.8; const res = damagePlayer(e.x, DMG.piston); if (res === 'hit') P.vx = e.face * 240; else if (res === 'blocked') { e.mode = 'stagger'; e.modeT = 1.2; e.stagger = 1.2; e.vx = -e.face * 60; number(e.x, e.y - e.h - 12, 'THE PISTON JAMS', '#8fd160'); SFX.clank(); } } if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 0.8; } break;
    case 'stagger': if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 0.5; e.stagger = 0; } break;
    case 'sprayTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'spray'; e.modeT = 0.9; e.sprayN = 0; } break;
    case 'spray': { e.sprayN = (e.sprayN || 0) + dt * 12; if (Math.floor(e.sprayN) !== Math.floor(e.sprayN - dt * 12) && Math.floor(e.sprayN) <= 8) { seeds.push({ x: e.x + e.face * 14, y: e.y - 14, vx: e.face * (180 + Math.random() * 40), vy: -20 + Math.random() * 40, dead: false, life: 0.7, g: 60, steam: true }); SFX.hiss(); } if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 0.7; } break; }
    case 'kickTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'kick'; e.modeT = 0.4; const c = movers.find(mv => mv.kind === 'cart' && mv.gone); if (c) { c.gone = false; c.x = e.x + e.face * 20; c.y = floor - c.h; c.vx = e.face * 220; c.vy = 0; c.rolling = true; c.dir = e.face; c.hit.clear(); number(e.x, e.y - e.h - 12, 'A CART, KICKED', '#ff6b6b'); SFX.heavy(); } } break;
    case 'kick': if (e.modeT <= 0) { e.mode = 'pace'; e.modeT = 0.8; } break;
  }
  if (e.stagger > 0 && e.mode !== 'lunge') want = 0;
  e.vx += (want - e.vx) * Math.min(1, dt * (e.mode === 'lunge' ? 12 : 6));
  const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) e.vy = 0; if (r.hitX && e.mode === 'lunge') { e.mode = 'stagger'; e.modeT = 0.8; e.stagger = 0.8; e.vx = 0; }
...
function updateTroll(e, dt) {
  ...
  let want = 0;
  if (e.mode === 'throwTell') { e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'throw'; e.modeT = 0.4; const tt = 0.9, tx = P.x + P.vx * 0.4; rocks.push({ x: e.x + e.face * 8, y: e.y - 24, vx: (tx - e.x) / tt, vy: -330, t: 0, dead: false, thrown: true }); SFX.heavy(); SFX.snort(); number(e.x, e.y - e.h - 10, 'HURLS', '#ff9a5c'); } }
  ...
  else if (near) { e.face = Math.sign(d) || e.face; if (ad < 32 && e.modeT <= 0) { e.mode = 'swatTell'; e.modeT = 0.5; number(e.x, e.y - e.h - 10, '!', '#ffd36b'); SFX.snort(); } else if (e.throwT <= 0 && ad > 56 && ad < 240) { e.throwT = 3.4; e.mode = 'throwTell'; e.modeT = 0.6; number(e.x, e.y - e.h - 10, '!!', '#ff9a5c'); SFX.snort(); } else want = ad > 64 ? e.face * e.speed : 0; }
  else want = e.face * 8;
...
function updateGoat(e, dt) {
  ...
  let want = 0;
  if (!e.rider) { e.fleeT = (e.fleeT || 0) - dt; want = e.face * 120; if (e.fleeT <= 0 || Math.abs(e.x - P.x) > 420) e.alive = false; }
  ...
  if (!e.air && e.vy === 0 && (e.mode === 'charge' || !e.rider) && isSolid(ftx, fty - 1) && !isSolid(ftx, fty - 3)) { e.vy = -250; e.air = true; }
  const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) { e.vy = 0; e.air = false; }
  if (r.ground && !e.air && e.mode !== 'charge' && e.rider && tileAt(ftx, fty) === T.AIR && !isOneWay(tileAt(ftx, fty))) { e.face = -e.face; e.vx = 0; }
  if (r.hitX) { if (e.mode === 'charge' && !e.air) { e.vy = -250; e.air = true; } else if (e.rider) { e.face = -e.face; e.vx = 0; } }
}
function updateRam(e, dt) {
  ...
  const d = P.x - e.x, ad = Math.abs(d);
  let want = 0;
  const pick = () => { e.n = (e.n || 0) + 1; const r = Math.random(); const flock = enemies.filter(q => q.alive && q.t === 'goat' && q.called).length; if (e.n % 4 === 0 && flock === 0) { e.mode = 'callTell'; e.modeT = 0.7; number(e.x, e.y - e.h - 12, 'CALLS THE FLOCK', '#c080ff'); SFX.bellow(); } else if (ad < 40 && r < 0.55) { e.mode = 'tossTell'; e.modeT = 0.45; number(e.x, e.y - e.h - 12, '!!!', '#ff9a5c'); SFX.snort(); } else if (e.n % 3 === 0 && r < 0.6) { e.mode = 'stampTell'; e.modeT = 0.5; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.snort(); } else if (ad < 70 && r < 0.5) { e.mode = 'buttTell'; e.modeT = 0.35; number(e.x, e.y - e.h - 12, '!', '#ffd36b'); SFX.snort(); } else { e.mode = 'lower'; e.modeT = p2 ? 0.5 : 0.7; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.snort(); } };
  switch (e.mode) {
    ...
    case 'pace': e.face = Math.sign(d) || e.face; want = e.face * (p2 ? 50 : 36); if (e.modeT <= 0) pick(); break;
    ...
    case 'charge': want = e.face * (p2 ? 270 : 220);
      ...
      if (e.x <= A.x0 + 16 || e.x >= A.x1 - 16 || e.modeT <= 0) { e.x = Math.max(A.x0 + 16, Math.min(A.x1 - 16, e.x)); e.vx = 0; e.mode = 'crash'; e.modeT = p2 ? 2.1 : 2.8; shakeCam(9); SFX.heavy(); SFX.stone(); zoomKick(1.12, 0.3); dust(e.x + e.face * 14, e.y, 14); number(e.x, e.y - e.h - 12, 'INTO THE WALL', '#8fd160'); const n = p2 ? 4 : 2; for (let i = 0; i < n; i++) rocks.push({ x: A.x0 + 24 + Math.random() * (A.x1 - A.x0 - 48), y: floor - 150, vy: 0, t: 0, dead: false }); sparks(e.x + e.face * 14, e.y - 10, e.face, 8); }
      break;
  ...
  if (e.mode !== 'butt') e.vx += (want - e.vx) * Math.min(1, dt * (e.mode === 'charge' ? 10 : 6));
  if (e.mode === 'butt') { e.x += e.vx * dt; e.y += e.vy * dt; if (e.y > floor) e.y = floor; }
  else { const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) e.vy = 0; }
  e.x = Math.max(A.x0 + 14, Math.min(A.x1 - 14, e.x));
}
...
function updateRocks(dt) {
  for (const r of rocks) { r.vy = Math.min(420, r.vy + 700 * dt); r.y += r.vy * dt; r.x += (r.vx || 0) * dt; r.t += dt;
    ...
    if (!P.dead && Math.abs(P.x - r.x) < 9 && P.y > r.y - 4 && P.y - 16 < r.y + 6) { r.dead = true; damagePlayer(r.x, DMG.rock, { up: true }); shatterRock(r); continue; }
...
const NPC_LINES = pr => {
  ...
  if (pr.kind === 'foreman') { if (n >= need) return ['ALL THREE. TAKE MY LAMP, KNIGHT. YOU WILL WANT IT DOWN THERE.', 'THE FORGEMASTER KEEPS THE HAMMER GOING. BURST HIS BOILER.']; if (n > 0) return ['THAT IS ' + n + ' OF MY THREE CANARIES.', 'ONE IN THE POCKET BEHIND THE SOFT ROCK. ONE UNDER THE ROTTEN TIMBERS. ONE IN THE DEEP.']; return ['THE CANARIES GOT LOOSE WHEN THE GOBLINS CAME. THREE OF THEM.', 'BRING THEM BACK AND MY LAMP IS YOURS. IT IS DARK BELOW.']; }
  ...
  if (pr.kind === 'oldknight') { const sv = silverAvail(); return [sv >= 15 ? 'FIFTEEN SILVER. THE KEEPER HAS A HERO FOR THAT. A HOT ONE.' : 'THREE SILVER COINS HIDE IN EVERY WOOD. THE KEEPER TRADES HEROES FOR THEM.', PROG.hero === 'pyro' ? 'A PYROMANCER. IN MY DAY WE HAD SHIELDS. YOURS IS ON FIRE.' : 'BLOCK EARLY, PLUNGE LATE. I LIVED THIS LONG.', 'THE HIGH BOUGHS ARE MINE NO MORE. MY KNEES. GO UP FOR ME.']; }
...
function updateKing(e, dt) {
  ...
  const goblet = () => { const sx = e.x, sy = e.y - 14, Tf = 0.8, G = 380, dx = P.x - sx, dy = (P.y - 8) - sy; seeds.push({ x: sx, y: sy, vx: Math.max(-220, Math.min(220, dx / Tf)), vy: dy / Tf - 0.5 * G * Tf, dead: false, life: 3, g: G, goblet: true }); SFX.clank(); e.throwT = 0.35; number(e.x, e.y - 30, 'HAVE A DRINK', '#ffd36b'); };
  e.throwT = Math.max(0, (e.throwT || 0) - dt);
  if (e.phase === 1) { // carried: the throne paces the hall; goblets and guards
    e.face = Math.sign(d) || e.face; e.x += e.dir * 26 * dt; if (e.x < A.x0 + 50) e.dir = 1; if (e.x > A.x1 - 50) e.dir = -1;
    const alive = enemies.filter(b => b.alive && b.t === 'bearer').length; e.y = floor - 9 + (4 - alive) * 2;
    e.gobletT -= dt; if (e.gobletT <= 0) { e.gobletT = 2.8; goblet(); }
    e.summonT -= dt; if (e.summonT <= 0 && enemies.filter(g => g.alive && g.t === 'shield' && g.guard).length < 2) { e.summonT = 12; number(e.x, e.y - 34, 'GUARDS!', '#ff6b6b'); SFX.roar(); for (const sx of [A.x0 + 20, A.x1 - 20]) enemies.push({ t: 'shield', x: sx, y: floor, vx: 0, vy: -100, w: 12, h: 14, hp: EHP.shield, speed: 24, face: Math.sign(P.x - sx) || 1, alive: true, dying: 0, anim: 0, flash: 0, stagger: 0.4, guard: true, turnT: 0 }); }
    return;
  }
  if (e.phase === 2) { // grounded: sceptre sweep, the shout, goblets
    e.y = floor; e.face = Math.sign(d) || e.face;
    if (e.mode === 'grounded' && e.modeT <= 0) e.mode = 'seat';
    if (e.mode === 'seat') { e.gobletT -= dt; e.sweepT -= dt; e.shoutT -= dt; e.boltT -= dt; e.crownT -= dt; e.cageT -= dt;
      if (e.cageT <= 0 && props.some(c => c.t === 'dropcage' && c.boss && !c.dropped && Math.abs(c.x - P.x) < 40)) { e.mode = 'cageTell'; e.modeT = 0.55; number(e.x, e.y - 30, '!!', '#ff6b6b'); SFX.charge(); }
      else if (e.boltT <= 0 && ad > 30) { e.mode = 'boltTell'; e.modeT = 0.6; number(e.x, e.y - 30, '!', '#8fd160'); SFX.buzz(); }
      else if (e.crownT <= 0 && ad > 50 && !e.crown) { e.mode = 'crownTell'; e.modeT = 0.4; number(e.x, e.y - 30, '!', '#ffd36b'); SFX.buzz(); }
      else if (e.sweepT <= 0 && ad < 60) { e.mode = 'sweepTell'; e.modeT = 0.5; number(e.x, e.y - 30, '!', '#ffd36b'); SFX.buzz(); }
      else if (e.shoutT <= 0 && ad < 90) { e.mode = 'shoutTell'; e.modeT = 0.6; number(e.x, e.y - 30, '!!', '#ff6b6b'); SFX.charge(); }
      else if (e.gobletT <= 0 && ad > 40) { e.gobletT = 3; goblet(); } }
    else if (e.mode === 'sweepTell' && e.modeT <= 0) { e.mode = 'sweep'; e.modeT = 0.3; e.sweepT = 3.2; SFX.slash(); if (!P.dead && Math.sign(P.x - e.x) === e.face && ad < 44 && Math.abs(P.y - e.y) < 22) { const res = damagePlayer(e.x, DMG.sceptre); if (res === 'blocked') { e.stagger = 0.9; number(e.x, e.y - 30, 'PARRIED', '#8fd160'); } } }
    else if (e.mode === 'shoutTell' && e.modeT <= 0) { e.mode = 'shout'; e.modeT = 0.4; e.shoutT = 7; SFX.roar(); shakeCam(5); ringAt(e.x, e.y - 12, 60, '#ffd36b', 0.4); number(e.x, e.y - 34, 'SILENCE!', '#ff6b6b'); if (!P.dead && ad < 70 && Math.abs(P.y - e.y) < 40) { damagePlayer(e.x, DMG.shout, { unblockable: true }); P.vx = (Math.sign(P.x - e.x) || 1) * 240; P.vy = -150; } }
    else if (e.mode === 'cageTell' && e.modeT <= 0) { e.mode = 'seat'; e.modeT = 0.5; e.cageT = 8; kingCage(e); }
    else if (e.mode === 'boltTell' && e.modeT <= 0) { e.mode = 'seat'; e.modeT = 0.5; e.boltT = 4.5; kingBolt(e); }
    else if (e.mode === 'crownTell' && e.modeT <= 0) { e.mode = 'seat'; e.modeT = 0.5; e.crownT = 6.5; kingCrown(e); }
    else if ((e.mode === 'sweep' || e.mode === 'shout') && e.modeT <= 0) { e.mode = 'seat'; e.modeT = 0.4; }
    if (e.mode === 'shout' && !e.volleyed) { e.volleyed = true; galleryVolley(e); } if (e.mode !== 'shout') e.volleyed = false;
    updateCrown(e, dt);
    return;
  }
  // phase 3: he stands. A walker with a slam and the shout.
  e.vy = (e.vy || 0) + 1000 * dt; if (e.vy > 300) e.vy = 300; let want = 0;
  if (e.mode === 'rise') { if (e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.4; e.h = 21; } }
  else if (e.mode === 'walk') { e.face = Math.sign(d) || e.face; want = ad > 26 ? e.face * 50 : 0; e.shoutT -= dt; e.boltT -= dt; e.cageT -= dt; if (e.modeT <= 0 && ad < 40) { e.mode = 'slamTell'; e.modeT = 0.7; number(e.x, e.y - 34, '!!', '#ff6b6b'); SFX.charge(); } else if (e.shoutT <= 0 && ad < 90) { e.mode = 'shoutTell'; e.modeT = 0.6; number(e.x, e.y - 34, '!!', '#ff6b6b'); } else if (e.boltT <= 0 && ad > 40) { e.mode = 'boltTell'; e.modeT = 0.5; number(e.x, e.y - 34, '!', '#8fd160'); SFX.buzz(); } else if (e.cageT <= 0 && props.some(c => c.t === 'dropcage' && c.boss && !c.dropped && Math.abs(c.x - P.x) < 40)) { e.mode = 'cageTell'; e.modeT = 0.45; number(e.x, e.y - 34, '!!', '#ff6b6b'); } }
  else if (e.mode === 'boltTell' && e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.6; e.boltT = 3.5; kingBolt(e); kingBolt(e, (Math.sign(P.x - e.x) || 1) * 40); }
  else if (e.mode === 'cageTell' && e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.6; e.cageT = 7; kingCage(e); }
  else if (e.mode === 'slamTell' && e.modeT <= 0) { e.mode = 'slam'; e.modeT = 0.4; shakeCam(7); SFX.heavy(); zoomKick(1.1, 0.25); dust(e.x + e.face * 16, e.y, 12); for (const dd of [-1, 1]) waves.push({ x: e.x + dd * 18, y: floor, dir: dd, life: 1.6, sp: 160 }); if (!P.dead && Math.sign(P.x - e.x) === e.face && ad < 36 && Math.abs(P.y - e.y) < 24) damagePlayer(e.x, DMG.kingSlam, { unblockable: true }); }
  else if (e.mode === 'slam' && e.modeT <= 0) { e.mode = 'walk'; e.modeT = 1.1; }
  else if (e.mode === 'shoutTell' && e.modeT <= 0) { e.mode = 'shout'; e.modeT = 0.4; e.shoutT = 6; SFX.roar(); shakeCam(5); ringAt(e.x, e.y - 12, 60, '#ffd36b', 0.4); number(e.x, e.y - 34, 'KNEEL!', '#ff6b6b'); if (!P.dead && ad < 70 && Math.abs(P.y - e.y) < 40) { damagePlayer(e.x, DMG.shout, { unblockable: true }); P.vx = (Math.sign(P.x - e.x) || 1) * 240; P.vy = -150; } }
  else if (e.mode === 'shout' && e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.6; }
  if (e.stagger > 0) want = 0;
  e.vx = (e.vx || 0) + (want - (e.vx || 0)) * Math.min(1, dt * 6);
  const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) e.vy = 0;
  e.x = Math.max(A.x0 + 12, Math.min(A.x1 - 12, e.x));
}
// King Gorm's court tricks: the sceptre bolt marks the floor and strikes, the crown is a boomerang, the cages drop from the rafters, the galleries throw.
function kingBolt(e, off = 0) { const A = L.arena; const x = Math.max(A.x0 + 10, Math.min(A.x1 - 10, P.x + off)); rain.push({ x, y: A.floor, t: 0.55, owner: e, fired: false, bolt: true }); SFX.buzz(); number(e.x, e.y - (e.phase === 3 ? 40 : 30), 'KNEEL', '#8fd160'); }
function strikeBolt(r) { const A = L.arena; bolts.push({ x: r.x, y: r.y, life: 0.28 }); shakeCam(3); SFX.thunder(); burst(r.x, r.y - 4, 10, ['#8fd160', '#dfffa0', '#fff6e0'], 90, 0.4); if (!P.dead && Math.abs(P.x - r.x) < 11 && P.y > r.y - 60 && P.y <= r.y + 2) { damagePlayer(r.x + 1, DMG.bolt, { up: true, unblockable: true }); number(P.x, P.y - 26, 'STRUCK', '#8fd160'); } }
function kingCrown(e) { e.crown = { t: 0, dir: Math.sign(P.x - e.x) || 1, y: e.y - (e.phase === 3 ? 20 : 14), hit: false }; SFX.throwWhoosh(); number(e.x, e.y - 30, 'THE CROWN', '#ffd36b'); }
function updateCrown(e, dt) { const c = e.crown; if (!c) return; c.t += dt; const T = 1.3; c.x = e.x + c.dir * 140 * Math.sin(Math.PI * c.t / T); if (!c.hit && !P.dead && Math.abs(P.x - c.x) < 9 && Math.abs((P.y - 8) - c.y) < 12) { c.hit = true; const res = damagePlayer(c.x, DMG.crownToss); if (res === 'blocked') { number(P.x, P.y - 24, 'DEFLECTED', '#8fd160'); c.t = Math.max(c.t, T * 0.6); } } if (c.t >= T) { e.crown = null; SFX.shieldCatch(); } }
function kingCage(e) { let best = null; for (const c of props) if (c.t === 'dropcage' && c.boss && !c.dropped && (!best || Math.abs(c.x - P.x) < Math.abs(best.x - P.x))) best = c; if (!best) return; best.dropped = true; best.landed = 0; best.hit.clear(); best.resetT = 3; SFX.stone(); number(best.x, best.y - 10, 'HOLD HIM', '#ff6b6b'); }
...
function ringBell(b) { b.rung = true; SFX.thunder(); SFX.clank(); shakeCam(4); number(b.x, b.y - 30, 'THE GATE DROPS', '#ff6b6b'); for (let ty = 0; ty < LH; ty++) { const i = ty * LW + b.gate; if (ty >= 15 && ty <= 19 && L.grid[i] === T.AIR) { L.grid[i] = T.PORT; tileSpr[i] = TILE.port[(ty + b.gate) % 2]; } } burst(b.gate * TS + 8, 17 * TS, 10, ['#7c8797', '#c9d1dc'], 60, 0.5); }
...
function updateEnemies(dt) {
  for (const e of enemies) {
    ...
    if (e.t === 'forgemaster') { if (bossActive) beastSeen('forgemaster'); if (bossActive || e.mode === 'sleep') updateForgemaster(e, dt); continue; }
  ...
  for (const s of seeds) {
    ...
    if (s.life <= 0 || isSolid(Math.floor(s.x / TS), Math.floor(s.y / TS))) { s.dead = true; burst(s.x, s.y, 3, [s.arrow ? '#e8dcc0' : s.spore ? '#c9a0ff' : '#ff9a5c'], 30, 0.2, 0, 1); if (s.fire && !s.reflected && s.life > 0) { const fy = Math.floor(s.y / TS) * TS; fires.push({ x: s.x, y: fy, life: 2.6, delay: 0 }); SFX.crack(); } if (s.slag) { const fy = Math.floor(s.y / TS) * TS; fires.push({ x: s.x, y: fy, life: 1.6, delay: 0 }); } if (s.spore && s.life > 0) clouds2.push({ x: s.x, y: s.y - 6, r: s.pod ? 20 : 11, life: s.pod ? 2.6 : 1.6, mild: true }); continue; }
...
function castEmber(dir) {
  if (P.dead || P.hurt > 0 || P.asleep > 0 || P.overheat > 0 || P.heat >= 100) { if (P.overheat > 0 || P.heat >= 100) number(P.x, P.y - 22, 'TOO HOT', '#ff6b6b'); return; }
  P.heat = Math.min(100, P.heat + (dir ? 12 : 18)); if (P.heat >= 100) { P.overheat = 1.6; number(P.x, P.y - 24, 'OVERHEATED', '#ff6b6b'); }
...
function updateEmbers(dt) {
  for (const b of embers) { b.life -= dt; b.vy += 110 * dt; b.x += b.vx * dt; b.y += b.vy * dt;
    if (Math.random() < dt * 40) parts.push({ x: b.x, y: b.y, vx: (Math.random() - 0.5) * 20, vy: -20, life: 0.25, max: 0.25, col: Math.random() < 0.5 ? '#ff9a5c' : '#ffd36b', size: 1, grav: 0 });
    ...
    for (const e of enemies) { if (!e.alive || e.harmless || b.hit.has(e)) continue; if (Math.abs(e.x - b.x) < e.w / 2 + 5 && b.y > e.y - e.h - 5 && b.y < e.y + 5) { b.hit.add(e); const big = !!e.maxHp; if (e.t === 'ram' && e.mode !== 'crash') { SFX.clank(); number(e.x, e.y - e.h - 8, 'HIS HIDE TURNS IT', '#9aa39a'); } else if (e.t === 'mother' || e.t === 'gill' || e.t === 'heart' || e.t === 'drone') { SFX.clank(); } else { hurtEnemy(e, big ? 4 : 12, b.x, false); if (!big) e.burn = Math.max(e.burn || 0, 1.2); flinch(e); } b.life = 0; burst(b.x, b.y, 8, ['#ff9a5c', '#ffd36b', '#ff6b2c'], 60, 0.4, 100, 2); break; } }
    const tx = Math.floor(b.x / TS), ty = Math.floor(b.y / TS);
    if (b.life > 0 && isSolid(tx, ty)) { b.life = 0; const fy = isSolid(tx, ty) && !isSolid(tx, ty - 1) ? ty * TS : Math.floor((b.y - b.vy * dt) / TS) * TS + TS; if (b.vy > 0) fires.push({ x: b.x, y: fy, life: 1.2, delay: 0 }); burst(b.x, b.y, 6, ['#ff9a5c', '#ffd36b'], 50, 0.35, 100, 1); SFX.crack(); }
  }
...
function updateProps(dt) {
  ...
  for (const z of (L.gusts || [])) { if (z.arena && !bossActive) continue; const ph = (time + (z.phase || 0)) % z.period, on = ph < z.on, soon = ph > z.period - 0.5; const zd = z.alt ? (Math.floor((time + (z.phase || 0)) / z.period) % 2 ? -z.dir : z.dir) : z.dir; if (P.x > z.x0 && P.x < z.x1 && P.y > z.y0 && P.y <= z.y1 + 4) { if (on && !P.dead) { P.vx += zd * (P.ground ? 200 : 260) * dt; if (Math.random() < dt * 40) parts.push({ x: camX + Math.random() * VW, y: z.y0 + Math.random() * (z.y1 - z.y0), vx: zd * 220, vy: 0, life: 0.35, max: 0.35, col: '#dfe8c0', size: 1, grav: 0 }); } else if (soon && Math.random() < dt * 12) parts.push({ x: camX + Math.random() * VW, y: z.y0 + Math.random() * (z.y1 - z.y0), vx: zd * 90, vy: 0, life: 0.4, max: 0.4, col: '#c9d1a0', size: 1, grav: 0 }); } }
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'hammer' && pr.drop > 0) { pr.drop -= dt; if (pr.drop <= 0 && !P.dead && Math.abs(P.x - pr.x) < 18 && Math.abs(P.y - pr.y) < 16 && P.ground) damagePlayer(pr.x, DMG.hammer, { unblockable: true, up: true }); if (pr.drop <= 0) { shakeCam(6); SFX.heavy(); dust(pr.x, pr.y, 12); for (const dd of [-1, 1]) waves.push({ x: pr.x + dd * 18, y: pr.y, dir: dd, life: 1.1, sp: 160 }); } }
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'exit' && !P.dead && state === 'play' && upPress && Math.abs(P.x - pr.x) < 14) { state = 'map'; map.node = Math.max(0, NODES.findIndex(n => n.kind === 'store' && (n.shop || 'shop') === LEVELS[levelIndex].id)); map.seg = NODE_AT[map.node]; map.t = 0; setView('normal'); SFX.uiSel(); music.play(menuTrack()); }
    ...
    if (pr.t === 'stray' && !pr.got && !P.dead && Math.abs(pr.x - P.x) < 12 && Math.abs(pr.y - P.y) < 16) { const Q = questOf(); pr.got = true; straysGot.add(pr.x); strayLast = { x: pr.x, y: pr.y }; if (pr.kind === 'sheep') SFX.bleat(); else if (pr.kind === 'canary') { SFX.bird(); SFX.sting(); } else { SFX.coin(); SFX.sting(); } burst(pr.x, pr.y - 4, 8, ['#e8e0d0', '#fff6c8'], 50, 0.5); number(pr.x, pr.y - 18, Q.name + ' ' + straysGot.size + '/' + Q.n, '#ffe6a0'); if (straysGot.size >= Q.n) questDone(pr.x, pr.y); }
 ...
 if (Math.random() < dt * 12) parts.push({ x: c.x + (Math.random() - 0.5) * c.r * 1.6, y: c.y + (Math.random() - 0.5) * c.r, vx: (Math.random() - 0.5) * 10, vy: -8, life: 0.8, max: 0.8, col: c.sleep ? '#c9a0ff' : '#d8d0c8', size: 1, grav: 0 }); }
  ...
  for (const f of fires) { if (f.delay > 0) { f.delay -= dt; continue; } f.life -= dt; if (!P.dead && Math.abs(P.x - f.x) < 9 && P.y > f.y - 14 && P.y <= f.y + 2) damagePlayer(f.x, DMG.fire, { up: true, unblockable: true }); for (const e of enemies) if (e.alive && e.t !== 'chief' && e.t !== 'wasp' && e.t !== 'king' && e.t !== 'master' && Math.abs(e.x - f.x) < 9 && Math.abs(e.y - f.y) < 6 && !(e.fireT > 0)) { e.fireT = 0.6; hurtEnemy(e, 10, f.x, false); } }
...
function updatePwaves(dt) {
  for (const w of pwaves) {
    ...
    for (const e of enemies) { if (!e.alive || w.hit.has(e) || e.harmless || Math.abs(e.x - w.x) > 11 || Math.abs(e.y - w.y) > 14) continue; w.hit.add(e); if (e.t === 'mother' || e.t === 'king' || e.t === 'gill' || e.t === 'heart' || e.t === 'wasp' || e.t === 'drone') continue; hurtEnemy(e, 15, w.x - w.dir * 20, false); e.stagger = Math.max(e.stagger, 0.8); number(e.x, e.y - e.h - 14, 'QUAKED', '#ffd36b'); }
  }
...
function updateThrown(dt) {
  ...
  else { const dx = P.x - s.x, dy = (P.y - 9) - s.y, d = Math.hypot(dx, dy) || 1; if (d < 10 || s.t > 3) { thrown = null; P.throwCd = 2.5; SFX.shieldCatch(); impactAt(P.x + P.face * 6, P.y - 9, 'steel'); return; } const sp = 300 + s.t * 60; s.x += dx / d * sp * dt; s.y += dy / d * sp * dt; }
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const m of movers) {
    ...
    else if (m.kind === 'swing') { g.strokeStyle = '#c9b27c'; g.lineWidth = 1; g.beginPath(); g.moveTo(Math.round(m.px - cx) + 0.5, Math.round(m.py - cy)); g.lineTo(Math.round(m.x - cx) + 2.5, Math.round(m.y - cy)); g.moveTo(Math.round(m.px - cx) + 0.5, Math.round(m.py - cy)); g.lineTo(Math.round(m.x + m.w - cx) - 2.5, Math.round(m.y - cy)); g.stroke(); g.fillStyle = '#5c3a1d'; g.fillRect(Math.round(m.px - cx) - 3, Math.round(m.py - cy) - 3, 6, 4); const n = m.w / TS; for (let i = 0; i < n; i++) g.drawImage(i === 0 ? TILE.logL : i === n - 1 ? TILE.logR : TILE.log[i % 3], Math.round(m.x) + i * TS - cx, Math.round(m.y) - cy); }
  ...
  for (const pr of props) {
    ...
    else if (pr.t === 'relic' && !pr.got) { const R = RELICS[pr.kind], yy = Math.round(pr.y - 8 + Math.sin(time * 3 + pr.ph) * 2 - cy), xx = Math.round(pr.x - cx); g.globalAlpha = 0.35 + 0.15 * Math.sin(time * 4); g.fillStyle = R.col; g.beginPath(); g.arc(xx, yy, 10, 0, 7); g.fill(); g.globalAlpha = 1; g.drawImage(PROP.relic[pr.kind], xx - 5, yy - 6); }
  ...
  for (const e of enemies) {
    ...
    if (e.t === 'king' && e.crown) { g.save(); g.translate(Math.round(e.crown.x - cx), Math.round(e.crown.y - cy)); g.rotate(e.crown.t * 14); g.drawImage(PROP.relic.crown, -5, -6); g.restore(); }
    if (e.t === 'king') { const fl = L.arena.floor; if (e.phase < 3) { g.drawImage(PROP.palanquin, Math.round(e.x) - 24 - cx, Math.round(e.y) - 26 - cy); const fr = e.throwT > 0 ? 1 : (e.mode === 'shout' || e.mode === 'shoutTell') ? 2 : 0; drawSet(SPR.king, null, fr, e.x - cx, e.y - 6 - cy, e.face, e.flash > 0, 1, 1); } else { const fr = e.mode === 'slam' || e.mode === 'slamTell' ? 1 : 0; drawSet(SPR.kingUp, null, fr, e.x - cx + (e.mode === 'slamTell' ? Math.round(Math.sin(e.anim * 60)) : 0), e.y - cy, e.face, e.flash > 0 || (e.mode === 'slamTell' && Math.floor(e.anim * 12) % 2 === 0), 1, 1); } if (e.mode === 'shoutTell' || e.mode === 'slamTell' || e.mode === 'sweepTell') text('!', e.x - cx, e.y - (e.phase < 3 ? 40 : 34) - cy, '#ffd36b', 'center'); continue; }
    ...
    else if (e.t === 'forgemaster') frame = e.mode === 'scald' ? 4 : e.mode === 'kickTell' || e.mode === 'kick' ? 3 : e.mode === 'sprayTell' || e.mode === 'spray' ? 2 : e.mode === 'lunge' || e.mode === 'lungeTell' ? 1 : 0;
    ...
    else if (e.t === 'spider') frame = e.mode === 'drop' ? 1 : 0;
    else if (e.squirrel) frame = e.vy !== 0 ? 2 : Math.floor(e.anim * 12) % 2;
    else if (e.t === 'owl') frame = e.mode === 'crash' ? 4 : e.mode === 'screech' || e.mode === 'screechTell' ? 3 : e.mode === 'perch' || e.mode === 'sleep' || e.mode === 'wake' ? 0 : 1 + Math.floor(e.anim * 8) % 2;
    ...
    else if (e.t === 'ram') frame = e.mode === 'lower' || e.mode === 'buttTell' || e.mode === 'callTell' ? 3 : e.mode === 'crash' ? 4 : e.mode === 'tossTell' || e.mode === 'toss' || e.mode === 'call' ? 5 : e.mode === 'stampTell' || e.mode === 'stamp' || e.mode === 'butt' ? 5 : Math.abs(e.vx) > 4 ? 1 + Math.floor(e.anim * (e.mode === 'charge' ? 16 : 8)) % 2 : 0;
    ...
    const wind = (e.t === 'thorn' && e.mode === 'wind') || (e.t === 'queen' && e.mode === 'aim') || ((e.t === 'brute' || e.t === 'chief') && (e.mode === 'raise' || e.mode === 'wind' || e.mode === 'slashWind' || e.mode === 'bashWind' || e.mode === 'crouch' || e.mode === 'whirlWind' || e.mode === 'rainAim')) || (e.t === 'pike' && e.mode === 'tell') || (e.t === 'ram' && (e.mode === 'lower' || e.mode === 'stampTell' || e.mode === 'buttTell'));
  ...
  for (const b of bolts) { const x = Math.round(b.x - cx), y = Math.round(b.y - cy), k = b.life / 0.28; g.globalAlpha = k; g.fillStyle = '#dfffa0'; g.fillRect(x - 1, y - 200, 3, 200); g.fillStyle = '#8fd160'; g.fillRect(x - 3, y - 200, 7, 200); g.globalAlpha = 1; g.fillStyle = '#fff6e0'; g.fillRect(x - 1, y - 200, 2, 200); }
  ...
  if (L.dark) { // the mine: black, with holes for every lamp, fire and the light you carry
    if (!DARKC || DARKC.width !== VW || DARKC.height !== VH) { DARKC = document.createElement('canvas'); DARKC.width = VW; DARKC.height = VH; }
    const dg = DARKC.getContext('2d'); dg.globalCompositeOperation = 'source-over'; dg.clearRect(0, 0, VW, VH); dg.fillStyle = 'rgba(4,4,10,' + L.dark + ')'; dg.fillRect(0, 0, VW, VH); dg.globalCompositeOperation = 'destination-out';
    ...
    for (const lt of lights) if (lt.r > 0 && lt.x > cx - 120 && lt.x < cx + VW + 120 && !(lt.ref && lt.ref.dark > 0) && !(lt.lantern && !lt.lantern.lit)) hole(lt.x - cx, lt.y - cy, lt.r * 1.3);
...
function drawMother(cx, cy) {
  ...
  const breathe = 1 + Math.sin(time * 1.2) * 0.02, capY = fy - 96 + Math.round(Math.sin(time * 1.2) * 2);
...
function drawControls() {
  ...
  const rows = [['move', 'ARROWS / WASD', 'STICK'], ['jump', SET.swapZX ? 'X / SPACE' : 'Z / SPACE', 'A'], ['swing', SET.swapZX ? 'Z / J' : 'X / J', 'X'], ['plunge', 'DOWN+SWING IN AIR', 'DOWN+X'], ['block', 'C / L ' + (SET.blockToggle ? 'TOGGLE' : 'HOLD'), 'LB RB'], ['dodge', 'V / SHIFT', 'B'], ['skill', 'F / B (equipped)', 'Y'], ['pause', 'ESC / P', 'START'], ['drop', 'DOWN ON A LEDGE', 'DOWN'], ['shrine', 'R (RETURN)', '']];
...
function desiredView() { const inLevel = state === 'play' || state === 'win' || state === 'gameover' || (state === 'menu' && menuFrom === 'play'); if (!inLevel) return 'normal'; return (SET.zoom === 'wide' || (bossActive && boss && (boss.t === 'mother' || boss.t === 'owl' || boss.t === 'forgemaster'))) ? 'zoom' : 'normal'; }
function render() {
  setView(desiredView());
  ...
  const talkers = signs.concat(props.filter(pr => pr.t === 'npc').map(pr => { const ls = NPC_LINES(pr); return { x: pr.x, text: ls[Math.floor(time / 2.6) % ls.length] }; }), props.filter(pr => pr.t === 'cage' && pr.kind === 'squire' && !pr.open).map(pr => ({ x: pr.x, text: 'KNIGHT! BREAK THE BARS!' })));
  for (const s of talkers) if (state === 'play' && Math.abs(s.x - P.x) < 28) {
  ...
  if (state === 'play' || state === 'win' || state === 'gameover' || (state === 'menu' && menuFrom === 'play')) {
    ...
    if (SET.bossIntro && ((bossActive && boss && boss.mode === 'wake') || miniIntroT > 0)) { const k = Math.min(1, (1.6 - (miniIntroT > 0 ? miniIntroT : boss.modeT)) / 0.35), bh = Math.round(VH * 0.11 * k); g.fillStyle = '#0a0810'; g.fillRect(0, 0, VW, bh); g.fillRect(0, VH - bh, VW, bh); const nm = miniIntroT > 0 ? 'THE GREAT HOUND' : boss.t === 'owl' ? 'THE OWL REEVE' : boss.t === 'forgemaster' ? 'THE FORGEMASTER' : boss.t === 'frog' ? 'THE BULLFROG KING' : boss.t === 'chief' ? 'THE GOBLIN CHIEFTAIN' : boss.t === 'mother' ? 'THE MOTHER CAP' : boss.t === 'king' ? 'KING GORM UNDERLEAF' : boss.t === 'ram' ? 'THE RAM LORD' : 'THE HORNET QUEEN'; if (k >= 1) { text(nm, VW / 2 + 1, VH / 2 - 5, '#3a2214', 'center', 12); text(nm, VW / 2, VH / 2 - 6, '#ffd36b', 'center', 12); } }
    ...
    { const sk = skillNow(); if (sk) { const slam = sk === 'groundSlam'; const cd = slam ? (P.slamCd || 0) : (P.throwCd || 0), max = slam ? 3 : 2.5; g.globalAlpha = (!slam && thrown) || cd > 0 ? 0.45 : 1; g.drawImage(slam ? SLAM_ICON : SHIELD_ICON, 76, 13); g.globalAlpha = 1; if (cd > 0) { g.fillStyle = '#c9d1dc'; g.fillRect(76, 26, Math.round(10 * (1 - cd / max)), 1); } } }
