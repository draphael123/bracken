...
function bakeAll(pal = {}) {
  ...
  PROP.relic.fleece = ART.bakeFleeceIcon(); PROP.relic.windcloak = (() => { const [c, g] = canvas(10, 12); g.fillStyle = '#bfe6f5'; g.beginPath(); g.moveTo(5, 0); g.lineTo(9, 3); g.lineTo(9, 11); g.lineTo(5, 9); g.lineTo(1, 11); g.lineTo(1, 3); g.closePath(); g.fill(); g.fillStyle = '#7aa8c8'; g.fillRect(4, 1, 2, 8); g.fillStyle = '#ffd36b'; g.fillRect(4, 0, 2, 1); return c; })();
...
const MEDALS = { moor: [480, 660, 960], mineworks: [540, 720, 1000], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
function spawnEnt(e) {
  ...
  const n0 = enemies.length;
  {
    ...
    switch (e.t) {
      ...
      case 'forgemaster': boss = { ...base, t: 'forgemaster', w: 40, h: 32, hp: EHP.forgemaster, maxHp: EHP.forgemaster, mode: 'sleep', modeT: 0, face: -1, phase: 1, hitT: 0, slamT: 3, sprayT: 6, dragT: 7, anvilT: 10, breathT: 5, hammerT: 2.6, cartT: 5, slagT: 6, plateT: 4 }; enemies.push(boss); break;
...
const CRAG_NODES = [
  ...
  { id: 'mineworks', kind: 'level', level: 7, x: 232, y: 118, name: 'THE GLASSWORKS' },
  { id: 'moor', kind: 'level', level: 8, x: 160, y: 150, name: 'GALE MOOR' },
];
const CRAG_PATH = [[36, 128], [62, 120], [92, 104], [120, 92], [150, 84], [184, 66], [214, 58], [250, 50], [280, 36], [270, 72], [252, 100], [232, 118], [210, 134], [184, 146], [160, 150]];
...
const NODE_AT = [0, 3, 6, 8, 10, 12, 20, 23, 25, 28, 31]; // PATH index of each node: wood 0-5, then scree, the village, the high store
...
const BEASTS = [
  ...
  { t: 'windcaller', name: 'THE WINDCALLER', sub: 'lord of the moor', desc: 'A goblin shaman on the highest stone, his war-kite over him on four lines. He raises the wind to sweep you onto the spikes, lifts stones off the floor to drop on you, and draws breath so that everything in the air falls, then blasts. He only bleeds when a cut line pulls him off his feet: cut a tether, then cut him. Cut all four and the kite drags him down to fight on foot with a staff and a shove.' },
  ...
  { t: 'golem', name: 'THE FACET', sub: 'the heart of the glassworks', desc: 'A golem grown from the cavern crystal. Dark, it turns every blade. Put a beam on it and it bleeds: turn the two mirrors so the light crosses the floor, and cut it while it stands lit, facet by facet. Every facet that shatters staggers it and makes it faster. It stamps, and a stamp turns any mirror it stands by. It throws shards. Lit too long, it raises ice against the beam: the beam melts that too.' },
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster', 'golem', 'windcaller'];
...
function hurtEnemy(e, dmg, fromX, plunge) {
  ...
  if (e.t === 'windcaller' && !callerOpen(e)) { SFX.clank(); sparks(e.x + (Math.sign(fromX - e.x) || 1) * 8, e.y - 12, Math.sign(e.x - fromX) || 1, 3); P.vx = (Math.sign(P.x - e.x) || 1) * 120; return; } // the wind turns the blade until a line pulls him down
  if (e.t === 'windcaller' && e.mode === 'yanked') dmg *= 2;
  if (e.t === 'kite' && e.mode !== 'fall') { e.mode = 'fall'; e.vy = -40; e.vx = (Math.sign(e.x - fromX) || 1) * 60; number(e.x, e.y - 40, 'THE STRING', '#ffd36b'); SFX.crack(); }
  if (e.t === 'golem' && !e.lit) { SFX.clank(); sparks(e.x + (Math.sign(fromX - e.x) || 1) * 12, e.y - 18, Math.sign(e.x - fromX) || 1, 4); return; } // dark crystal turns the blade; lit, it bleeds
  ...
  if (e.hp <= 0) {
    ...
    if (e.t === 'chief') chiefDies(); else if (e.t === 'queen' || e.t === 'frog' || e.t === 'king' || e.t === 'ram' || e.t === 'owl' || e.t === 'forgemaster' || e.t === 'golem' || e.t === 'windcaller') queenDies();
...
function updatePlayer(dt) {
  ...
  const cap = (P.block ? 32 : wading ? 46 : spored ? 40 : RUN * (PROG.charm === 'swift' ? 1.12 : 1)) + (P.gustT > 0 && !P.ground ? 150 : 0); // a gust can carry you faster than your legs
  if (move && !groundAtk) {
...
function updateWindcaller(e, dt) {
  // On the highest stone under his war-kite. He raises the wind (a sweep to the spikes), lifts stones and drops them on you, and draws breath so
  // everything falls, then blasts. He only bleeds when a cut line pulls him off his feet. Cut all four and the kite drags him down to fight on foot.
  const A = L.arena, floor = A.floor, p2 = e.phase === 2; e.modeT -= dt; e.anim = (e.anim || 0) + dt; e.hitT = Math.max(0, e.hitT - dt);
  const d = P.x - e.x, ad = Math.abs(d);
  if (e.mode === 'sleep') { e.x = e.sx; e.y = e.sy; return; }
  e.wind = 0; let want = 0;
  if (!p2) {
    e.x = e.sx; if (e.mode !== 'torn') e.y = e.sy;
    switch (e.mode) {
  ...
  // on foot: no wind left in him but a staff and a shove
  e.vy += 1000 * dt; if (e.vy > 300) e.vy = 300;
  switch (e.mode) {
    case 'land': want = 0; if (e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.4; } break;
    case 'walk': e.face = Math.sign(d) || e.face; want = ad > 22 ? e.face * 58 : 0; e.staffT = (e.staffT || 0) - dt; e.shoveT = (e.shoveT || 3) - dt;
      if (e.modeT <= 0 && !P.dead) { if (ad < 30 && e.staffT <= 0) { e.staffT = 1.6; e.mode = 'staffTell'; e.modeT = 0.45; number(e.x, e.y - 30, '!', '#ffd36b'); SFX.buzz(); } else if (e.shoveT <= 0 && ad < 110) { e.shoveT = 6; e.mode = 'shoveTell'; e.modeT = 0.6; number(e.x, e.y - 30, '!!', '#bfe6f5'); SFX.charge(); } } break;
    case 'staffTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'staff'; e.modeT = 0.3; SFX.slash(); if (!P.dead && Math.sign(P.x - e.x) === e.face && ad < 32 && Math.abs(P.y - e.y) < 24) { const res = damagePlayer(e.x, DMG.staff); if (res === 'blocked') { e.stagger = 1.0; number(e.x, e.y - 30, 'PARRIED', '#8fd160'); } } } break;
    case 'staff': if (e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.5; } break;
    case 'shoveTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'shove'; e.modeT = 0.8; SFX.buzz(); } break;
    case 'shove': e.wind = e.face; if (!P.dead && Math.sign(P.x - e.x) === e.face && ad < 120 && Math.abs(P.y - e.y) < 40) { P.vx += e.face * 320 * dt; if (Math.random() < dt * 40) parts.push({ x: e.x + e.face * (10 + Math.random() * 100), y: e.y - 20 + (Math.random() - 0.5) * 30, vx: e.face * 240, vy: 0, life: 0.3, max: 0.3, col: '#e8f0f8', size: 1, grav: 0 }); } if (e.modeT <= 0) { e.mode = 'walk'; e.modeT = 0.6; } break;
  }
  if (e.stagger > 0) want = 0;
  e.vx += (want - e.vx) * Math.min(1, dt * 6);
  const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) e.vy = 0;
  e.x = Math.max(A.x0 + 40, Math.min(A.x1 - 16, e.x));
}
const callerOpen = e => e.phase === 2 || e.mode === 'yanked' || e.mode === 'torn';
...
function updateForgemaster(e, dt) {
  ...
  // for a fire breath, the floor plates glow and burn.
  ...
  if (e.mode !== 'wake' && e.mode !== 'stun') {
    ...
    if (p2 && !e.opened) { e.opened = true; L.dark = Math.min(L.dark, 0.5); number(e.x, e.y - e.h - 12, 'THE FURNACE OPENS', '#ff6b2c'); SFX.roar(); shakeCam(6); for (const dx of [-90, 90]) enemies.push({ t: 'bat', x: e.x + dx, y: floor - 90, hx: e.x + dx, hy: floor - 90, w: 10, h: 6, hp: EHP.bat, hp0: EHP.bat, mode: 'hang', modeT: 0, cd: 0.5, face: -1, alive: true, dying: 0, anim: Math.random(), flash: 0, stagger: 0, vx: 0, vy: 0 }); }
  }
  let want = 0;
...
function traceBeams(dt) {
  ...
  for (const cr of props) { if (cr.t !== 'crystal') continue;
    let x = cr.x, y = cr.y - 8, [dx, dy] = cr.dir; let x0 = x, y0 = y;
    for (let n = 0; n < 60; n++) {
...
function drawBeams(cx, cy) {
  for (const b of beams) { const x0 = Math.round(b.x0 - cx), y0 = Math.round(b.y0 - cy), x1 = Math.round(b.x1 - cx), y1 = Math.round(b.y1 - cy); g.globalAlpha = 0.22 + 0.08 * Math.sin(time * 9); g.strokeStyle = '#bfe6f5'; g.lineWidth = 7; g.beginPath(); g.moveTo(x0, y0); g.lineTo(x1, y1); g.stroke(); g.globalAlpha = 0.9; g.strokeStyle = '#eefaff'; g.lineWidth = 2; g.beginPath(); g.moveTo(x0, y0); g.lineTo(x1, y1); g.stroke(); g.globalAlpha = 1; if (Math.random() < 0.3) { const t = Math.random(); parts.push({ x: b.x0 + (b.x1 - b.x0) * t, y: b.y0 + (b.y1 - b.y0) * t, vx: (Math.random() - 0.5) * 16, vy: (Math.random() - 0.5) * 16, life: 0.3, max: 0.3, col: '#eefaff', size: 1, grav: 0 }); } }
}
...
const TAM_LINES = {
  ...
  mineworks: [['THE MINERS BROKE INTO SOMETHING DOWN THERE. LIGHT, THE FOREMAN SAYS. AND A THING THAT GREW IN IT.', 'THE CRYSTALS THROW BEAMS. THE FOREMAN SAYS STRIKE THE MIRRORS.'],
               ['THE HEART IS SHATTERED. THE MOUNTAIN GOES HIGHER STILL.', 'WE WILL SEE THE TOP OF IT, YOU AND I.']],
...
const TAM_MAP = {
  ...
  hanging: ['THE REEVE IS AN OWL. I AM NOT.', 'THEY WENT UNDER THE MOUNTAIN.'], mineworks: ['THE MINE BROKE INTO LIGHT.', 'THE HEART IS SHATTERED. HIGHER STILL.'], moor: ['THE WIND OWNS THE MOOR.', 'THE WIND IS QUIET. THE SUMMIT IS NEXT.'],
};
const NPC_LINES = pr => {
...
function updateProps(dt) {
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'mirror') { pr.turnT = Math.max(0, pr.turnT - dt); pr.glow = Math.max(0, (pr.glow || 0) - dt); if (hb && !P.hitSet.has(pr) && overlap(hb, { l: pr.x - 7, r: pr.x + 7, t: pr.y - 16, b: pr.y })) { P.hitSet.add(pr); pr.o = 1 - pr.o; pr.turnT = 0.25; SFX.clank(); SFX.spark(); sparks(pr.x, pr.y - 8, P.face, 5); number(pr.x, pr.y - 22, 'TURNED', '#bfe6f5'); } }
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'hotplate') { if (pr.glow > 0) { pr.glow -= dt; if (Math.random() < dt * 10) parts.push({ x: pr.x + (Math.random() - 0.5) * 24, y: pr.y - 1, vx: 0, vy: -20, life: 0.4, max: 0.4, col: '#ff6b2c', size: 1, grav: 0 }); if (pr.glow <= 0) { pr.hot = 3; SFX.hiss(); burst(pr.x, pr.y - 2, 8, ['#ff6b2c', '#ffd36b'], 40, 0.4, -60, 1); } }
      if (pr.hot > 0) { pr.hot -= dt; if (Math.random() < dt * 20) parts.push({ x: pr.x + (Math.random() - 0.5) * 28, y: pr.y - 2, vx: 0, vy: -50, life: 0.3, max: 0.3, col: Math.random() < 0.5 ? '#ff6b2c' : '#ffd36b', size: 2, grav: 0 }); if (!P.dead && P.ground && Math.abs(P.x - pr.x) < 16 && Math.abs(P.y - pr.y) < 4 && !(pr.cd > 0)) { pr.cd = 0.5; if (isPyro() && PROG.items.kindle) { if (P.hp < P.maxHp) { P.hp = Math.min(P.maxHp, P.hp + 1); } } else damagePlayer(pr.x, DMG.hotplate, { unblockable: true, up: true }); } } pr.cd = Math.max(0, (pr.cd || 0) - dt); }
    ...
    if (pr.t === 'vent') { // an updraft of spores on a timer: ride it up
      ...
      if (pr.active) {
        if (Math.random() < dt * 40) parts.push({ x: pr.x + (Math.random() - 0.5) * 16, y: pr.y - 2, vx: (Math.random() - 0.5) * 12, vy: -140 - Math.random() * 80, life: pr.h / 180, max: pr.h / 180, col: pr.wind ? (Math.random() < 0.5 ? '#eefaff' : '#c8d8e8') : Math.random() < 0.5 ? '#c8bcb0' : '#9a5aa8', size: Math.random() < 0.3 ? 2 : 1, grav: 0 });
...
function update(dt) {
  ...
  updateMovers(wdt); updatePlayer(wdt); updateEnemies(wdt); updateWisp(wdt); updateSlide(wdt); updateFlood(wdt); traceBeams(wdt); updateProps(wdt); updateCorpses(wdt); updateParticles(wdt); updateWeather(dt); updateCamera(dt);
  updatePolish(dt);
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const pr of props) {
    ...
    else if (pr.t === 'tether') { const x = Math.round(pr.x - cx), y = Math.round(pr.y - cy); g.fillStyle = '#5c3a1d'; g.fillRect(x - 2, y - 10, 4, 10); g.fillStyle = '#8b6a2a'; g.fillRect(x - 3, y - 12, 6, 3); if (!pr.cut) { g.fillStyle = '#c9d1dc'; g.fillRect(x - 1, y - 14, 2, 2); } else { g.fillStyle = '#3a2416'; g.fillRect(x - 3, y - 4, 6, 2); } }
    ...
    else if (pr.t === 'mirror') { const x = Math.round(pr.x - cx), y = Math.round(pr.y - cy); g.fillStyle = '#5c3a1d'; g.fillRect(x - 1, y - 8, 2, 8); g.fillRect(x - 4, y - 1, 8, 1); g.fillStyle = '#3a3a44'; g.fillRect(x - 6, y - 14, 12, 8); g.strokeStyle = pr.glow > 0 ? '#eefaff' : '#c9d1dc'; g.lineWidth = 2; g.beginPath(); if (pr.o === 0) { g.moveTo(x - 5, y - 6); g.lineTo(x + 5, y - 15); } else { g.moveTo(x - 5, y - 15); g.lineTo(x + 5, y - 6); } g.stroke(); if (pr.turnT > 0) { g.globalAlpha = pr.turnT * 3; g.strokeStyle = '#bfe6f5'; g.beginPath(); g.arc(x, y - 10, 10, 0, 7); g.stroke(); g.globalAlpha = 1; } }
    else if (pr.t === 'receiver') { const x = Math.round(pr.x - cx), y = Math.round(pr.y - cy); g.fillStyle = '#5a6270'; g.fillRect(x - 5, y - 12, 10, 12); g.fillStyle = pr.opened ? '#8fd160' : pr.lit ? '#eefaff' : '#3a4a60'; g.beginPath(); g.arc(x, y - 7, 3, 0, 7); g.fill(); if (pr.lit && !pr.opened) { g.fillStyle = '#bfe6f5'; g.fillRect(x - 5, y - 14, Math.round(10 * Math.min(1, pr.litT / 1.2)), 1); } }
    ...
    else if (pr.t === 'vent' && pr.wind) { const x = Math.round(pr.x - cx), y = Math.round(pr.y - cy); g.fillStyle = '#5a5a66'; g.fillRect(x - 9, y - 3, 18, 3); g.fillStyle = '#8a8a98'; g.fillRect(x - 7, y - 5, 14, 2); g.fillStyle = '#3a3a44'; g.fillRect(x - 5, y - 4, 10, 1); if (pr.active) { g.globalAlpha = 0.3; g.strokeStyle = '#eefaff'; g.lineWidth = 1; for (let k = 0; k < 3; k++) { const yy = y - 8 - ((time * 90 + k * 30) % pr.h); g.beginPath(); g.moveTo(x - 6 + k * 5, yy); g.lineTo(x - 6 + k * 5, yy - 10); g.stroke(); } g.globalAlpha = 1; } }
  ...
  for (const e of enemies) {
    ...
    else if (e.t === 'windcaller') frame = /Tell$|sweep|lift|still|blast|shove/.test(e.mode) ? 1 : 0;
    else if (e.t === 'golem') frame = e.mode === 'stagger' ? 5 : e.mode === 'stompTell' || e.mode === 'stomp' || e.mode === 'shroudTell' || e.mode === 'shroud' ? 3 : e.mode === 'throwTell' || e.mode === 'throw' ? 4 : Math.abs(e.vx) > 4 ? 1 + Math.floor(e.anim * 5) % 2 : 0;
    ...
    if (e.t === 'windcaller' && e.alive && e.phase === 1 && e.mode !== 'sleep') { const kx = Math.round((e.mode === 'torn' ? e.x : e.sx) - cx) + Math.round(Math.sin(time * 1.3) * 10), ky = Math.round(e.sy - 120 - cy) + Math.round(Math.sin(time * 2.1) * 6); for (const pr of props) if (pr.t === 'tether' && !pr.cut) { g.strokeStyle = 'rgba(232,220,192,0.8)'; g.lineWidth = 1; g.beginPath(); g.moveTo(Math.round(pr.x - cx) + 0.5, Math.round(pr.y - 14 - cy)); g.lineTo(kx, ky + 14); g.stroke(); } g.strokeStyle = '#e8dcc0'; g.beginPath(); g.moveTo(Math.round(e.x - cx) + 0.5, Math.round(e.y - 28 - cy)); g.lineTo(kx, ky + 14); g.stroke(); g.fillStyle = '#c9463d'; g.beginPath(); g.moveTo(kx, ky - 22); g.lineTo(kx + 18, ky); g.lineTo(kx, ky + 14); g.lineTo(kx - 18, ky); g.closePath(); g.fill(); g.fillStyle = '#ffd36b'; g.beginPath(); g.moveTo(kx, ky - 14); g.lineTo(kx + 10, ky); g.lineTo(kx, ky + 8); g.lineTo(kx - 10, ky); g.closePath(); g.fill(); g.fillStyle = '#2a2230'; g.fillRect(kx, ky - 22, 1, 36); g.fillRect(kx - 18, ky, 36, 1); for (let k = 1; k < 6; k++) { g.fillStyle = k % 2 ? '#c9463d' : '#e8dcc0'; g.fillRect(kx - 1 + Math.round(Math.sin(time * 5 + k) * 3), ky + 14 + k * 5, 3, 3); } }
    if (e.t === 'windcaller' && e.alive && callerOpen(e) && e.phase === 1) { const k = 0.5 + 0.5 * Math.sin(time * 10); g.globalAlpha = 0.35 + 0.35 * k; g.strokeStyle = '#8fd160'; g.lineWidth = 2; g.beginPath(); g.ellipse(Math.round(e.x - cx), Math.round(e.y - cy) - 2, 18 + k * 3, 5, 0, 0, Math.PI * 2); g.stroke(); g.globalAlpha = 1; }
    if (e.t === 'golem' && e.alive) { const gx = Math.round(e.x - cx), gy = Math.round(e.y - cy); const gems = [[0, -34], [-11, -25], [11, -25], [0, -18]]; gems.forEach(([ox, oy], k) => { const broken = k < (e.facets || 0); g.fillStyle = broken ? '#2a3a50' : e.lit ? (Math.floor(time * 10) % 2 ? '#fff6e0' : '#ff7ab8') : '#a8306a'; g.fillRect(gx + ox - 2, gy + oy - 2, 4, 4); if (!broken && e.lit) { g.globalAlpha = 0.35; g.fillStyle = '#ff7ab8'; g.beginPath(); g.arc(gx + ox, gy + oy, 7, 0, 7); g.fill(); g.globalAlpha = 1; } }); if (e.lit) { g.globalAlpha = 0.18; g.fillStyle = '#eefaff'; g.fillRect(gx - 18, gy - e.h - 2, 36, e.h + 2); g.globalAlpha = 1; } }
    ...
    const bigF = e.t === 'frog' ? 1.35 : e.t === 'ram' ? 1.5 : e.t === 'windcaller' ? 1.5 : 1; const sq = e.sq > 0 ? e.sq / 0.16 : 0;
  ...
  for (const s of seeds) { if (s.arrow) { const a = Math.atan2(s.vy, s.vx); g.save(); g.translate(Math.round(s.x - cx), Math.round(s.y - cy)); g.rotate(a); g.fillStyle = '#e8dcc0'; g.fillRect(-5, -1, 8, 1); g.fillStyle = '#c9d1dc'; g.fillRect(3, -1, 3, 2); g.fillStyle = s.reflected ? '#8fd160' : '#c9463d'; g.fillRect(-6, -2, 2, 3); g.restore(); } else if (s.shard) { g.save(); g.translate(Math.round(s.x - cx), Math.round(s.y - cy)); g.rotate(Math.atan2(s.vy, s.vx)); g.fillStyle = '#bfe6f5'; g.fillRect(-5, -2, 10, 4); g.fillStyle = '#eefaff'; g.fillRect(-4, -1, 6, 1); g.restore(); } else if (s.acid) { const x = Math.round(s.x - cx), y = Math.round(s.y - cy); g.fillStyle = '#b8d878'; g.fillRect(x - 2, y - 2, 4, 4); g.fillStyle = '#e8ff9a'; g.fillRect(x - 1, y - 1, 2, 2); } else if (s.lantern) { const x = Math.round(s.x - cx), y = Math.round(s.y - cy); g.fillStyle = '#5a6270'; g.fillRect(x - 2, y - 4, 4, 6); g.fillStyle = '#ffd36b'; g.fillRect(x - 1, y - 3, 2, 3); g.fillStyle = '#8a919c'; g.fillRect(x - 1, y - 5, 2, 1); } else if (s.skull) { g.save(); g.translate(Math.round(s.x - cx), Math.round(s.y - cy)); g.rotate(s.life * 6); g.fillStyle = '#e8e0d0'; g.fillRect(-4, -4, 8, 7); g.fillRect(-3, 3, 6, 2); g.fillStyle = '#2a2230'; g.fillRect(-3, -2, 2, 2); g.fillRect(1, -2, 2, 2); g.fillRect(-1, 3, 1, 2); g.fillRect(1, 3, 1, 2); g.restore(); } else if (s.goblet) { g.save(); g.translate(Math.round(s.x - cx), Math.round(s.y - cy)); g.rotate(s.life * 9); g.fillStyle = '#e0b040'; g.fillRect(-3, -3, 6, 4); g.fillRect(-1, 1, 2, 3); g.fillStyle = '#fff6c8'; g.fillRect(-2, -3, 2, 1); g.restore(); } else if (s.pod) { const x = Math.round(s.x - cx), y = Math.round(s.y - cy); g.fillStyle = '#6a3a7a'; g.fillRect(x - 4, y - 5, 8, 8); g.fillStyle = '#9a5aa8'; g.fillRect(x - 3, y - 4, 6, 6); g.fillStyle = '#e0b0f0'; g.fillRect(x - 2, y - 4, 2, 2); g.fillStyle = '#3f6e2c'; g.fillRect(x - 1, y - 7, 2, 2); } else if (s.spore) { g.fillStyle = '#9a5aa8'; g.fillRect(Math.round(s.x - cx) - 2, Math.round(s.y - cy) - 3, 5, 5); g.fillStyle = '#e0b0f0'; g.fillRect(Math.round(s.x - cx) - 1, Math.round(s.y - cy) - 3, 2, 2); } else if (s.venom) { g.fillStyle = '#8fd160'; g.fillRect(Math.round(s.x - cx) - 2, Math.round(s.y - cy) - 2, 4, 4); g.fillStyle = '#dfffa0'; g.fillRect(Math.round(s.x - cx) - 1, Math.round(s.y - cy) - 2, 2, 1); } else drawSet(SPR.seed, null, 0, s.x - cx, s.y + 3 - cy, 1, false); }
