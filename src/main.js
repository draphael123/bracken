...
import { bakeScout, bakeSiren, bakeTideguard, bakeHerald, bakeFisher, bakeFisherIcon } from './redraw/tidebound.js';
import { bakeSailor, bakeNetter, bakeNet, bakeUrchin, bakeAngler, bakePetrel, bakeReefmaw } from './redraw/reef.js';
...
{ const N = (hero, branch, row, col, id, name, max, desc, parent, active) => TREE.push({ id, hero, branch, row, col, name, max, desc, parent: parent || null, active: !!active });
  ...
  // MORE ON THE KEYS: a new active at the root of the deeper rows of every tree
  N('knight', 0, 3, 1, 'lunge', 'LUNGE', 3, 'F: a dashing thrust through everything in front of you. nothing touches you while you go', 'heavyPlunge', true);
  N('knight', 1, 2, 1, 'warCry', 'WAR CRY', 3, 'F: a shout that throws back and staggers what is near, gives back 30 stamina, and for four seconds every blow on you lands a quarter lighter', 'shieldThrow', true);
  N('knight', 2, 3, 1, 'whirlwind', 'WHIRLWIND', 3, 'F: spin with the blade out: two cuts at everything around you', 'spring', true);
  N('pyro', 0, 3, 1, 'meteor', 'METEOR', 3, 'F: call fire out of the sky onto the nearest foe ahead. the hotter you run, the harder it lands', 'leapFlame', true);
  N('pyro', 1, 3, 1, 'flameRing', 'RING OF FIRE', 3, 'F: a ring of flame rolls out from you, burning and throwing back what it passes and eating what flies at you', 'wisp', true);
  N('paladin', 0, 3, 1, 'lightLance', 'SPEAR OF LIGHT', 3, 'F: a lance of light straight ahead, through everything in its path to the first wall', 'ascension', true);
  N('paladin', 1, 3, 1, 'divineShield', 'DIVINE SHIELD', 3, 'F: a ward of light all round you: nothing touches you for two seconds', 'sureStride', true);
  N('paladin', 2, 3, 1, 'hammerLeap', 'HAMMER LEAP', 3, 'F: leap and bring the maul down: the ground carries it both ways', 'hammerfallDmg', true);
  // A THIRD WAY DOWN EVERY TREE: its own chain, in the far column, ending in something that changes how the tree plays
  N('knight', 0, 1, 2, 'bleed', 'OPEN WOUND', 3, 'a cut leaves a wound that bleeds: 1 damage every half second a point, for three seconds', 'thirdCut');
  N('knight', 0, 3, 2, 'execute', 'EXECUTION', 1, 'a heavy cut finishes any foe already under a quarter of its health', 'bleed');
  N('knight', 1, 1, 2, 'plated', 'PLATED', 3, 'every blow that lands on you lands 5% lighter a point', 'steady');
  N('knight', 1, 3, 2, 'vengeance', 'VENGEANCE', 1, 'what the shield turns is kept, and your next swing carries it (up to 30)', 'plated');
  N('knight', 2, 1, 2, 'momentum', 'MOMENTUM', 3, 'run a second without stopping and the next blow lands 12% harder a point', 'footing');
  N('knight', 2, 3, 2, 'evasion', 'EVASION', 1, 'a dodge through a blow gives back 20 stamina and makes the next swing a heavy one', 'momentum');
  N('pyro', 0, 1, 2, 'scatter', 'SCATTER', 3, 'an ember bursts where it strikes: 3 damage a point to everything else near it', 'skip');
  N('pyro', 0, 3, 2, 'conflagration', 'CONFLAGRATION', 1, 'a burning foe that dies sets light to whatever stands near it', 'scatter');
  N('pyro', 1, 1, 2, 'pilot', 'PILOT LIGHT', 3, 'your heat fades 20% slower a point', 'longFlame');
  N('pyro', 1, 3, 2, 'backdraft', 'BACKDRAFT', 1, 'coming to full heat throws a ring of flame off you', 'pilot');
  N('pyro', 2, 1, 2, 'ashCloak', 'ASH CLOAK', 3, 'for a moment after a dodge, blows land 12% lighter a point', 'fleet');
  N('pyro', 2, 3, 2, 'emberHeart', 'EMBER HEART', 1, 'under a third of your health everything you do hits half as hard again', 'ashCloak');
  N('paladin', 0, 1, 2, 'zeal', 'ZEAL', 3, 'every foe judgement strikes gives you back 4 health a point', 'radiance');
  N('paladin', 0, 3, 2, 'beacon', 'BEACON', 1, 'while the light is full you burn: foes near you catch fire', 'zeal');
...
const MORE_ICONS = {
  ...
  meteor: pixIcon(['.o........', '..o.......', '...oo.....', '....oyy...', '.....ywwy.', '....oywwyo', '.....oyyo.', '......oo..', '..........', 'ssssssssss', 'bbbbbbbbbb', '..........']),
...
const skillIcon = k => k === 'groundSlam' ? SLAM_ICON : k === 'shieldThrow' ? SHIELD_ICON : k === 'risingCut' ? RISE_ICON : PYRO_ICONS[k] || PAL_ICONS[k] || MORE_ICONS[k] || FLAME_ICON;
let wisp = null; // the pyromancer's flame familiar
let meteors = [], fireRings = [], lanceBeams = []; // METEOR, RING OF FIRE, SPEAR OF LIGHT in flight
...
function drawSkillFx(cx, cy) {
  for (const m of meteors) { const x = Math.round(m.x - cx), y = Math.round(m.y - cy); g.fillStyle = '#ff6b2c'; g.beginPath(); g.arc(x, y, 6, 0, 7); g.fill(); g.fillStyle = '#ffd36b'; g.beginPath(); g.arc(x, y, 4, 0, 7); g.fill(); g.fillStyle = '#fff6c8'; g.fillRect(x - 1, y - 2, 3, 3);
    g.globalAlpha = 0.35; g.fillStyle = 'rgba(10,8,20,1)'; g.beginPath(); g.ellipse(Math.round(m.tx - cx), Math.round(m.ty - cy) - 1, 12, 3, 0, 0, 7); g.fill(); g.globalAlpha = 1; } // its shadow on the ground
  for (const r of fireRings) { g.globalAlpha = Math.max(0, 1 - r.r / 86); g.strokeStyle = '#ff9a5c'; g.lineWidth = 3; g.beginPath(); g.ellipse(Math.round(r.x - cx), Math.round(r.y - cy), r.r, r.r * 0.6, 0, 0, 7); g.stroke(); g.strokeStyle = '#ffd36b'; g.lineWidth = 1; g.stroke(); g.globalAlpha = 1; }
  for (const b of lanceBeams) { const k = b.t / 0.3, x0 = Math.round(b.x0 - cx), y = Math.round(b.y - cy), x1 = x0 + b.dir * b.len; g.globalAlpha = k; g.fillStyle = '#ffd36b'; g.fillRect(Math.min(x0, x1), y - 3, Math.abs(x1 - x0), 6); g.fillStyle = '#fff6c8'; g.fillRect(Math.min(x0, x1), y - 1, Math.abs(x1 - x0), 2); g.globalAlpha = 1; }
...
const MASTER = bakeMaster(), KING = bakeKingBig(); SPR.chandelier = bakeChandelier(); SPR.harpy = bakeHarpy(); SPR.crow = bakeCrow(); SPR.horn = bakeHornblower(); SPR.bale = bakeBale(); SPR.goat = bakeCragRam(); SPR.troll = bakeTroll(); SPR.greathound = bakeGreatHound(); SPR.spider = bakeSpider(); SPR.squirrel = bakeSquirrel(); SPR.owl = bakeOwl(); SPR.lamplighter = bakeLamplighter(); SPR.keeper = bakeKeeper(); SPR.bard = bakeBard(); SPR.oldknight = bakeOldKnight(); SPR.miner = bakeMiner(); SPR.bat = bakeBat(); SPR.forgemaster = bakeForgemasterBig(); SPR.grub = bakeGrub(); SPR.rockgoblin = bakeRockGoblin(); SPR.golem = bakeGolem(); SPR.hare = bakeHare(); SPR.wight = bakeWight(); SPR.kite = SPR.sprig; SPR.sailor = bakeSailor(); SPR.netter = bakeNetter(); SPR.netArt = bakeNet(); SPR.urchin = bakeUrchin(); SPR.angler = bakeAngler(); SPR.petrel = bakePetrel(); SPR.reefmaw = bakeReefmaw(); SPR.turtle = bakeTurtle(); SPR.eel = bakeEel(); SPR.heronfoe = bakeHeronFoe(); SPR.crab = bakeCrab(); SPR.scout = bakeScout(); SPR.siren = bakeSiren(); SPR.tideguard = bakeTideguard(); SPR.herald = bakeHerald(); SPR.fisher = [bakeFisher(0), bakeFisher(1)]; SPR.fisherIcon = bakeFisherIcon(); SPR.soldier = bakeSoldier(); SPR.javelin = bakeJavelineer(); SPR.heavy = bakeHeavyKnight(); SPR.windcaller = bakeWindcaller(); SPR.stormshaman = bakeGoblinShaman(); /* the boss has his own sprite now; the storm shaman keeps the old one */
...
let TILE, PROP, BG, SHORE = null, REEF = null, RAINART = null;
function bakeAll(pal = {}) {
  ...
  const sky = (pal.sky === 'night' || pal.sky === 'teal' || pal.sky === 'autumn' || pal.sky === 'crag' || pal.sky === 'sea' || pal.sky === 'storm') ? null : (pal.sky || [[104, 170, 220], [205, 232, 210]]);
  SHORE = SHORE || LWT.bakeShoreTiles(); REEF = REEF || RFT.bakeReefTiles(); RAINART = RAINART || RFT.bakeRain(64, 64); PROP.fallArt = PROP.fallArt || { make: h => LWT.bakeWaterfall(h) };
  BG = { sky: pal.sky === 'storm' ? RFT.bakeSkyStorm(VH) : pal.sky === 'sea' ? LWT.bakeSkySea(VH) : sky ? ART.bakeSky(VH, sky[0], sky[1]) : pal.sky === 'teal' ? ART.bakeSkyTeal(VH) : pal.sky === 'autumn' ? ART.bakeSkyAutumn(VH) : pal.sky === 'crag' ? ART.bakeSkyCrag(VH) : ART.bakeSkyNight(VH), skyDusk: ART.bakeSkyDusk(VH), sun: ART.bakeSun(), far: pal.far === 'reef' ? RFT.bakeFarReef(320, 90, 1) : pal.far === 'sea' ? LWT.bakeFarSea(320, 90, 1) : pal.far === 'crag' ? ART.bakeFarCrags(320, 90, 1) : ART.bakeFar(320, 90, 1), mid: pal.mid === 'wrecks' ? RFT.bakeMidWrecks(480, 140, 2) : pal.mid === 'coast' ? LWT.bakeMidCoast(480, 140, 2) : pal.mid === 'crag' ? ART.bakeMidCrags(480, 140, 2) : ART.bakeMid(480, 140, 2), near: pal.near === 'reef' ? RFT.bakeNearReef(640, 300, 3) : pal.near === 'shore' ? LWT.bakeNearShore(640, 300, 3) : pal.near === 'crag' ? ART.bakeNearCrag(640, 300, 3) : pal.near === 'mushroom' ? ART.bakeNearMushrooms(640, 300, 3) : pal.near === 'autumn' ? ART.bakeNearAutumn(640, 300, 3) : ART.bakeNear(640, 300, 3, pal.canopy), nearTrees: pal.near === 'mushroom' ? ART.bakeNear(640, 300, 5, pal.canopy) : null, fg: pal.fg === 'reef' ? RFT.bakeFGReef(640, VH, 4) : pal.fg === 'shore' ? LWT.bakeFGShore(640, VH, 4) : ART.bakeFG(640, VH, 4) };
}
bakeAll();
applySkin();
...
function resolveTiles() {
  ...
  for (let y = 0; y < LH; y++) for (let x = 0; x < LW; x++) {
    ...
    const shore = L.palette && L.palette.set === 'shore' && SHORE, reefT = L.palette && L.palette.set === 'reef' && REEF;
    const SET2 = reefT ? REEF : shore ? SHORE : null, wetT = SET2 && L.wetZone && x >= L.wetZone[0] && x <= L.wetZone[1];
    ...
    if (t === T.SOLID) {
      ...
      if (up !== T.SOLID && up !== T.CRATE) {
        s = timber ? REEF.hullTop[(rnd() * 4) | 0] : SET2 ? (wetT && eL + '' + eR === '00' ? SET2.wet[(rnd() * 3) | 0] : SET2.top[eL + '' + eR][(rnd() * 4) | 0]) : L.palette && L.palette.myc ? TILE.mycTop[eL + '' + eR][(rnd() * 3) | 0] : TILE.top[eL + '' + eR][(rnd() * 4) | 0];
      ...
      const crag = L.palette && L.palette.dress === 'crag', shoreOW = L.palette && (L.palette.set === 'shore' ? SHORE : L.palette.set === 'reef' ? REEF : null);
  ...
  // THE FROST (the Sunspire): icicles under every ledge and overhang, and little clusters of ice on the ground
  if (L.frost) { const R2 = mulberry(LW * 13 + 5);
    for (let y = 1; y < LH - 1; y++) for (let x = 1; x < LW - 1; x++) { const t = tileAt(x, y), below = tileAt(x, y + 1);
      if ((isSolid(x, y) || isOneWay(t)) && below === T.AIR && R2() < 0.5) { const ow = isOneWay(t) && t !== T.CRYST, n = 1 + ((R2() * 2) | 0); for (let k = 0; k < n; k++) decor.push({ k: 'icicle', x: x * TS + 1 + ((R2() * 9) | 0), y: y * TS + (ow ? 7 : TS) - 1, c: PROP.icicle[(R2() * 3) | 0] }); }
      if (isSolid(x, y) && tileAt(x, y - 1) === T.AIR && R2() < 0.07) decor.push({ k: 'frost', x: x * TS + ((R2() * 6) | 0), y: y * TS - 7, c: PROP.frost[(R2() * 2) | 0] }); } }
...
const MEDALS = { reef: [600, 840, 1180], longwater: [540, 760, 1080], crown: [660, 900, 1260], storm: [600, 820, 1150], moor: [480, 660, 960], scree: [450, 630, 920], spire: [520, 700, 980], hanging: [480, 660, 960], wood: [240, 360, 540], marsh: [300, 450, 660], stockade: [330, 480, 720], spore: [360, 520, 780], kings: [420, 600, 900] };
...
// A RUNG BELL, said plainly: while a hall is shut, the HUD shows a bell, how many of the garrison are still up, and the
...
function drawAlarmHud() {
  ...
  g.fillStyle = '#3a3040'; g.fillRect(x - 22, y + 14, 44, 1); g.fillStyle = '#ffd36b'; g.fillRect(x - 22, y + 14, Math.round(44 * Math.max(0, 1 - (sec.onT || 0) / 20)), 1);
}
...
function spawnEnt(e) {
  ...
  const n0 = enemies.length;
  {
    ...
    switch (e.t) {
      ...
      case 'herald': boss = { ...base, t: 'herald', w: 18, h: 36, hp: EHP.herald, maxHp: EHP.herald, mode: 'sleep', modeT: 0, face: -1, phase: 1, hitT: 0, raiseT: 6, sweepT: 1.2, thrustT: 2, glideT: 4 }; enemies.push(boss); break;
      ...
      case 'deco': { const K = { hiveBg: [PROP.hiveBg, true], drip: [PROP.honeyDrip[0], false, PROP.honeyDrip], frogStatue: [PROP.frogStatue[e.v || 0], true], lilyLantern: [PROP.lilyLantern[0], false, PROP.lilyLantern], banner: [(e.hang ? PROP.bannerHung : PROP.banner)[e.v || 0], true], boneThrone: [PROP.boneThrone, true], skullPile: [PROP.skullPile[e.v || 0], false], hangCage: [e.hang ? PROP.hangCage : PROP.gibbet, true], eyrie: [PROP.eyrie, false], siege: [PROP.siege, true], bough: [PROP.bough, true], bothy: [PROP.bothy, true], cabin: [PROP.cabin, true], trunk: [PROP.trunk[e.v || 0], true], bracket: [PROP.bracket[e.v || 0], true], axle: [PROP.axle, true], pillar: [PROP.pillar[e.v || 0], true], strut: [PROP.strut[e.v || 0], true], deadTree: [PROP.deadTree[e.v || 0], true], counter: [PROP.counter, false], wares: [PROP.wares[e.v || 0], true], barrels: [PROP.barrelStack, true], lanternPost: [PROP.lanternPost, true], beehive: [PROP.beehive, true], birdhouse: [PROP.birdhouse, true], fishTrap: [PROP.fishTrap[e.v || 0], true], spearRack: [PROP.spearRack, true], bones: [PROP.bones[e.v || 0], false], well: [PROP.well, true], cart: [PROP.cart, true], fence: [PROP.fence[e.v || 0], true], mill: [PROP.mill, true], stone: [PROP.standingStone[e.v || 0], true], cairn: [PROP.cairn, false], foldGate: [PROP.foldGate, true], rootDecor: [PROP.rootDecor[e.v || 0], false], sporePod: [PROP.sporePod[0], false, PROP.sporePod], timber: [PROP.timber[e.v || 0], true], cobweb: [PROP.cobweb[e.v || 0], true], tent: [PROP.tent[e.v || 0], true], spire: [TILE.spire[0][e.v || 0], true], throne: [SPR.throne, true], hallWindow: [HALLWIN || (HALLWIN = bakeHallWindow()), true], bridgepost: [PROP.bridgePost, false], bridgetower: [PROP.bridgeTower, true], gatehouse: [PROP.gatehouse, true], forge: [PROP.forge, true], anvil: [PROP.anvil, false], stilt: [PROP.stilt, true], frozen: [PROP.frozen[e.v || 0], true], mastStump: [PROP.reef.mastStump, false], rigging: [PROP.reef.rigging[(e.v || 0) % 2], true], sailRag: [PROP.reef.sailRag[(e.v || 0) % 2], true], wreckBow: [PROP.reef.wreckBow, true], wreckStern: [PROP.reef.wreckStern, true], figurehead: [PROP.reef.figurehead, false], capstan: [PROP.reef.capstan, false], anchor: [PROP.reef.anchor, false], seaChest: [PROP.reef.seaChest, false], shipBell: [PROP.reef.shipBell, false], lanternBuoy: [PROP.reef.lanternBuoy[e.v === 0 ? 0 : 1], false], coralFan: [PROP.reef.coralFan[(e.v || 0) % 3], false], brainCoral: [PROP.reef.brainCoral[(e.v || 0) % 2], false], urchinRock: [PROP.reef.urchinRock[(e.v || 0) % 2], false], spar: [PROP.reef.spar[(e.v || 0) % 2], false], bubbleVent: [PROP.reef.bubbleVent, false], airBell: [PROP.reef.airBell, false], kelpTall: [PROP.reef.kelpTall[(e.v || 0) % 3], false], wheel: [PROP.reef.wheel, false], reefRock: [PROP.reef.reefRock[(e.v || 0) % 2], false], coralTuft: [PROP.lw.coralTuft[(e.v || 0) % 3], false], barnacleRock: [PROP.lw.barnacleRock[(e.v || 0) % 2], false], saltCrust: [PROP.lw.saltCrust[(e.v || 0) % 2], false], drownedHut: [PROP.lw.drownedHut, true], fishCottage: [PROP.lw.cottage[(e.v || 0) % 2], true], bellTower: [PROP.lw.bellTower, true], seaLantern: [PROP.lw.seaLantern[e.v === 0 ? 0 : 1], false], buoy: [PROP.lw.buoy, false], tributeChest: [PROP.lw.tributeChest, false], rowboat: [PROP.lw.rowboat, false], netPoles: [PROP.lw.netPoles, true], pierPost: [PROP.lw.pierPost, false], tidePool: [PROP.lw.tidePool, false], kelp: [PROP.lw.kelp[(e.v || 0) % 3], false] }[e.kind]; if (!K) break; const c = K[0]; let gy = e.y; if (!e.hang && GROUNDED_DECO.has(e.kind)) { const gnd = (x, y) => isSolid(x, y) || isOneWay(tileAt(x, y)); let n = 0; while (gnd(e.x, gy) && n++ < 4) gy--; n = 0; while (!gnd(e.x, gy + 1) && n++ < 4) gy++; if (!gnd(e.x, gy + 1)) gy = e.y; } const pyg = (gy + 1) * TS; /* snapped onto the surface: a spire a row low sank into the rock, a row high floated */ const onPlank = !e.hang && tileAt(e.x, gy + 1) === T.PLANK; // a bridge plank's board sits a few pixels down its tile: stand things on the board, not in the air over it
        ...
        if (e.kind === 'seaLantern' && e.v !== 0) lights.push({ x: px, y: pyg - 19, r: 40, torch: true }); if (e.kind === 'lanternPost') lights.push({ x: px, y: pyg - 18, r: 40, torch: true }); if (e.kind === 'counter') lights.push({ x: px + 6, y: pyg - 20, r: 56, glow: true }); if (e.kind === 'mill') lights.push({ x: px, y: pyg - 46, r: 30, glow: true }); if (e.kind === 'sporePod') lights.push({ x: px, y: pyg - 8, r: 40, glow: true }); break; }
...
const COAST_PATH = [[48, 172], [74, 160], [108, 150], [134, 132], [152, 104], [136, 92], [118, 84], [96, 72]]; // down off Highcrown's back face to the river, the town, and out over the water to the reef
...
const NODE_AT = [0, 3, 6, 8, 10, 12, 20, 23, 25, 28, 31, 34, 39, 44, 47]; // PATH index of each node: wood 0-5, the crags, then the coast
...
function updateTree(dt) {
  ...
  // and then over into the tree beside it. The chosen skill is what says which tree is open.
  const inB = b => ns.filter(n => n.branch === b);
  const pick = (list, row, col) => { let best = null, bs = 1e9; for (const n of list) { const s2 = Math.abs(n.row - row) * 4 + Math.abs(n.col - col); if (s2 < bs) { bs = s2; best = n; } } return best ? ns.indexOf(best) : treeI; };
  const go = (dxc, dr) => {
    if (respec) { if (dr < 0) { const b = treeBranch, rows = inB(b).filter(n => n.row === Math.max(...inB(b).map(q => q.row))); treeI = ns.indexOf(rows[0]); } return; }
    const b = cur.branch;
    if (dr) { const list = inB(b).filter(n => Math.sign(n.row - cur.row) === dr);
      if (!list.length) { if (dr > 0) treeI = ns.length; return; }
      const near = Math.min(...list.map(n => Math.abs(n.row - cur.row)));
      treeI = pick(list.filter(n => Math.abs(n.row - cur.row) === near), cur.row + dr * near, cur.col); return; }
    const same = inB(b).filter(n => Math.sign(n.col - cur.col) === dxc);
    if (same.length) { const near = Math.min(...same.map(n => Math.abs(n.col - cur.col))); treeI = pick(same.filter(n => Math.abs(n.col - cur.col) === near), cur.row, cur.col + dxc * near); return; }
    const nb = (b + dxc + 3) % 3, list = inB(nb); if (!list.length) return;
    const edge = dxc > 0 ? Math.min(...list.map(n => n.col)) : Math.max(...list.map(n => n.col));
    treeI = pick(list.filter(n => n.col === edge), cur.row, edge); SFX.ui();
  };
...
function drawTree() {
  g.drawImage(MAPC, 0, 0); g.fillStyle = 'rgba(10,9,18,0.92)'; g.fillRect(0, 0, VW, VH);
  ...
  panel(3, 2, VW - 6, VH - 4);
  text((HEROES.find(k => k.id === h) || {}).name + '   LEVEL ' + heroLevel(), 9, 7, UI.title, 'left', 6);
  { const p = godMode() ? 'ANY' : String(left), w = (p.length + 7) * 6 + 8, x = VW - 8 - w; // the points to spend, in a plate of their own so they are the first thing you see
    ...
    text(p + ' POINTS', x + w / 2, 5, any ? UI.gold : UI.dim, 'center', 6); }
  // the three trees, as tabs: the open one is lit, and each carries what is spent in it
  const tw = Math.floor((VW - 20) / 3);
  for (let i = 0; i < 3; i++) { const x = 8 + i * (tw + 2), on = i === b;
    const sp = TREE.filter(n => n.hero === h && n.branch === i).reduce((t, n) => t + (tal(n.id) || 0), 0);
  ...
  text(cur && cur.active && tal(cur.id) ? 'Z LEARN    F OR G SETS THE KEY    Q CLOSE' : 'ARROWS MOVE    Z LEARN    Q CLOSE', VW / 2, VH - 9, '#8a8a94', 'center', 6);
}
...
const BEASTS = [
  ...
  { t: 'turtle', name: 'SNAPPING TURTLE', sub: 'the river', desc: 'Slow, and its shell turns anything from the front: hit it there and it pulls its head in. It snaps on a tell; after the snap its neck is out and it is yours.' },
  { t: 'eel', name: 'EEL', sub: 'the plunge pools', desc: 'It only wants you in the water. Its eyes show before it lunges: be at the surface, or be out.' },
  { t: 'heronfoe', name: 'GREY HERON', sub: 'the shallows', desc: 'It stands so still you forget it. When its neck coils back, it is about to spear you. One cut and it is done.' },
  { t: 'crab', name: 'SHORE CRAB', sub: 'too far from the sea', desc: 'Claws up, it turns a blade. Come down on it from above and it goes over on its back, and then it is soft.' },
  { t: 'scout', name: 'TIDEBOUND SCOUT', sub: 'the watchers', desc: 'Pale and silent. It throws a harpoon at where you stand, twice, and then it is somewhere else. A parried harpoon goes back.' },
  { t: 'siren', name: 'SIREN', sub: 'the rocks', desc: 'She sings, and you walk toward her whether you mean to or not. Hit her and she goes under for a while.' },
  { t: 'tideguard', name: 'TIDEGUARD', sub: 'the drowned realm', desc: 'Coral plate turns half of a cut. The trident reaches further than you think: when it is drawn back, get out of its line, or block it and he is open.' },
  ...
  { t: 'petrel', name: 'STORM PETREL', sub: 'off the wrecks', desc: 'Circles above and dives on a line at where you are, not where you will be. Walk out of the line, or take it out of the air.' },
  { t: 'reefmaw', name: 'THE REEFMAW', sub: 'a moray the length of a mast', desc: 'It has four holes and is only in one: bubbles say which. Its hide turns a blade while it lurks. When its bite hits coral instead of you, its jaw sticks fast, and that is when it can be cut. The hole floods higher each phase; the coral stools and the diving bells are all that is left.' },
  ...
  { t: 'javelin', name: 'JAVELINEER', sub: 'the skirmish line', desc: 'Keeps his distance and throws at where you stand. Parry a javelin and it goes back at him. Close in and he runs.' },
  { t: 'heavy', name: 'HEAVY KNIGHT', sub: 'full plate', desc: 'Slow, and his plate turns most of a cut. A double mark is the overhead: get out from under it. A single mark is the sweep: block it and he is off balance. He is open while he drags his blade out of the floor, and a plunge finds the gaps in his helm.' },
...
const BOSS_T = ['queen', 'frog', 'chief', 'mother', 'greathound', 'king', 'ram', 'owl', 'forgemaster', 'golem', 'windcaller', 'lance', 'roc', 'gqueen', 'herald', 'reefmaw'];
...
const KEYS = {
  ...
  left: ['ArrowLeft', 'a', 'A'], right: ['ArrowRight', 'd', 'D'], down: ['ArrowDown', 's', 'S'], up: ['ArrowUp', 'w', 'W'], pause: ['Escape', 'p', 'P'], talents: ['q', 'Q'],
};
...
function damagePlayer0(fromX, dmg, { up = false, unblockable = false } = {}) {
  ...
  if (tal('plated')) dmg = Math.max(1, Math.round(dmg * (1 - 0.05 * tal('plated')))); // PLATED
...
function spawnCorpse(e, dir) {
  ...
  switch (e.t) {
    ...
    case 'brute': case 'heavy': Object.assign(c, { vx: dir * 30, vy: -80, spin: 0, life: 1.2, max: 1.2, tip: true }); SFX.gobDieLow(); break;
    ...
    case 'reefmaw': Object.assign(c, { vx: 0, vy: 26, spin: 0, life: 2, max: 2, grav: 0 }); break;
    case 'herald': Object.assign(c, { vx: 0, vy: 12, spin: 0, life: 2.4, max: 2.4, grav: 0, royal: true }); break; // he goes down into the water without a word
...
function hurtEnemy0(e, dmg, fromX, plunge) {
  ...
  if (e.hp <= 0) {
    ...
    if (e.t === 'chief') chiefDies(); else if (!e.mini && (e.t === 'gqueen' || e.t === 'queen' || e.t === 'frog' || e.t === 'king' || e.t === 'ram' || e.t === 'owl' || e.t === 'forgemaster' || e.t === 'golem' || e.t === 'windcaller' || e.t === 'lance' || e.t === 'suncatcher' || e.t === 'roc' || e.t === 'herald' || e.t === 'reefmaw')) queenDies(e); // (a boss missing from this list leaves the walls shut and the fight running)
...
function updatePlayer(dt) {
  ...
  if (isPyro() && skillPress('fireWall') && P.ground && cdReady('fireWall') && !P.dead && !(P.hurt > 0) && !(P.asleep > 0) && !(P.dodge > 0)) { if (spend(25)) { cdSet('fireWall'); P.atk = -1; for (let i = 1; i <= 5; i++) fires.push({ x: P.x + P.face * i * 14, y: P.y, life: 3, delay: i * 0.08, own: true }); SFX.heavy(); SFX.puff(); number(P.x, P.y - 24, 'FIRE WALL', '#ff9a5c'); shakeCam(2); } else number(P.x, P.y - 22, 'TIRED', '#9aa39a'); }
  ...
  if (isPaladin() && skillPress('consecrate') && P.ground && cdReady('consecrate') && !P.dead && !(P.hurt > 0) && !(P.asleep > 0) && !(P.dodge > 0)) { if (spend(25)) { cdSet('consecrate'); hallows.push({ x: P.x, y: P.y, life: 4 + tal('consecrate'), r: 40, tick: 0 }); SFX.mend(); SFX.heavy(); ringAt(P.x, P.y - 4, 40, '#ffd36b', 0.5); motes(P.x, P.y - 6, 14, 30); P.castT = 0.3; } else number(P.x, P.y - 22, 'TIRED', '#9aa39a'); }
  if (isPaladin() && skillPress('holyCharge') && cdReady('holyCharge') && !P.dead && !(P.hurt > 0) && !(P.asleep > 0) && !(P.dodge > 0)) { if (spend(20)) { cdSet('holyCharge'); P.chargeT = 0.3; P.dodge = 0.3; P.vx = P.face * 330; P.inv = Math.max(P.inv, 0.4); P.chargeHit = new Set(); SFX.throwWhoosh(); SFX.aegis(); streaks(P.x, P.y - 10, 8, ['#fff6c8', '#ffd36b'], 160); } else number(P.x, P.y - 22, 'TIRED', '#9aa39a'); }
  ...
  if (P.lungeT > 0) { P.lungeT -= dt; P.vx = P.face * 470; ghosts.push({ x: P.x, y: P.y, face: P.face, life: 0.14, frame: 1 });
    for (const e of enemies) if (e.alive && !e.harmless && !P.lungeHit.has(e) && overlap({ l: P.x - 12, r: P.x + 12, t: P.y - 20, b: P.y }, box(e))) { P.lungeHit.add(e); hurtEnemy(e, Math.round((swordDmg() + 4) * 1.4 * amul('lunge')), P.x, false); sparks(e.x, e.y - e.h / 2, P.face, 8); hitstop(0.04); } }
  ...
  if (isPyro() && skillPress('meteor') && cdReady('meteor') && canAct()) { if (spend(25)) { cdSet('meteor'); let tx = P.x + P.face * 90, best = 220; for (const e of enemies) if (e.alive && !e.harmless && Math.sign(e.x - P.x) === P.face && Math.abs(e.x - P.x) < best && Math.abs(e.y - P.y) < 120) { best = Math.abs(e.x - P.x); tx = e.x; }
      const txT = Math.floor(tx / TS); let gy = Math.floor((P.y - 8) / TS); for (let k = 0; k < 16 && !isSolid(txT, gy) && !isOneWay(tileAt(txT, gy)); k++) gy++; meteors.push({ x: tx - P.face * 50, y: camY - 20, x0: tx - P.face * 50, y0: camY - 20, tx, ty: gy * TS, t: 0 }); P.castT = 0.3; SFX.stormChant(); } else tired(); }
  // RING OF FIRE
  ...
  if (isPaladin() && skillPress('divineShield') && cdReady('divineShield') && !P.dead && !(P.asleep > 0)) { cdSet('divineShield'); P.divineT = 2 + 0.5 * (tal('divineShield') - 1); SFX.aegis(); SFX.lightFull(); ringAt(P.x, P.y - 10, 24, '#ffd36b', 0.4); }
  if (P.divineT > 0) P.divineT -= dt;
  // HAMMER LEAP: up, and down with the maul
  if (isPaladin() && skillPress('hammerLeap') && cdReady('hammerLeap') && P.ground && canAct()) { if (spend(25)) { cdSet('hammerLeap'); P.hleapT = 1.4; P.vy = -330; P.vx = P.face * 200; P.ground = false; P.coyote = 0; P.onMover = null; P.canCut = false; P.atk = -1; SFX.pJump(); SFX.throwWhoosh(); } else tired(); }
  if (P.hleapT > 0) { P.hleapT -= dt; if (P.hleapT < 1.25 && P.ground) { P.hleapT = 0; shakeCam(8); zoomKick(1.1, 0.22); ringAt(P.x, P.y - 2, 40, '#ffd36b', 0.4); dust(P.x - 10, P.y, 8); dust(P.x + 10, P.y, 8); SFX.forgeHammer(); SFX.heavy(); hitstop(0.05);
      for (const d of [-1, 1]) pwaves.push({ x: P.x + d * 8, y: P.y, dir: d, life: 1.1 * (1 + 0.2 * tal('farTremor')), sp: 220, hit: new Set() });
  ...
  if (isPyro()) { // C tapped = an ember; C held = the jet. A FULL bar banks for a few seconds and the next press of C is THE PYRE.
    ...
    else P.heat -= (P.asleep > 0 || P.sleepM > 0.2 ? 8 : 16) * (1 - 0.2 * tal('pilot')) * dt; // PILOT LIGHT keeps it in longer
  ...
  if (isPaladin()) { // C tapped = MEND. C held = AEGIS. A full bar and the next press of C is JUDGEMENT.
    ...
    if (tal('crusade') && P.aegisWas && !P.aegis && !P.dead) { let any = false; // CRUSADE
      ...
      if (any) { SFX.judgement(); shakeCam(4); } ringAt(P.x, P.y - 10, 40, '#fff6c8', 0.35); motes(P.x, P.y - 10, 8, 8); }
    if (tal('beacon') && (P.light || 0) >= 100 && !P.dead) { P.beaconT = (P.beaconT || 0) - dt; if (P.beaconT <= 0) { P.beaconT = 0.5; // BEACON
      for (const e of enemies) if (e.alive && !e.harmless && Math.abs(e.x - P.x) < 44 && Math.abs(e.y - P.y) < 32) { e.burn = Math.max(e.burn || 0, 1.6); flame(e.x, e.y - e.h / 2, 3, 3, 30, 1); } }
      if (Math.random() < dt * 20) motes(P.x + (Math.random() - 0.5) * 26, P.y - 6 - Math.random() * 14, 1, 4); }
  ...
  const cap = ((P.block || P.jet) ? 32 : P.swim ? 72 : wading ? 46 : spored ? 40 : RUN * (PROG.charm === 'swift' ? 1.12 : 1) * (1 + 0.04 * (tal('swiftness') + tal('lightFeet') + tal('sureStride'))) * (isPyro() ? 1.15 : isPaladin() ? 0.9 : 1)) + (P.gustT > 0 && !P.ground ? 150 : 0); // a gust can carry you faster than your legs
  ...
  if (move && !groundAtk) {
    const acc = P.ground ? (onSlick ? 360 : 1000) : 700;
  ...
  if (P.abuf > 0 && !stunned && !P.plunge && !dodging && !P.aegis) {
    if (!P.ground && !P.swim && (keys.down || P.abufDown)) { P.abuf = 0; P.abufDown = false; if (spend(plungeCost())) { P.plunge = true; P.vy = Math.max(P.vy, 60); P.atk = -1; P.hitSet.clear(); SFX.slash();
  ...
  if (P.swim) { const under = swimP.capped ? 99 : P.y - swimP.y;
    P.vy += 150 * dt; if (under > 15) P.vy -= 300 * dt; if (keys.down) P.vy += 380 * dt; if (keys.up || (keys.jump && under > 18)) P.vy -= 340 * dt;
    P.vy = Math.max(-140, Math.min(115, P.vy)); P.plunge = false; P.airRolled = false; P.canCut = true;
    if (P.jbuf > 0 && under < 18) { P.jbuf = 0; P.vy = -310; P.swim = false; burst(P.x, swimP.y, 8, ['#e8f4f0', '#7cc8c8'], 60, 0.4, 400, 1); SFX.pJump(); }
    ...
    else if (under > 24) { P.breath = (P.breath ?? breathMax) - dt; if (Math.random() < dt * 4) parts.push({ x: P.x + (Math.random() - 0.5) * 6, y: P.y - 16, vx: 0, vy: -30, life: 0.9, max: 0.9, col: '#e8f4f0', size: 1, grav: -20 });
      if (P.breath <= 0) { P.breath = 0; P.drownT = (P.drownT || 0) - dt; if (P.drownT <= 0) { P.drownT = 1; P.inv = 0; damagePlayer(P.x, 6, { unblockable: true }); } } }
  ...
  for (const e of enemies) {
    ...
    if (!P.plunge && P.vy > 40 && pb.b <= e.y - e.h + 7 && P.dodge <= 0) {
      ...
      if (e.t === 'shield' || e.t === 'soldier' || e.t === 'heavy' || e.t === 'turtle' || e.t === 'tideguard' || e.t === 'herald' || e.t === 'sailor' || e.t === 'reefmaw' || e.t === 'queen' || e.t === 'brute' || e.t === 'chief' || e.t === 'king' || (e.t === 'master' && e.mounted) || (e.t === 'ram' && !ramOpen(e))) { P.vy = keys.jump ? -260 : -190; SFX.clank(); sparks(e.x, e.y - e.h, P.face, 6); e.stagger = Math.max(e.stagger, 0.3); number(e.x, e.y - e.h - 6, 'HELM', '#c9d1dc'); squash(0.85, 1.2, 0.1); continue; }
...
function bossEnd(e) {
  ...
  switch (e.t) {
    ...
    case 'herald': { // he sinks without a word; the sea goes out of the square; the glaive stays planted, pointing to sea
      shakeCam(8); zoomKick(1.14, 0.6); SFX.seaBell(); SFX.waveCrash(); heraldWave = null;
    ...
    case 'reefmaw': { // it goes back down its own hole for the last time, and the water goes off the reef
      shakeCam(10); zoomKick(1.16, 0.6); SFX.boreRoar(); SFX.waveCrash();
      const rp = (L.pools || []).find(q => q.arenaTide); if (rp) { rp.tideY = floor + 6; poolLevel(rp, floor + 6); }
      for (let i = 0; i < 46; i++) parts.push({ x: x + (Math.random() - 0.5) * 40, y: y - Math.random() * 40, vx: (Math.random() - 0.5) * 90, vy: -40 - Math.random() * 70, life: 1.7, max: 1.7, col: Math.random() < 0.5 ? '#7cc8c8' : '#e8f4f0', size: 2, grav: 70 });
      say('THE REEF IS QUIET', '#7cc8c8'); break; }
...
// THE QUEEN'S LINE (Highcrown): soldiers with sword and shield, javelineers who keep their distance, and heavy knights in plate.
function updateTroop(e, dt) {
  const d = P.x - e.x, ad = Math.abs(d), dy = Math.abs(e.y - P.y);
  e.vy += 1000 * dt; if (e.vy > 300) e.vy = 300; e.modeT -= dt; e.cd = Math.max(0, (e.cd || 0) - dt); e.guardT = Math.max(0, (e.guardT || 0) - dt);
  let want = 0, near = false;
  if (e.t === 'soldier') { // he closes behind the shield, turns slowly, and swings on a clear tell
  ...
  } else if (e.t === 'javelin') { // he keeps a spear's throw off, and throws where you stand
    near = ad < 250 && dy < 90 && !P.dead; if (near && e.mode !== 'throw') e.face = Math.sign(d) || e.face;
    if (e.mode === 'aim') { want = 0; if (e.modeT <= 0) { e.mode = 'throw'; e.modeT = 0.35; SFX.throwWhoosh();
    ...
    else if (e.mode === 'sweep') { want = 0; if (e.modeT <= 0) { e.mode = 'rest'; e.modeT = 0.8; e.cd = 1.2; } }
    else if (e.mode === 'rest') { want = 0; if (e.modeT <= 0) e.mode = 'walk'; }
    else { e.mode = 'walk'; if (near) { e.face = Math.sign(d) || e.face; want = ad > 26 ? e.face * e.speed : 0;
      if (e.cd <= 0 && ad < 36) { e.mode = 'raise'; e.modeT = 0.95; number(e.x, e.y - e.h - 12, '!!', '#ff6b6b'); SFX.charge(); }
      else if (e.cd <= 0 && ad < 58) { e.mode = 'windUp'; e.modeT = 0.6; number(e.x, e.y - e.h - 12, '!', '#ffd36b'); SFX.charge(); } } }
    if (Math.abs(e.vx) > 4 && e.mode === 'walk' && Math.floor(e.anim * 5) !== Math.floor((e.anim - dt) * 5) && Math.abs(P.x - e.x) < 260) { if (SFX.foeStep) SFX.foeStep(); if (Math.abs(P.x - e.x) < 120) shakeCam(0.6); }
  }
  if (e.stagger > 0) want = 0;
  e.vx += (want - e.vx) * Math.min(1, dt * (e.t === 'heavy' ? 4 : 8));
  ...
  if (r.ground && (aheadT === T.AIR || aheadT === T.SPIKE) && Math.sign(e.vx) === dirM) { e.vx = 0; if (!near && e.mode === 'walk') e.face = -e.face; } /* the line holds its ground: nobody steps off an edge */
  if (r.hitX) { e.vx = 0; if (!near && e.mode === 'walk') e.face = -e.face; }
}
...
// close - all contact, no attack. ANGLER: hangs in the dark with its lure lit, then bites the length of its own body.
// PETREL: circles overhead and dives on a line, and has to climb again after.
function updateReef(e, dt) {
  ...
  if (e.t === 'sailor') { const near = ad < 150 && dy < 40 && !P.dead;
    ...
    else { e.mode = 'walk'; if (near && e.stagger <= 0) { e.face = Math.sign(d) || e.face;
      if (ad < 70) want = -e.face * e.speed; else if (ad > 150) want = e.face * e.speed * 0.7;
      if (e.hasNet && e.cd <= 0 && ad > 46) { e.mode = 'castTell'; e.modeT = 0.5; number(e.x, e.y - e.h - 10, '!', '#ffd36b'); } } else want = 0;
      if (!e.hasNet && e.cd <= 0.4) e.hasNet = true; }
  } else if (e.t === 'urchin') { grav = false; const pl = e.pool = reefHome(e);
    const top = pl ? pl.y + 8 : e.y - 40, bot = pl ? (pl.bottom || pl.y + 80) - 4 : e.y + 40;
    const chase = pl && P.swim && !P.dead && ad < 120 && dy < 80;
    e.spin += dt; e.bristle = ad < 40 && dy < 34 ? 1 : 0;
    ...
    if (e.mode === 'biteTell') { e.vx *= 0.7; e.vy *= 0.7; e.face = Math.sign(d) || e.face;
      if (e.modeT <= 0) { e.mode = 'bite'; e.modeT = 0.5; const dd = Math.hypot(d, (P.y - 8) - e.y) || 1; e.vx = d / dd * 300; e.vy = ((P.y - 8) - e.y) / dd * 300; e.hit = false; SFX.slash(); } }
    else if (e.mode === 'bite') { if (!e.hit && !P.dead && ad < 18 && Math.abs((P.y - 8) - e.y) < 14) { e.hit = true; const res = damagePlayer(e.x, DMG.anglerBite); if (res === 'hit') P.vx = Math.sign(e.vx) * 180; }
      if (e.modeT <= 0) { e.mode = 'swim'; e.cd = 2.2; e.vx *= -0.2; e.vy = 0; } }
    else { e.mode = 'swim';
      if (mine && ad < 110 && dy < 70 && e.cd <= 0 && e.stagger <= 0) { e.mode = 'biteTell'; e.modeT = 0.5; number(e.x, e.y - 16, '!', '#ff6b6b'); SFX.snort(); }
    ...
    else if (e.mode === 'climb') { e.x += (e.hx - e.x) * Math.min(1, dt * 1.5); e.y += (e.hy - e.y) * Math.min(1, dt * 2.2); if (e.modeT <= 0) { e.mode = 'hover'; e.modeT = 1.2 + Math.random(); } }
    else { e.mode = 'hover'; e.x += Math.sin(e.anim * 1.6) * 26 * dt; e.y += Math.sin(e.anim * 2.2) * 10 * dt; e.face = Math.sign(P.x - e.x) || e.face;
      if (e.modeT <= 0 && !P.dead && ad < 170 && P.y > e.y + 10) { e.mode = 'dive'; e.modeT = 1.2; e.hit = false; const dd = Math.hypot(d, (P.y - 8) - e.y) || 1; e.dvx = d / dd * 260; e.dvy = ((P.y - 8) - e.y) / dd * 260; SFX.caw(); number(e.x, e.y - 10, '!', '#ffd36b'); } }
    return;
  }
  if (e.stagger > 0) want = 0;
...
function updateShore(e, dt) {
  const d = P.x - e.x, ad = Math.abs(d), dy = Math.abs(e.y - P.y);
  e.modeT -= dt; e.cd = Math.max(0, (e.cd || 0) - dt); e.guardT = Math.max(0, (e.guardT || 0) - dt);
  let want = 0, near = false, grav = true;
  if (e.t === 'turtle') { near = ad < 130 && dy < 30 && !P.dead;
    if (e.mode === 'hide') { if (e.modeT <= 0) e.mode = 'walk'; }
    ...
    else { e.mode = 'swim'; if (inMine && ad < 120 && e.cd <= 0 && e.stagger <= 0) { e.mode = 'lungeTell'; e.modeT = 0.45; number(e.x, e.y - 12, '!', '#ff6b6b'); }
    ...
    else if (e.mode === 'sing') { if (!P.dead && ad < 170 && dy < 90) { const pull = P.block || P.aegis || P.jet ? 18 : 52; moveBody(P, -Math.sign(d) * pull * dt, 0, false); if (Math.random() < dt * 6) parts.push({ x: e.x + (Math.random() - 0.5) * 10, y: e.y - 16, vx: -Math.sign(d) * -30, vy: -10, life: 1, max: 1, col: '#7ff0e0', size: 1, grav: 0, glow: true }); } if (e.modeT <= 0) { e.mode = 'idle'; e.modeT = 2.5; } }
...
// THE REEFMAW. A moray the length of a mast, living in the coral under the wreck. It has four holes along the arena
// and is only ever in one of them: bubbles over a hole say which, and that is the whole fight - read the bubbles,
// stand somewhere else, and punish the bite. A bite that lands on coral instead of you leaves its jaw STUCK for two
// seconds, and that is the only time its hide can be cut properly. It floods the hole higher each phase, so the
// standing room shrinks to the three coral stools; the diving bells at either end are the air.
function mawPool() { return (L.pools || []).find(p => p.arenaTide); }
function mawHoles() { return (L.arena && L.arena.holes) || []; }
function updateMaw(e, dt) {
  const A = L.arena, floor = A.floor, pl = mawPool(); e.modeT -= dt; e.anim += dt;
  const holes = mawHoles(), hx = holes[e.hole] !== undefined ? holes[e.hole] : e.x;
  const d = P.x - e.x, ad = Math.abs(d);
  const level = [floor + 6, floor + 6, floor - 14, floor - 34][e.phase] || floor + 6;
  if (e.mode === 'sleep') { e.x = hx; e.y = floor; e.rise = 0; e.h = 12; if (pl) poolLevel(pl, floor + 6); return; }
  if (pl) { const want = e.mode === 'dead' ? floor + 6 : level; pl.tideY = pl.tideY === undefined ? floor + 6 : pl.tideY + (want - pl.tideY) * Math.min(1, dt * 1.2); poolLevel(pl, pl.tideY); }
  if (e.phase === 1 && e.hp < e.maxHp * 0.66) { e.phase = 2; SFX.boreRoar(); shakeCam(5); number(e.x, e.y - 60, 'THE HOLE FILLS', '#7cc8c8'); }
  if (e.phase === 2 && e.hp < e.maxHp * 0.33) { e.phase = 3; SFX.boreRoar(); shakeCam(6); number(e.x, e.y - 60, 'IT COMES RIGHT OUT', '#7cc8c8'); }
  const p2 = e.phase >= 2, p3 = e.phase >= 3;
  e.face = Math.sign(d) || e.face;
  const setHole = () => { let best = e.hole; // it moves to the hole nearest you, but never the one it just used
    let bs = 1e9; for (let i = 0; i < holes.length; i++) { if (i === e.hole) continue; const s2 = Math.abs(holes[i] - P.x); if (s2 < bs) { bs = s2; best = i; } }
    e.hole = best; e.x = holes[best]; };
  switch (e.mode) {
    case 'wake': if (e.modeT <= 0) { e.mode = 'lurk'; e.modeT = 1.2; } e.rise = Math.max(0, e.rise - dt * 2); break;
    case 'lurk': { e.rise = Math.max(0, e.rise - dt * 2);
      if (Math.random() < dt * 22) parts.push({ x: e.x + (Math.random() - 0.5) * 14, y: floor - 4, vx: 0, vy: -50, life: 0.9, max: 0.9, col: '#e8f4f0', size: 1, grav: -30 });
      if (e.modeT <= 0) { e.mode = 'riseTell'; e.modeT = p3 ? 0.45 : 0.65; SFX.snort(); number(e.x, floor - 20, '!', '#ffd36b'); } break; }
    case 'riseTell': { if (Math.random() < dt * 50) parts.push({ x: e.x + (Math.random() - 0.5) * 18, y: floor - 4, vx: (Math.random() - 0.5) * 30, vy: -80, life: 0.7, max: 0.7, col: '#7cc8c8', size: 2, grav: -20 });
      if (e.modeT <= 0) { e.mode = 'rise'; e.modeT = 0.5; SFX.mawRoar(); shakeCam(3); } break; }
    case 'rise': { e.rise = Math.min(1, e.rise + dt * 2.4);
      if (e.modeT <= 0) { const far = ad > 90;
        if (far && !p3) { e.mode = 'sink'; e.modeT = 0.4; }
        else if (p2 && Math.random() < 0.35) { e.mode = 'thrashTell'; e.modeT = 0.5; number(e.x, e.y - 50, '!', '#ff6b6b'); }
        else { e.mode = 'biteTell'; e.modeT = p3 ? 0.34 : 0.5; e.aimX = P.x; e.aimY = P.y; number(e.x, e.y - 50, '!', '#ffd36b'); SFX.charge(); } } break; }
    case 'biteTell': if (e.modeT <= 0) { e.mode = 'bite'; e.modeT = 0.38; e.hit = false; SFX.mawSnap(); shakeCam(2); } break;
    case 'bite': { const reach = 78, tx = e.x + e.face * reach * Math.min(1, 1 - e.modeT / 0.38);
      if (!e.hit && !P.dead && Math.abs(P.x - tx) < 26 && Math.abs(P.y - (e.y - 20)) < 40) { e.hit = true; const res = damagePlayer(e.x, DMG.mawBite); if (res === 'hit') { P.vx = e.face * 240; P.vy = -150; } }
      if (e.modeT <= 0) { // a bite that caught nothing drives its jaw into the coral and holds it there
        if (!e.hit) { e.mode = 'stuck'; e.modeT = p3 ? 1.6 : 2.2; SFX.crack(); shakeCam(5); number(e.x + e.face * 40, e.y - 10, 'ITS JAW IS STUCK', '#8fd160'); }
        else { e.mode = 'recoil'; e.modeT = 0.55; } } break; }
    case 'recoil': if (e.modeT <= 0) { e.mode = 'sink'; e.modeT = 0.4; } break;
    case 'thrashTell': if (e.modeT <= 0) { e.mode = 'thrash'; e.modeT = 0.8; e.hit = false; SFX.mawRoar(); shakeCam(4); } break;
    case 'thrash': { const sweep = 100;
      if (!e.hit && !P.dead && Math.abs(P.x - e.x) < sweep && P.y > e.y - 60 && P.y < e.y + 10) { e.hit = true; const res = damagePlayer(e.x, DMG.mawThrash, { up: true }); if (res === 'hit') { P.vx = Math.sign(P.x - e.x || 1) * 260; } }
      if (Math.random() < dt * 40) parts.push({ x: e.x + (Math.random() - 0.5) * sweep * 2, y: e.y - Math.random() * 40, vx: (Math.random() - 0.5) * 120, vy: -40, life: 0.5, max: 0.5, col: '#e8f4f0', size: 2, grav: 60 });
      if (e.modeT <= 0) { e.mode = 'sink'; e.modeT = 0.5; } break; }
    case 'stuck': { e.rise = Math.min(1, e.rise + dt); if (e.modeT <= 0) { e.mode = 'sink'; e.modeT = 0.5; SFX.thud(); } break; }
    case 'reel': if (e.modeT <= 0) { e.mode = 'sink'; e.modeT = 0.45; } break;
    case 'sink': { e.rise = Math.max(0, e.rise - dt * 2.2);
      if (e.modeT <= 0) { setHole(); e.mode = 'lurk'; e.modeT = p3 ? 0.7 : p2 ? 1 : 1.3; } break; }
    case 'dead': return;
  }
  if (e.mode !== 'bite' && e.mode !== 'thrash') e.x += (hx - e.x) * Math.min(1, dt * 6);
  e.y = floor; e.h = 12 + 32 * (e.rise || 0); // down in the hole there is nothing of it to hit
}
...
function updateHerald(e, dt) {
  ...
  const level = [floor - 8, floor - 8, floor - 16, floor - 36][e.phase] || floor - 8;
  if (e.mode === 'sleep') { e.y = heraldGround(e.x); if (pl) poolLevel(pl, floor + 6); return; }
  // the tide: where it should be, eased toward it
  if (pl) { const want = e.mode === 'ebb' || e.mode === 'mired' ? floor + 6 : e.mode === 'dead' ? floor + 6 : level; pl.tideY = pl.tideY === undefined ? floor + 6 : pl.tideY + (want - pl.tideY) * Math.min(1, dt * (e.mode === 'ebb' ? 3 : 1.2)); poolLevel(pl, pl.tideY); }
  if (e.phase === 1 && e.hp < e.maxHp * 0.66) { e.phase = 2; SFX.seaBell(); SFX.boreRoar(); shakeCam(5); number(e.x, e.y - 60, 'THE SEA COMES UP THE SQUARE', '#7cc8c8'); }
  if (e.phase === 2 && e.hp < e.maxHp * 0.33) { e.phase = 3; SFX.seaBell(); SFX.boreRoar(); shakeCam(6); number(e.x, e.y - 60, 'THE SQUARE GOES UNDER', '#7cc8c8'); }
  const surf = pl && !pl.dry ? pl.y : null, gy = heraldGround(e.x); // he stands on the water where there is water over the ground
  const standY = surf !== null && surf < gy ? surf : gy;
  let want = 0;
  switch (e.mode) {
    case 'wake': e.y = standY; if (e.modeT <= 0) { e.mode = 'stride'; e.modeT = 0.8; } break;
    case 'stride': { e.face = Math.sign(d) || e.face; want = ad > 50 ? e.face * (p3 ? 62 : 44) : 0; e.raiseT -= dt; e.sweepT -= dt; e.thrustT -= dt; e.glideT -= dt;
      if (e.modeT <= 0 && !P.dead) {
        if (e.raiseT <= 0) { e.raiseT = p3 ? 9 : 11; e.mode = 'raise'; e.modeT = 1.0; SFX.callerChant(); SFX.seaBell(); number(e.x, e.y - e.h - 12, '!!', '#7cc8c8'); }
        else if (p3 && e.glideT <= 0 && ad > 110) { e.glideT = 5; e.mode = 'glide'; e.modeT = 0.8; e.gx = P.x - Math.sign(d) * 40; SFX.throwWhoosh(); }
        else if (e.sweepT <= 0 && ad < 70) { e.sweepT = p2 ? 2.2 : 2.8; e.mode = 'sweepTell'; e.modeT = 0.6; number(e.x, e.y - e.h - 12, 'LOW', '#ffd36b'); SFX.snort(); }
    ...
    case 'sweepTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'sweep'; e.modeT = 0.3; SFX.slash(); shakeCam(3); if (!P.dead && Math.sign(d) === e.face && ad < 74 && P.y > e.y - 14 && P.y < e.y + 20) { const res = damagePlayer(e.x, DMG.heraldSweep, { unblockable: true }); if (res === 'hit') { P.vx = e.face * 240; P.vy = -120; } } } break;
    case 'sweep': if (e.modeT <= 0) { e.mode = 'rec'; e.modeT = 0.5; } break;
    case 'thrustTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'thrust'; e.modeT = 0.25; SFX.slash(); if (!P.dead && Math.sign(d) === e.face && ad < 116 && Math.abs(P.y - (e.y - 14)) < 28) { const res = damagePlayer(e.x, DMG.heraldThrust); if (res === 'blocked') { e.mode = 'reel'; e.modeT = 1.1; number(e.x, e.y - e.h - 12, 'PARRIED', '#8fd160'); SFX.clank(); } else if (res === 'hit') P.vx = e.face * 220; } } break;
    case 'thrust': if (e.modeT <= 0) { e.mode = 'rec'; e.modeT = 0.45; } break;
    case 'rec': case 'reel': if (e.modeT <= 0) { e.mode = 'stride'; e.modeT = 0.4; } break;
  ...
  if (heraldWave) { const w = heraldWave, crest = (surf !== null ? surf : floor) - 24; w.x += w.dir * 230 * dt; w.crest = crest;
    if (Math.random() < dt * 40) parts.push({ x: w.x, y: crest + Math.random() * 20, vx: w.dir * 120, vy: -50 - Math.random() * 60, life: 0.5, max: 0.5, col: Math.random() < 0.5 ? '#e8f4f0' : '#7cc8c8', size: 2, grav: 300 });
    if (!w.hit && !P.dead && Math.abs(P.x - w.x) < 14 && P.y > crest + 2) { w.hit = true; damagePlayer(w.x - w.dir * 10, DMG.heraldWave, { unblockable: true }); P.vx = w.dir * 260; P.vy = -150; }
    if (w.x < A.x0 || w.x > A.x1) heraldWave = null; }
  e.vx += (want - e.vx) * Math.min(1, dt * 6);
  e.x += e.vx * dt; e.x = Math.max(A.x0 + 18, Math.min(A.x1 - 18, e.x));
  const sy = (surf !== null && surf < heraldGround(e.x)) && e.mode !== 'mired' ? surf : heraldGround(e.x); e.y += (sy - e.y) * Math.min(1, dt * 10);
}
function drawHeraldWave(cx, cy) { const w = heraldWave; if (!w || !L.arena) return; const x = Math.round(w.x - cx), top = Math.round(w.crest - cy), bot = Math.round(L.arena.floor - cy) + 2;
  g.globalAlpha = 0.85; g.fillStyle = '#7cc8c8'; g.beginPath(); g.moveTo(x - w.dir * 30, bot); g.quadraticCurveTo(x - w.dir * 4, top - 2, x + w.dir * 6, top + 4); g.lineTo(x + w.dir * 4, bot); g.closePath(); g.fill(); g.globalAlpha = 1;
  g.fillStyle = '#e8f4f0'; for (let k = 0; k < 5; k++) g.fillRect(x - w.dir * (k * 5) - 1, top + 2 + k * 2, 3, 1); }
...
function updateOwl(e, dt) {
  ...
  const d = P.x - e.x, ad = Math.abs(d); e.hootCd = Math.max(0, (e.hootCd || 0) - dt);
  if (!e.perches) { e.perches = (L.perches || [[54, 12]]).map(([tx, ty]) => ({ x: tx * TS + 8, y: ty * TS })); e.perchI = Math.floor(e.perches.length / 2); e.x = e.perches[e.perchI].x; e.y = e.perches[e.perchI].y; } /* set up even if the fight woke before it ever slept a frame */
  ...
  const dazzle = why => { if (['grounded', 'dazzled', 'descend', 'carry', 'wake', 'sleep', 'crash', 'stuckTalons'].includes(e.mode)) return; e.mode = 'dazzled'; e.vx = 0; e.vy = 0; e.hits = 0; number(e.x, e.y - 24, why, '#ffd36b'); SFX.screech(); SFX.golemShatter(); flash = Math.max(flash, 0.25); shakeCam(5); burst(e.x, e.y - 8, 16, ['#fff6c8', '#ffd36b'], 90, 0.6); };
  if (e.perches) for (const pr of lanterns) { if (pr.perch && pr.lit && !pr.wasLit && ['sit', 'fanTell', 'fan', 'gust', 'screechTell', 'screech', 'hootTell'].includes(e.mode) && Math.abs(pr.x - e.perches[e.perchI].x) < 40) dazzle('THE LIGHT IN ITS EYES'); pr.wasLit = pr.lit; }
  if (L.owlFlare) { const f = L.owlFlare; L.owlFlare = null; if (Math.hypot(f.x - e.x, (f.y - 30) - e.y) < 120) dazzle('BLINDED'); }
  ...
  switch (e.mode) {
    ...
    case 'sit': { e.x = e.perches[e.perchI].x; e.y = e.perches[e.perchI].y; e.face = Math.sign(d) || e.face;
      ...
      break; }
    // THE HOOT: a ring of sound off the branch - it throws you off the perch
    case 'hootTell': e.face = Math.sign(d) || e.face; if (e.modeT <= 0) { e.mode = 'screech'; e.modeT = 0.4; SFX.owlHoot(); ringAt(e.x, e.y - 8, 70, '#dfe8ff', 0.4); shakeCam(3);
      if (!P.dead && Math.hypot(P.x - e.x, (P.y - 8) - e.y) < 70) { const brace = P.block || P.aegis || P.jet; damagePlayer(e.x, DMG.owlHoot, { unblockable: true }); P.vx = (Math.sign(P.x - e.x) || 1) * (brace ? 120 : 300); P.vy = brace ? -60 : -160; P.ground = false; } } break;
    // THE PLUNGE: up out of sight, its shadow finds you on the floor, and it comes straight down. It sticks in the boards.
    ...
    // DAZZLED: the light in its eyes - it drops out of the air and lies on the boards
    case 'dazzled': { e.vy = (e.vy || 0) + 900 * dt; e.y += e.vy * dt; if (e.y >= floor - 8) { e.y = floor - 8; e.vy = 0; e.mode = 'grounded'; e.modeT = p2 ? 3.0 : 3.6; e.stagger = e.modeT; number(e.x, e.y - 24, 'ON THE GROUND: HIT IT', '#8fd160'); SFX.heavy(); shakeCam(4); dust(e.x, floor, 10); } break; }
  ...
  if (e.mode !== 'swoop' && e.mode !== 'crash' && e.mode !== 'carry' && e.mode !== 'dazzled' && e.mode !== 'plunge') e.vy = 0;
  e.x = Math.max(A.x0 + 14, Math.min(A.x1 - 14, e.x));
}
...
const GQ_FRAME = { pinned: 10, gLeapTell: 6, gLeap: 11, gPerch: 3, gDropTell: 6, gDrop: 11, shadowTell: 2, shadow: 2, sleep: 0, wake: 0, court: 0, point: 1, throw: 2, topple: 10, rise: 3, stand: e => Math.abs(e.vx) > 6 ? 4 + Math.floor(e.anim * 5) % 2 : 3, slamTell: 6, slam: 7, slamRec: 7, sweepTell: 3, sweep: 8, rec: 8,
...
// the gallery in three stretches, one to a pillar: the boundaries fall halfway between pillars
function gqSections() { const G = L.arena && L.arena.gallery; const sup = props.filter(p => p.t === 'support').sort((a, b) => a.x - b.x); if (!G) return;
  sup.forEach((p, k) => { const cx = Math.floor(p.x / TS); p.sx0 = k === 0 ? G.x0 : Math.floor((Math.floor(sup[k - 1].x / TS) + cx) / 2) + 1; p.sx1 = k === sup.length - 1 ? G.x1 : Math.floor((cx + Math.floor(sup[k + 1].x / TS)) / 2); p.saved = p.saved || new Map(); }); }
function gqDropSection(pr) { const A = L.arena, G = A && A.gallery; if (!G) return; const keep = A.taken || (A.taken = new Map());
  for (let x = pr.sx0; x <= pr.sx1; x++) { const i = G.row * LW + x; if (L.grid[i] !== T.AIR) { pr.saved.set(i, L.grid[i]); if (!keep.has(i)) keep.set(i, L.grid[i]); L.grid[i] = T.AIR; tileSpr[i] = null; destroyed.add(i); } if (x % 2 === 0) parts.push({ x: x * TS + 8, y: G.row * TS + 4, vx: (Math.random() - 0.5) * 60, vy: 20, life: 1.2, max: 1.2, col: Math.random() < 0.5 ? '#5a5a68' : '#8a5a32', size: 3, grav: 500 }); }
  resolveTiles(); SFX.heavy(); SFX.crack(); SFX.stone(); shakeCam(9); zoomKick(1.08, 0.3); pr.regrowT = 10;
  const l = pr.sx0 * TS - 6, r = (pr.sx1 + 1) * TS + 6;
  if (!P.dead && P.x > l && P.x < r && P.y > G.row * TS) damagePlayer(P.x - 1, 15, { up: true, unblockable: true });
  const q = boss && boss.t === 'gqueen' && boss.alive ? boss : null;
  if (q && q.x > l && q.x < r && q.mode !== 'pinned' && q.mode !== 'shadow') { q.mode = 'pinned'; q.modeT = q.phase >= 3 ? 3.8 : 4.6; q.vx = 0; q.dropT = 0.3; q.hp -= Math.round(q.maxHp * 0.07); q.flash = 0.4; burst(q.x, q.y - 40, 24, ['#5a5a68', '#8a5a32', '#e0b040'], 90, 0.8); gqSay(q, 'THE GALLERY COMES DOWN ON HER', '#8fd160'); SFX.golemShatter(); if (q.hp <= 0) hurtEnemy(q, 1, q.x, false); }
}
function gqRegrow(pr) { const A = L.arena, G = A && A.gallery; if (!G) return;
  for (const [i, t0] of pr.saved) { L.grid[i] = t0; tileSpr[i] = null; destroyed.delete(i); if (A.taken) A.taken.delete(i); }
  pr.saved.clear(); pr.broken = false; pr.hp = 4; pr.shake = 0.3; resolveTiles(); SFX.stone(); SFX.clank();
  for (let x = pr.sx0; x <= pr.sx1; x++) parts.push({ x: x * TS + 8, y: G.row * TS + 6, vx: 0, vy: -24, life: 0.6, max: 0.6, col: '#c9b27c', size: 2, grav: 0 }); }
...
function updateGQueen(e, dt) {
  ...
  const d = P.x - e.x, ad = Math.abs(d);
  let want = 0, physics = e.mode !== 'wake' && e.mode !== 'sleep'; e.armourSaid = Math.max(0, (e.armourSaid || 0) - dt); e.dropT = Math.max(0, (e.dropT || 0) - dt);
  if (e.mode === 'sleep') return;
  // the phases turn on her health
  // (she no longer runs for the roof: the whole fight is in her hall, and it turns on her health)
  if (e.phase === 1 && e.hp <= e.maxHp * 0.66) { e.phase = 2; L.windowsOut = true; SFX.bellow(); shakeCam(6); flash = Math.max(flash, 0.2); gqSay(e, 'THE STORM COMES IN', '#dfe8ff'); }
  if (e.phase === 2 && e.hp <= e.maxHp * 0.33) { e.phase = 3; SFX.bellow(); shakeCam(6); gqSay(e, 'SHE STEPS THROUGH SHADOW', '#c9a0ff'); }
  // THE STORM through the windows, once they are out: a gust down the hall on a beat
  if (e.phase >= 2 && L.windowsOut && e.mode !== 'pinned') { e.gustT -= dt; if (e.gustT <= 0) { e.gustT = 7.5; e.gustDir = Math.random() < 0.5 ? -1 : 1; e.gusting = 2; SFX.thunder(); flash = Math.max(flash, 0.12); }
  ...
  if (e.onRoof && e.mode !== 'ceil' && e.mode !== 'ceilTell' && e.mode !== 'roofWait') { // (the roof phase is retired)
  ...
  switch (e.mode) {
    case 'wake': e.x = e.throneX; e.y = e.throneY; if (e.modeT <= 0) { e.mode = 'rise'; e.modeT = 0.9; SFX.bellow(); gqSay(e, 'THE QUEEN RISES', '#ff6b6b'); } break;
    // ---- phase one: the court ----
    ...
    case 'decreeTell': want = 0; e.face = Math.sign(d) || e.face; if (Math.random() < dt * 30) parts.push({ x: e.x + (Math.random() - 0.5) * 30, y: e.y - 60 - Math.random() * 10, vx: 0, vy: -30, life: 0.4, max: 0.4, col: '#c9a0ff', size: 2, grav: 0, glow: true });
      ...
      } break; // (no guard at the door any more: she fights her own fight)
    ...
    case 'slamRec': case 'rec': case 'dazed': want = 0; if (e.mode === 'dazed' && Math.random() < dt * 8) parts.push({ x: e.x + (Math.random() - 0.5) * 24, y: e.y - 60, vx: 0, vy: -20, life: 0.5, max: 0.5, col: '#ffd36b', size: 2, grav: 0 });
      if (e.modeT <= 0) { e.mode = 'stand'; e.modeT = 0.5; } break;
    // PINNED: her own gallery on top of her. This is the only time a blade gets through the plate.
    case 'pinned': want = 0; e.vx = 0; if (Math.random() < dt * 14) parts.push({ x: e.x + (Math.random() - 0.5) * 34, y: e.y - 40 - Math.random() * 16, vx: 0, vy: -20, life: 0.5, max: 0.5, col: '#ffd36b', size: 2, grav: 0 }); if (e.modeT <= 0) { e.mode = 'stand'; e.modeT = 0.4; SFX.bellow(); shakeCam(3); burst(e.x, e.y - 20, 14, ['#5a5a68', '#8a5a32'], 80, 0.6); } break;
  ...
  if (!physics) return;
  if (e.mode !== 'charge' && e.mode !== 'leap' && e.mode !== 'gLeap' && e.mode !== 'gDrop') e.vx += (want - e.vx) * Math.min(1, dt * 5);
...
function updateRoc(e, dt) {
  ...
  switch (e.mode) {
    ...
    case 'rake': { e.vx = e.rakeDir * 280; e.vy = 0; e.y = floor - 14; if (Math.random() < dt * 30) dust(e.x - e.rakeDir * 10, floor, 2);
      ...
      if ((e.rakeDir > 0 ? e.x > A.x1 - 40 : e.x < A.x0 + 40) || e.modeT <= 0) { e.vx = 0; e.mode = 'rise'; e.modeT = 0.8; } break; }
...
function updateLance(e, dt) {
  ...
  switch (e.mode) {
    ...
    case 'pace': {
      ...
      else if (ad > 110 && (e.galeT ?? 2) <= 0) { e.galeT = 8; e.javT = Math.max(e.javT, 3); e.mode = 'galeTell'; e.modeT = 0.8; number(e.x, e.y - e.h - 14, '!!', '#bfe6f5'); SFX.stormChant && SFX.stormChant(); SFX.bellow(); }
    ...
    case 'charge': {
      ...
      if (edge || e.modeT <= 0 || (e.passT !== -1 && e.passT <= 0)) { e.mode = 'planted'; e.modeT = p2 ? 2.0 : 2.8; e.stagger = e.modeT; e.vx = 0;
    ...
    case 'thrust': if (e.modeT <= 0) { if (e.whiff) { e.mode = 'recover'; e.modeT = 0.75; } else { e.mode = 'pace'; e.modeT = 0.5; } } break;
    ...
    // THE GALE: the pennant goes up and the storm comes down the bridge at his back - it drags you to his point.
    // Brace behind your shield, or get up on a lookout: the towers break the wind. When it drops, he thrusts.
    case 'galeTell': e.face = Math.sign(d) || e.face; want = 0; if (Math.random() < dt * 50) parts.push({ x: e.x + e.face * (60 + Math.random() * 120), y: floor - 4 - Math.random() * 26, vx: -e.face * 160, vy: 0, life: 0.5, max: 0.5, col: '#dfe8ff', size: 1, grav: 0 });
    ...
    case 'whirlTell': want = 0; if (Math.random() < dt * 40) { const a = Math.random() * 6.28; parts.push({ x: e.x + Math.cos(a) * 40, y: e.y - 14 + Math.sin(a) * 18, vx: -Math.cos(a) * 90, vy: -Math.sin(a) * 50 - 20, life: 0.4, max: 0.4, col: '#dfe8ff', size: 1, grav: 0 }); }
      if (e.modeT <= 0) { e.mode = 'whirl'; e.modeT = 0.5; SFX.gust && SFX.gust(); SFX.throwWhoosh(); shakeCam(4); ringAt(e.x, e.y - 14, 56, '#dfe8ff', 0.45);
        if (!P.dead && ad < 56 && Math.abs(P.y - e.y) < 44) { const res = damagePlayer(e.x, DMG.lanceWhirl, { unblockable: true, up: true }); P.vx = (Math.sign(d) || 1) * 320; P.vy = -300; P.ground = false; } } break;
    case 'whirl': want = 0; if (e.modeT <= 0) { e.mode = 'recover'; e.modeT = 0.7; } break;
    ...
    case 'guard': {
      ...
      if (ad < 44 && e.whirlT <= 0) { e.whirlT = 7; e.mode = 'whirlTell'; e.modeT = 0.65; number(e.x, e.y - e.h - 12, '!!', '#bfe6f5'); SFX.snort(); }
    ...
    case 'rush': {
      ...
      if (e.modeT <= 0) { e.mode = e.hitT > 0 ? 'guard' : 'stumble'; e.modeT = e.hitT > 0 ? 0.5 : 1.1; e.vx = e.face * 40; if (e.mode === 'stumble') { SFX.heavy(); dust(e.x + e.face * 10, e.y, 6); } }
      break; }
    ...
    case 'guardSwing': if (e.modeT <= 0) { if (e.whiff) { e.mode = 'recover'; e.modeT = 0.75; } else { e.mode = 'guard'; e.modeT = 0.4; } } break;
  }
  if (e.mode !== 'charge' && e.mode !== 'rush' && e.mode !== 'vault') e.vx += (want - e.vx) * Math.min(1, dt * (e.mode === 'reel' || e.mode === 'stumble' ? 2 : 5));
  const r = moveBody(e, e.vx * dt, e.vy * dt, false); if (r.ground) e.vy = 0;
  e.x = Math.max(A.x0 + 18, Math.min(A.x1 - 18, e.x));
  if (e.mode === 'vault' && r.ground && e.airT > 0.2) { e.mode = 'planted'; e.modeT = p2 ? 1.5 : 1.9; e.stagger = e.modeT; e.vx = 0;
...
const TAM_LINES = {
  ...
  reef: [['THE TRIBUTE NEVER STOPPED BEING SENT. IT STOPPED ARRIVING. THE SHIP IS OUT ON THAT REEF WITH THE SEALS STILL IN HER.', 'THE WATER IS OVER MY HEAD OUT THERE, SIR. I WILL HOLD THE BOAT.'],
         ['THREE SEALS, AND THE WAX STILL GOOD.', 'THE REEF IS QUIET NOW. THERE ARE LIGHTS UNDER IT, DEEPER DOWN, IN ROWS. LIKE A STREET.']],
};
...
const TAM_QUEST = {
  ...
  reef: [['THE TRIBUTE SHIP BROKE OVER THIS REEF WITH THREE ROYAL SEALS ABOARD. ONE IS IN HER HOLD, ONE IS OUT ON THE SHELF, ONE IS IN THE STERN CABIN.'], n => ['THAT IS ' + n + ' OF THE THREE SEALS. THE WAX IS STILL GOOD.'], ['ALL THREE. THE DIVERS LEFT THIS ON THE BELL: IT BURNS UNDER WATER.']],
};
const TAM_MAP = {
  ...
  storm: ['HER KNIGHT HOLDS THE BRIDGE.', 'HER CASTLE IS ABOVE US.'], crown: ['THE QUEEN. THE LAST OF THEM.', 'IT IS DONE. WE CAN GO HOME.'], longwater: ['THE RIVER RUNS SALT.', 'THE GLAIVE POINTS OUT TO SEA.'], reef: ['THE FLEET IS ON THE REEF.', 'THREE SEALS, AND SOMETHING IN THE HOLE.'],
};
const NPC_LINES = pr => {
...
function resetCastle() {
  ...
  for (const pr of props) if (pr.t === 'support') { pr.broken = false; pr.hp = 4; pr.regrowT = 0; pr.creakT = 0; if (pr.saved) pr.saved.clear(); } // and her pillars stand again
...
function drawSlick(cx, cy) {
  for (const z of (L.slick || [])) { const x0 = z[0] * TS - cx, x1 = (z[1] + 1) * TS - cx, y = z[2] * TS - cy; if (x1 < 0 || x0 > VW || y < -8 || y > VH + 8) continue;
    ...
    const gl = x0 + ((time * 60 + z[0] * 37) % Math.max(20, x1 - x0 + 40)) - 20; if (gl > x0 && gl < x1 - 6) { g.fillStyle = '#ffffff'; g.fillRect(Math.round(gl), Math.round(y) + 1, 6, 1); } } }
...
function updateCastleProps(dt, hb) {
  for (const pr of props) {
    ...
    if (pr.t === 'support') { if (pr.sx0 === undefined) gqSections();
      if (pr.creakT > 0) { pr.creakT -= dt; if (Math.random() < dt * 40) parts.push({ x: (pr.sx0 + Math.random() * (pr.sx1 - pr.sx0 + 1)) * TS, y: pr.top + 2, vx: 0, vy: 30, life: 0.5, max: 0.5, col: '#8a8a98', size: 1, grav: 300 }); if (pr.creakT <= 0) gqDropSection(pr); }
      else if (pr.broken) { if (pr.regrowT > 0 && bossActive) { pr.regrowT -= dt; if (pr.regrowT <= 0) gqRegrow(pr); } }
      else { if (pr.shake > 0) pr.shake -= dt;
...
const GOBLINISH = new Set(['soldier', 'javelin', 'heavy', 'sprig', 'thief', 'sapper', 'archer', 'pike', 'shield', 'brute', 'hearthgob', 'miner', 'sentry', 'sweep', 'thorn', 'rockgoblin', 'snuffer', 'cutter', 'horn', 'shaman', 'stormshaman', 'master', 'sailer']);
const FLYERS = new Set(['wasp', 'bat', 'harpy', 'crow', 'drone', 'kite', 'sailer']);
const HEAVY = new Set(['heavy', 'brute', 'hearthgob', 'troll', 'master', 'shield', 'goat', 'greathound']);
...
function updateEnemies(dt) {
  for (const e of enemies) {
    ...
    if (e.t === 'soldier' || e.t === 'javelin' || e.t === 'heavy') { updateTroop(e, dt); continue; }
    if (e.t === 'sailor' || e.t === 'netter' || e.t === 'urchin' || e.t === 'angler' || e.t === 'petrel') { updateReef(e, dt); continue; }
...
function castJudgement() { trialEvent('judgement'); P.light = 0; P.blastT = 0.5; P.inv = Math.max(P.inv, 0.6); P.vx = 0; SFX.judgement(); shakeCam(9); zoomKick(1.12, 0.35); killFlash = 0.035; hitstop(0.08); // a white flash: the red one is for being hurt
  ...
  for (const e of enemies) if (e.alive && !e.harmless && Math.abs(e.x - P.x) < 110 && Math.abs(e.y - P.y) < 80) { bolts.push({ x: e.x, y: e.y, life: 0.45, storm: true, holy: true }); hurtEnemy(e, e.maxHp ? 20 : 30, P.x, true); if (!e.maxHp && e.alive) e.stagger = Math.max(e.stagger || 0, 1.2); if (tal('smite') && e.alive) e.burn = Math.max(e.burn || 0, 3); if (tal('zeal') && P.hp > 0) { P.hp = Math.min(P.maxHp, P.hp + 4 * tal('zeal')); number(P.x, P.y - 30, '+' + 4 * tal('zeal'), '#8fd160'); } } // ZEAL
}
function bankHeat() { if (P.full) return; trialEvent('heat'); if (tal('backdraft')) { fireRings.push({ x: P.x, y: P.y - 8, r: 6, hit: new Set() }); SFX.puff(); shakeCam(3); } // BACKDRAFT
  P.full = true; P.fullT = tal('blaze') ? 10 : 5; SFX.heatFull(); ringAt(P.x, P.y - 10, 18, '#ffd36b', 0.3); burst(P.x, P.y - 10, 10, ['#ffd36b', '#fff6c8'], 50, 0.4, -40, 1); }
function castPyre() {
...
function updateProps(dt) {
  ...
  if (meteors.length || fireRings.length || lanceBeams.length) updateSkillFx(dt);
...
function drawFalls(cx, cy) {
  for (const f of (L.falls || [])) { const x0 = Math.round(f.x0 - cx), w = Math.round(f.x1 - f.x0), y0 = Math.round(f.y0 - cy), h = Math.round(f.y1 - f.y0); if (x0 > VW || x0 + w < 0 || y0 > VH || y0 + h < 0) continue;
    ...
    g.globalAlpha = 1;
    if (Math.random() < 0.3) parts.push({ x: f.x0 + Math.random() * (f.x1 - f.x0), y: f.y1 - 4, vx: (Math.random() - 0.5) * 50, vy: -30 - Math.random() * 40, life: 0.5, max: 0.5, col: '#e8f4f0', size: 1, grav: 120 }); } }
...
let bore = null;
function updateBore(dt) {
  const B = L.bore; if (!B || P.dead) { if (!B) bore = null; return; }
  if (!bore) bore = { t: B.period * 0.5, x: null };
  if (bore.x === null) { bore.t -= dt; const near = P.x > B.x0 - 200 && P.x < B.x1 + 100;
    if (bore.t < 2 && !bore.warned && near) { bore.warned = true; SFX.boreRoar(); shakeCam(2); }
    if (bore.t <= 0) { bore.x = B.x1; bore.warned = false; bore.hit = false; if (near) SFX.waveCrash(); } return; }
  bore.x -= B.speed * dt; const crest = B.surface - B.h;
  if (Math.random() < dt * 40) parts.push({ x: bore.x + Math.random() * 10, y: crest + Math.random() * B.h, vx: -B.speed * 0.6, vy: -60 - Math.random() * 60, life: 0.5, max: 0.5, col: Math.random() < 0.5 ? '#e8f4f0' : '#7cc8c8', size: 2, grav: 300 });
  const raft = movers.find(m => m.kind === 'raft' && m.x < bore.x + 8 && m.x + m.w > bore.x - 8); if (raft && !raft.bored) { raft.bored = true; raft.x = Math.max(raft.x0, raft.x - 36); SFX.splash(); } // it lifts the raft and throws it back a little
  if (!bore.hit && !P.dead && Math.abs(P.x - bore.x) < 14 && P.y > crest + 2 && P.y < B.surface + 60 && !(P.onMover && P.onMover.kind === 'raft')) { bore.hit = true; damagePlayer(bore.x + 20, 12, { unblockable: true }); P.vx = -240; P.vy = -120; number(P.x, P.y - 24, 'THE BORE', '#7cc8c8'); }
  if (bore.x < B.x0) { bore.x = null; bore.t = B.period; for (const m of movers) m.bored = false; }
}
function drawBore(cx, cy) {
  const B = L.bore; if (!B || !bore) return;
  if (bore.x === null) { if (bore.t < 2) { const k = 1 - bore.t / 2, x = Math.round(B.x1 - cx); g.globalAlpha = 0.3 + 0.5 * k; g.fillStyle = '#e8f4f0'; g.fillRect(Math.max(0, Math.min(VW - 3, x)), Math.round(B.surface - cy) - 4 - Math.round(k * 6), 3, 6 + Math.round(k * 6)); g.globalAlpha = 1; } return; } // the white line coming down the river
  const x = Math.round(bore.x - cx), top = Math.round(B.surface - B.h - cy), bot = Math.round(B.surface - cy) + 6; if (x < -40 || x > VW + 40) return;
  const gr = g.createLinearGradient(x, 0, x + 40, 0); gr.addColorStop(0, '#e8f4f0'); gr.addColorStop(0.2, '#7cc8c8'); gr.addColorStop(1, 'rgba(46,122,136,0)');
...
function drawWater(cx, cy, surfaceOnly = false) {
  for (const p of (L.pools || [])) {
    ...
    if (!surfaceOnly) {
      if (p.capped || p.streetTide) continue; // a room with rock over it, or a tide standing over real ground: the water goes OVER what is in it, or it would be a blue rectangle
      if (p.shallow) {
        const d = p.depth || 22;
    ...
    if (p.swim && !p.shallow) { g.globalAlpha = p.capped ? 0.42 : p.streetTide ? 0.46 : 0.5; g.fillStyle = sea ? '#2e7a88' : '#3b7fae'; g.fillRect(x0, y + 3, x1 - x0, h - 3); g.globalAlpha = 1;
      if (p.capped || p.streetTide) { const gr2 = g.createLinearGradient(0, y, 0, y + h); gr2.addColorStop(0, 'rgba(10,26,32,0.45)'); gr2.addColorStop(0.3, 'rgba(10,26,32,0)'); gr2.addColorStop(1, 'rgba(6,16,22,0.5)'); g.fillStyle = gr2; g.fillRect(x0, y, x1 - x0, h); } } // over whoever is swimming in it: they are IN the water
...
function drawWorld(cx, cy, showPlayer) {
  ...
  for (const pr of props) {
    ...
    if (pr.t === 'towertop') g.drawImage(PROP.towertop, Math.round(pr.x) - 16 - cx, Math.round(pr.y) - 18 - cy);
      ...
      if (pr.hang && (pr.state !== 'down' || pr.downT < 1.2)) { const sh = pr.state === 'tell' ? Math.round(Math.sin(time * 70) * 2) : 0; g.drawImage(SPR.chandelier, x + sh - Math.round(SPR.chandelier.width / 2), by - SPR.chandelier.height); }
        ...
        if (pr.regrowT > 0) { const k = 1 - pr.regrowT / 10, hh = Math.round((yb - 8 - yt) * k); g.fillStyle = '#62626e'; g.fillRect(x - 5, yb - 8 - hh, 10, hh); g.strokeStyle = '#8a5a32'; g.lineWidth = 1; g.strokeRect(x - 7.5, yt + 0.5, 15, yb - yt - 1); for (let yy = yt + 6; yy < yb; yy += 10) { g.beginPath(); g.moveTo(x - 7, yy); g.lineTo(x + 7, yy + 8); g.stroke(); } } } /* the masons are at it: a scaffold, and the pillar rising in it */
      else if (pr.sx0 !== undefined && boss && boss.t === 'gqueen' && boss.alive && bossActive && boss.x > pr.sx0 * TS - 6 && boss.x < (pr.sx1 + 1) * TS + 6 && boss.mode !== 'pinned') { const k2 = 0.5 + 0.5 * Math.sin(time * 8); g.globalAlpha = 0.35 + 0.45 * k2; g.strokeStyle = '#ffd36b'; g.lineWidth = 1; g.strokeRect(x - 8.5, yt - 1.5, 17, yb - yt + 2); g.globalAlpha = 1; } /* she is under this one: strike it */
      if (!pr.broken) { for (let yy = yt; yy < yb; yy += 8) { g.fillStyle = (yy / 8) % 2 ? '#5a5a68' : '#62626e'; g.fillRect(x - 5, yy, 10, Math.min(8, yb - yy)); g.fillStyle = '#3a3a44'; g.fillRect(x - 5, yy, 10, 1); } g.fillStyle = '#6e6e7a'; g.fillRect(x - 7, yt, 14, 3); g.fillRect(x - 7, yb - 3, 14, 3); if (pr.hp < 4) { g.strokeStyle = '#1b1626'; g.lineWidth = 1; g.beginPath(); g.moveTo(x - 3, yt + 10); for (let k = 0; k < 4 - pr.hp; k++) g.lineTo(x + (k % 2 ? 3 : -2), yt + 16 + k * 9); g.stroke(); } } }
  ...
  for (const c of corpses) {
    ...
    else if (SPR[c.t] || c.t === 'hopper') drawRot(c.t === 'hopper' && c.color && c.color !== 'green' ? SPR['hopper_' + c.color] : SPR[c.t], c.frame, c.x - cx, c.y - cy, c.face, c.rot, al);
  }
  for (const e of enemies) {
    ...
    else if (e.t === 'owl') frame = e.mode === 'crash' || e.mode === 'grounded' || e.mode === 'stuckTalons' || e.mode === 'dazzled' ? 4 : e.mode === 'hootTell' ? 3 : e.mode === 'plunge' ? 6 : e.mode === 'riseUp' || e.mode === 'stalk' ? 1 + Math.floor(e.anim * 9) % 2 : e.mode === 'screech' || e.mode === 'screechTell' ? 3 : e.mode === 'sit' || e.mode === 'sleep' || e.mode === 'wake' || e.mode === 'fanTell' || e.mode === 'fan' ? 0 : e.mode === 'swoop' ? 6 : e.mode === 'land' ? 7 : e.mode === 'takeoff' ? (Math.floor(e.anim * 14) % 2 ? 1 : 2) : e.mode === 'fly' ? (e.modeT > 2.4 ? 1 + Math.floor(e.anim * 9) % 2 : 5) : e.mode === 'carry' ? 1 + Math.floor(e.anim * 8) % 2 : 1 + Math.floor(e.anim * 8) % 2;
    ...
    else if (e.t === 'sailor') frame = e.mode === 'hookTell' ? 5 : e.mode === 'hook' ? 6 : e.guardT > 0 ? 4 : Math.abs(e.vx) > 4 ? Math.floor(e.anim * 6) % 4 : 0;
    else if (e.t === 'netter') frame = e.mode === 'castTell' ? 4 : e.mode === 'cast' ? 5 : !e.hasNet ? 6 : Math.abs(e.vx) > 4 ? Math.floor(e.anim * 8) % 4 : 0;
    else if (e.t === 'urchin') frame = e.bristle ? 3 : Math.floor(e.anim * 3) % 3;
    else if (e.t === 'angler') frame = e.mode === 'bite' ? 3 : e.mode === 'biteTell' ? 2 : Math.floor(e.anim * 3) % 2;
    else if (e.t === 'petrel') frame = e.mode === 'dive' ? 2 : e.mode === 'climb' ? 3 : Math.floor(e.anim * 8) % 2;
    else if (e.t === 'reefmaw') frame = ({ lurk: 0, riseTell: 1, rise: 2, biteTell: 3, bite: 4, thrash: 5, thrashTell: 5, stuck: 6, reel: 6, recoil: 6, sink: 7, dead: 8 })[e.mode] ?? (e.rise > 0.5 ? 2 : 0);
    else if (e.t === 'herald') frame = ({ sweepTell: 3, sweep: 4, thrustTell: 5, thrust: 6, raise: 7, wave: 7, ebb: 8, mired: 8, reel: 8, dead: 9 })[e.mode] ?? (Math.abs(e.vx) > 6 ? 1 + Math.floor(e.anim * 4) % 2 : 0);
    ...
    else if (e.t === 'heavy') frame = e.mode === 'raise' ? 4 : (e.mode === 'slam' || e.mode === 'sweep' || (e.mode === 'rest' && e.modeT > 0.5 && !(e.parried > 0))) ? 5 : (e.mode === 'windUp' || e.parried > 0) ? 6 : Math.abs(e.vx) > 4 ? Math.floor(e.anim * 5) % 4 : 0;
  ...
  drawReflections(cx, cy); drawWater(cx, cy, true); drawFalls(cx, cy); drawBore(cx, cy); drawHeraldWave(cx, cy); drawBreath(cx, cy);
  ...
  drawBloom(cx, cy); drawWindFx(); drawSpiderSigns(cx, cy); drawSlick(cx, cy); drawSkillFx(cx, cy);
...
function drawControls() {
  ...
  const rows = [['move', 'ARROWS / WASD', 'STICK'], ['jump', SET.swapZX ? 'X / SPACE' : 'Z / SPACE', 'A'], ['swing', SET.swapZX ? 'Z / J' : 'X / J', 'X'], ['plunge', 'DOWN+SWING IN AIR', 'DOWN+X'], ['block', 'C / L ' + (SET.blockToggle ? 'TOGGLE' : 'HOLD'), 'LB RB'], ['dodge', 'V / SHIFT', 'B'], ['skill', 'F / B (equipped)', 'Y'], ['skill two', 'G / N (equipped)', 'RT'], ['talk', 'E / T (signs, folk)', 'D-PAD UP'], ['pause', 'ESC / P', 'START'], ['drop', 'DOWN ON A LEDGE', 'DOWN'], ['shrine', 'R (RETURN)', '']];
