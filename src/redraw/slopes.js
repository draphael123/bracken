...
const T = 16;
const SAND = { crust: '#f6e2ae', crustL: '#fff1c8', lit: '#e8c784', base: '#dcb676', mid: '#cc9f62', deep: '#b8894f', band: '#a87a46', dark: '#8c6238', ripple: '#bf955c', fill: '#d0a66a', grain: '#c49860', pebble: '#8a7d6e', pebbleL: '#b0a491', root: '#6e5236' };
...
function sandAt(depth, x, rnd, shade) {
  if (depth < 1) return shade ? SAND.crust : SAND.crustL;
  if (depth < 2) return SAND.crust;
  const d = depth + Math.sin(x * 0.7) * 0.6;
  let c = d < 4 ? (shade ? SAND.base : SAND.lit) : d < 6.5 ? SAND.base : SAND.fill;
  if (rnd() < 0.07) c = c === SAND.lit ? SAND.base : c === SAND.base ? SAND.fill : SAND.grain;   // grain
  return c;
}
/* one tile: `surf(x)` = the surface in the tile (px down from its top) at column x; `shade` = the face turned from the sun */
function sandTile(seed, surf, shade, rippleDir) {
  const rnd = mulberry(seed); const [c, g] = canvas(T, T);
  for (let x = 0; x < T; x++) { const s = surf(x + 0.5), top = Math.ceil(s - 0.001);
    for (let y = Math.max(0, top); y < T; y++) px(g, x, y, sandAt(y - s, x, rnd, shade)); }
  ...
  for (let i = 0; i < 3; i++) { const x0 = (rnd() * (T - 4)) | 0, dep = 3 + ((rnd() * 4) | 0), len = 2 + ((rnd() * 3) | 0);
    for (let k = 0; k < len; k++) { const x = x0 + k; if (x >= T) break; const y = Math.round(surf(x + 0.5) + dep); if (y < T) px(g, x, y, SAND.ripple); } }
  // a pebble, sometimes a root, deep in the fill
  if (rnd() < 0.5) { const x = 2 + ((rnd() * 11) | 0), y = Math.max(Math.ceil(surf(x + 0.5)) + 6, 10 + ((rnd() * 4) | 0)); if (y < T - 1) { px(g, x, y, SAND.pebble); px(g, x + 1, y, SAND.pebble); px(g, x, y - 1, SAND.pebbleL); } }
  if (rnd() < 0.25) { const x = 1 + ((rnd() * 13) | 0); let y = Math.ceil(surf(x + 0.5)) + 3; for (let k = 0; k < 4 && y < T; k++, y++) px(g, x + (k >> 1), y, SAND.root); }
  return c;
}
export function bakeSandSlopes() {
  const out = {};
  for (const kind of Object.values(SLOPE)) {
    const shade = slopeRise(kind) < 0;   // rising to the left: turned from a sun in the upper left
    out[kind] = [0, 1, 2].map(i => sandTile(9100 + kind * 7 + i * 3, x => heightAt(kind, x), shade, slopeRise(kind)));
  }
  ...
  out.under = {};
  for (const kind of Object.values(SLOPE)) out.under[kind] = [0, 1, 2].map(i => sandTile(9200 + kind * 7 + i * 3, x => heightAt(kind, x) - T, slopeRise(kind) < 0, 0));
  out.top = [0, 1, 2].map(i => sandTile(9300 + i * 3, () => 0, false, 0));      // the flat beside a slope: the same skin
  out.fill = [0, 1, 2].map(i => sandTile(9400 + i * 3, () => -12, false, 0));   // sand under sand: the fill colour and its grain, no skin
  return out;
}
