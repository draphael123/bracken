...
function bakeRoom(st, w, h, tx0, ty0) {
  const [c, g] = canvas(w, h), r = mulberry(tx0 * 131 + ty0 * 17 + st.length), holes = [], chains = [], wind = [];
  ...
  if (st === 'reading') for (const fx of [0.25, 0.7]) { const wx = w * fx, wy = h * 0.2, ww = 54, wh = h * 0.5;
  ...
  if (st === 'clock') { const ccx = w / 2, ccy = h * 0.35, R = Math.min(96, h * 0.3);
  ...
  if (st === 'dome') { hole(g, r, w * 0.3, h * 0.25, w * 0.22, h * 0.2, holes); hole(g, r, w * 0.72, h * 0.3, w * 0.18, h * 0.24, holes); }
  /* THE HOLES the tower has taken, and its cracks */
  const nh = st === 'dome' ? 1 : st === 'reading' ? 1 : 2 + Math.floor(r() * 2);
  for (let k = 0; k < nh; k++) hole(g, r, w * (0.1 + k / nh * 0.8 + r() * 0.1), h * (0.1 + r() * 0.45), 30 + r() * 34, 22 + r() * 24, holes);
  ...
  return { canvas: c, wind, chains };
}
export function paintFallenRoom(g, st, sx, sy, w, h, tx0, ty0, time = 0) {
  const key = st + ':' + tx0 + ':' + ty0 + ':' + w + 'x' + h; let R = cache.get(key); if (!R) { R = bakeRoom(st, w, h, tx0, ty0); cache.set(key, R); }
