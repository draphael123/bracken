...
export function polishCoastAndTown(L,id,T){
 if(id==='reef'){
  ...
  for(const [x,y,kind,v]of[[135,31,'coiledCable',0],[180,31,'seaChest',0],[205,31,'rumBarrels',0],[229,36,'brainCoral',1],[277,36,'coralFan',2],[314,36,'kelpTall',1],[368,23,'figurehead',0]])L.ents.push({t:'deco',x,y,kind,v});
 }
 if(id==='waymeet'){
 ...
 if(id==='reef'||id==='waymeet'){
  L.routeSupports=[];const at=(x,y)=>L.grid[y*L.W+x];
  for(let y=id==='waymeet'?25:0;y<L.H-1;y++)for(let x=1;x<L.W-1;x++)if(at(x,y)===T.ONEWAY&&(at(x-1,y)!==T.ONEWAY||at(x+1,y)!==T.ONEWAY)){
   let b=y+1;while(b<L.H-1&&![T.SOLID,T.PLANK,T.ONEWAY].includes(at(x,b)))b++;
   if(b-y>1&&b-y<=24)L.routeSupports.push({x,y,bottom:b});
  }
 }
 return L;
}
