...
export function updateBuriedDead(e,dt,c){
 ...
 if(m==='eruptTell'){if(Math.abs(P.x-e.markX)<42&&P.y>A.floor-90)hit(e.markX,26,true);sound('heavy');}
 rest();
}
export function updateZombie(e,dt,c){
 const{P,move,hit,snare,say,solid}=c;e.anim+=dt;e.modeT-=dt;e.vx=0;
 ...
 e.vx=solid(e.x+e.face*16,e.y+4)?e.face*25:0;if(move(e,e.vx*dt,e.vy*dt)?.ground)e.vy=0;
}
export function deadFrame(e){return e.mode==='burrow'||e.mode==='eruptTell'||e.mode==='buried'||e.mode==='riseTell'?6:e.open>0?5:e.mode==='slamTell'?3:e.mode==='callTell'?4:e.mode?.endsWith('Tell')?2:Math.abs(e.vx)>2?Math.floor(e.anim*5)%2:0;}
export function bakeDead(big=false){
 const R=[],w=big?78:26,h=big?98:32,s=big?3:1;
 ...
 R.push(outline(c));}
 const W=R.map(c=>whiten(c));return{R,L:R.map(flipX),white:{R:W,L:W.map(flipX)},ax:big?39:13,ay:big?95:32,w,h};
}
