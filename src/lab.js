...
export async function bossLab(BK, opts = {}) {
  ...
  for (const lvId of bosses) for (const h of heroes) {
    ...
    for (; f < maxF && boss.alive; f++) {
      ...
      const rushing = (boss.mode === 'rush' || (boss.t === 'masthead' && boss.mode === 'sail')) && ad < 46;   /* THE RAM is answered as it arrives, like the shoulder */
