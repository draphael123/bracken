...
// ITS MODE TIMER IS `modeT`, NOT `t`. This module was written pure and used e.t, and the game uses e.t for a creature's
// TYPE: wired in, the first frame did `'tome' - dt` and every book in the tower turned into a NaN that nothing updated
// or drew. modeT is what every other creature in main.js calls this, and it costs nothing to agree with them.
...
export function newTome(x, y) { return { x, y, hx: x, hy: y, hp: TOME.hp, mode: 'drift', modeT: 0, cd: 0.5 + Math.random() * 0.5, face: 1, lock: null, vx: 0, vy: 0, anim: Math.random() * 6, hitMult: 1, alive: true }; }
...
export function tomeStep(e, w, dt, token) {
  ...
  e.modeT -= dt; e.cd -= dt; e.anim += dt; const dx = w.px - e.x, dy = (w.py - 8) - e.y, d = Math.hypot(dx, dy);
  ...
  switch (e.mode) {
    case 'drift': {   /* riffling near its shelf, turning to face you */
      ...
      if (d < TOME.notice && e.cd <= 0 && (!token || !token.holder)) { if (token) token.holder = e; e.mode = 'tell'; e.modeT = TOME.tell; e.lock = { x: w.px, y: w.py - 8 }; ev('tell', { mark: '!', lock: e.lock }); }
      break; }
    case 'tell': if (e.modeT <= 0) { const lx = e.lock.x - e.x, ly = e.lock.y - e.y, n = Math.hypot(lx, ly) || 1; e.vx = lx / n * TOME.dartSpeed; e.vy = ly / n * TOME.dartSpeed; e.mode = 'dart'; e.modeT = Math.min(TOME.dartMax, n / TOME.dartSpeed + 0.04); ev('dart'); } break;
    case 'dart': e.x += e.vx * dt; e.y += e.vy * dt; ev('hit', { box: tomeBox(e), blockable: true, dmg: TOME.dmg });
      if (e.modeT <= 0) { e.mode = 'spent'; e.modeT = TOME.spent; e.vx = e.vy = 0; release(); ev('spent'); } break;
    case 'spent': e.y += 10 * dt; if (e.modeT <= 0) { e.mode = 'drift'; e.cd = TOME.cool; } break;
    case 'shut': { const fy = w.floorY ? w.floorY(e.x) : e.y; if (e.y < fy) e.y = Math.min(fy, e.y + 220 * dt); if (e.modeT <= 0) { e.mode = 'drift'; e.cd = TOME.cool; e.hitMult = 1; } break; }
  }
  return out;
}
/* THE SHIELD SHUTS IT: call when its dart is blocked (perfect: on the beat) */
export function tomeBlocked(e, perfect, token) { if (e.mode !== 'dart') return; e.mode = 'shut'; e.modeT = perfect ? TOME.shutPerfect : TOME.shut; e.vx = e.vy = 0; e.hitMult = TOME.shutMult; if (token && token.holder === e) token.holder = null; }
