...
/* a selection may name a level's mini as id:mini */
const ids = sel ? sel.split(',').filter(s => !s.endsWith(':mini')) : LEVELS.filter(l => (!l.hidden || l.secret) && l.build().arena).map(l => l.id);
const minis = sel ? sel.split(',').filter(s => s.endsWith(':mini')).map(s => s.slice(0, -5)) : LEVELS.filter(l => (!l.hidden || l.secret) && l.build().mini).map(l => l.id);
const jobs = [...ids.map(id => [id, false]), ...minis.map(id => [id, true])];
/* SEEDS=1919,7,42 MODES=normal: several dice for one fight - one seed is one story, and a knight's new throw moves the dice */
const SEEDS = (process.env.SEEDS || '1919').split(',').map(Number), MODES = (process.env.MODES || 'refill,normal').split(',');
...
try {
  ...
  for (const [id, mini] of jobs) for (const mode of MODES) for (const seed0 of SEEDS) {
    ...
    const row = await pg.evalp(`(async()=>{BK.manualSimulation=true;
      ...
      const random=Math.random;let seed=${seed0};Math.random=()=>{seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296;};
      let par=0,blk=0;
    ...
    const r = { id, mini, mode, seed: seed0, killed: row.killed, secs: row.secs, outcome: row.outcome, hpLeft: row.hpLeftPct, taken: row.takenPerMin, skipped: row.skipped, error: row.error, parries: row.parries, blocks: row.blocks };
    rows.push(r); writeFileSync(out, JSON.stringify(rows, null, 1));
    console.log((id + (mini ? '(mini)' : '')).padEnd(18), mode.padEnd(7), String(seed0).padEnd(5), r.skipped ? 'skipped ' + r.skipped : r.error ? 'ERROR ' + r.error : (r.killed ? 'KILLED ' + r.secs + 's' : 'alive ' + r.hpLeft + '%') + ' ' + (r.outcome || ''));
  }
