...
1. HEAVY (UPHEAVAL, hold X) - CHARGE SETS DISTANCE, AND CLOSE FOES MUST BE HITTABLE. Today upheavalX() = 26 + 40*wound px ahead, so even a tap lands a body-length away and a foe touching her is never hit. New: a quick release = a stone spike at her front foot that hits a foe in contact (it must not trap her in rock: A12/freeCell); holding walks the eruption point out to ~2x today's max; the floor marker keeps showing where it will come up while charging (C1). ALSO (Daniel): the pillar SHATTERS ALMOST IMMEDIATELY - it erupts, launches, and breaks up about 0.4 s later (a crack frame, then a burst of shards), instead of standing ~4 s as a platform. Report what this costs: any Geomancer-only shortcut or check that relied on standing on a pillar (STONE STEP / ARCHWAY are separate pieces and keep their own lifetimes). Assert: a foe at contact range is hit by a minimal charge; a full charge reaches the new max; the pillar is gone within 0.5 s.
...
## Lane 6: One dodge instead of two

Start this AFTER Lane 5 has pushed (it builds on claude/geo2).

```text
You are a second-PC lane (ONE DODGE) for BRACKEN (16-bit platformer, plain JS, no build step, no dependencies).
SETUP: this is a second computer (Windows or Mac; use its shell). In a fresh folder: `git clone https://github.com/draphael123/bracken.git bracken-dodge && cd bracken-dodge && git checkout -b claude/onedodge origin/master && git merge origin/claude/geo2`. Node 24 and Chrome are installed (do not download anything). At most one Chrome page at a time. Push ONLY to claude/onedodge after every green commit. NEVER touch master, NEVER run the full `npm run check`, never deploy. Level editor and Boss Rush are PARKED. Read docs/AGENT-HANDOFF.md "EXPENSIVE LESSONS". Patches: exact-match replacements; /* */ not // in patch strings; commit messages say WHY; re-run failing checks alone; prove every new assertion red first.
DANIEL'S DECISION: "we have a dodge roll... why not just make it a standard dodge like when you double tap the keys. Doesn't really make sense to have two separate dodges." Today there are two moves in src/main.js: THE DASH (double-tap a direction, ~6978: distance, NO invulnerability, works in the air once per landing, feeds the DASH ATTACK / DASH_STRIKE and the Geomancer's ROLLING STONE) and THE DODGE (V/SHIFT or pad B, ~7000: a roll with invulnerable grace, ground-only unless a passive, per-hero: the Warden's back-step, the Geomancer's BURROW from lane 5).
THE JOB: MERGE THEM INTO ONE MOVE, triggered by double-tapping a direction. Keep the V/SHIFT / pad-B button as a second input for THE SAME move (players on a pad cannot double-tap a stick reliably, and every tell in the game was timed for an instant press - say so in the controls text). The merged move: goes the way you tap (the button goes the way you face or hold), has the dodge's short invulnerable grace at its start, keeps the dash's air use (once per landing) and its DASH ATTACK / ROLLING STONE follow-ups, one stamina cost, one cooldown. Each hero's flavour becomes their version of it (Warden back-step, Pyromancer scorch, Paladin shoulder, Pirate rum, Death Knight long passing, Geomancer BURROW - floor-only, so in the air she falls back on a plain dash). Passives that touched either move (airDash, airRoll, swash, lightFoot, heavyTread, longPassing, VAULTER...) must still mean something: list each with its new wording. Update the trials (THE DODGE / THE DASH ATTACK), hints, the controls screen, and the in-page bots (apress('dodge')) so the pilots still play.
Measure before/after with the lab (dice pinned): if the merge moves any boss's win rate by more than 10 points, report it - do not retune bosses in this lane.
Keep green: combat-feel, attack-buffer, attack-animation, starter-kits, ability-poses, skill-passives, talents, levelling, progression, reaper-input, knight-rework, geomancer, pilot-actions, boss-openings, boss-fight-end, combat-replay, tells, textfit, skill-menu, comments, syntax.
FINAL MESSAGE: shas, the per-hero table (what their one dodge now is), every passive's new wording, lab before/after, checks (+ re-runs), parked questions with recommendations.
```
